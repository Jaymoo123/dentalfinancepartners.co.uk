...
Draft a complete gold-reference rewrite brief matching the depth and 15-section structure of the gold-reference and birmingham city templates above. The brief must include: gap-mode diagnosis, the primary + secondary query targets, the cannibalisation/distinctiveness statement, the section-by-section content plan to ~${prev.diagnosis.target_word_count} words, the statute spine (every section number with its Act, to be verified), the competitor depth benchmark, internal-link targets within the live corpus, and the metaTitle/metaDescription/h1 plan. In the content plan, EXPLICITLY specify any comparison or reference TABLE the page needs (its columns + rows): a comparison/decision/"vs"/"options" page MUST plan a side-by-side comparison table, and a rates/bands/thresholds/deadlines page should plan a reference table where it aids scanning (plain HTML, no pricing). The brief must ALSO include these three named sections (consistent with the existing brief templates):
