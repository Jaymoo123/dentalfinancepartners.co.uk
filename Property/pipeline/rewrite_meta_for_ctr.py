...
WORKSHEET = Path('C:/Users/user/AppData/Local/Temp/property_analysis/phase2_worksheet.json')
BLOG_DIR = ROOT / 'Property' / 'web' / 'content' / 'blog'

TARGET_BUCKETS = {
    'A_PAGE2_CLIMBER_NO_CLICK',
    'B_HIGH_IMPR_NO_CLICK_PAGE1',
    'B_LOW_CTR',
    'B_DEEP_RANKING_NO_CLICK',
}

SYSTEM_PROMPT = """You are a senior UK property tax editor writing SERP meta titles and descriptions for Property Tax Partners (www.propertytaxpartners.co.uk).

The page you are writing meta for is RANKING in Google but earning ZERO or near-zero clicks. The current meta is failing. Your job is to rewrite both fields to maximise click-through rate while staying accurate to the article.

ABSOLUTE RULES — non-negotiable, the output will be rejected if any are violated:

1. TITLE: 50-60 characters total. Front-load with the dominant searcher intent. Include the year "2026" or "2025/26" if the article is tax-rate-bound. End with the brand pipe " | Property Tax Partners" ONLY if there are characters left under 60, otherwise omit the brand.

2. DESCRIPTION: 130-155 characters. Open with the searcher's outcome ("Cut your CGT bill", "Stop paying Section 24", "Save £2,400 on incorporation"). Include one specific number (rate, threshold, deadline, £-figure) from the article. Close with a benefit-clarity verb (read, calculate, claim, plan, avoid).
...
5. NO hype clichés: "ultimate", "unlock", "secret", "amazing", "discover", "supercharge". Property tax readers are practical, not entertainment-seeking.
...
8. NO fabrication. The £-figure or rate you cite must appear in the article body provided. If no specific number is in the article, cite a general benefit ("clear 12-step process", "with worked examples") instead of inventing one.
