 ...
 * DJH called, possibly days later. Now the first reply gets ONE immediate,
 * service-only acknowledgement that (a) confirms a human will call, and (b)
 * asks the two questions that make the handoff warmer: best time to call and
 * rough portfolio size. Their answer arrives as another inbound reply, lands in
...
function buildAckBody(firstName: string, bookingUrl: string): string {
  return (
    `Thanks ${firstName}, we got your reply. One of the team will call you soon. ` +
    `So we call at a good moment, reply with the best time for you and roughly how many properties you own. ` +
    `Prefer to pick a slot? ${bookingUrl}`
  );
}
  ...
  try {
    ...
    if (priorAcks === 0) {
      ...
      const sendResult = await sender.send({
        ...
        to: opts.replyTo,
        body: buildAckBody(ctx.firstName, ctx.bookingUrl),
      });
...
<p><strong>${lead.full_name}</strong> (already handed off) sent another ${opts.channel === "whatsapp" ? "WhatsApp" : "SMS"} reply:</p>
<div style="background:#f1f5f9;border-radius:6px;padding:10px 14px;font-style:italic;">&ldquo;${quoted
            .replace(/&/g, "&amp;")
            .replace(/</g, "&lt;")
            .replace(/>/g, "&gt;")}&rdquo;</div>
<p style="font-size:13px;color:#64748b;">If this changes the forward (for example a call time or portfolio detail), pass it on to DJH.</p>
</div>`,
          text: `${lead.full_name} (already handed off) replied by ${opts.channel}: "${quoted}"`,
        });
