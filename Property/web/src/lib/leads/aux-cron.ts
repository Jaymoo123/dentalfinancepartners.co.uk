...
export function buildIcsForSlot(params: IcsSlotParams): string {
  ...
  const uid     = `${params.leadId}-${params.date}-${params.windowKey}@propertytaxpartners.co.uk`;
  const summary = `Property tax review call (${params.label})`;
  const desc    = "A property tax specialist will call you in this window. Nothing to prepare.";

  return [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Property Tax Partners//Lead Review//EN",
    "BEGIN:VEVENT",
    `UID:${uid}`,
...
// ---------------------------------------------------------------------------
// Email constants (mirroring config/lead-nurture.ts)
// ---------------------------------------------------------------------------
...
export async function runLeadAuxScans(): Promise<{ reminders: number; nudges: number }> {
  ...
  try {
            ...
            try {
              ...
              const { html, text } = renderLeadServiceEmail({
                ...
                paragraphs: [
                  ...
                  "We will ring you then. They will have read your enquiry before they call, the call takes about 20 minutes, and there is nothing to prepare.",
    ...
    // Apply the upper bound (>= 2h ago) in JS since adminSelect can carry only one value per key.
