...
export const dividendTaxCalculator: GenericTool = {
  ...
  compute: (v) => {
    ...
    if (r.atBasic > 0) rows.push({ label: "Taxed at 10.75%", value: gbp(r.atBasic * DIV_BASIC) });
    if (r.atHigher > 0) rows.push({ label: "Taxed at 35.75%", value: gbp(r.atHigher * DIV_HIGHER) });
  ...
  faqs: [
    ...
    },
    {
      ...
      answer:
        "10.75% for dividends in the basic-rate band, 35.75% in the higher-rate band and 39.35% in the additional-rate band. Dividends sit on top of your other income, so the band depends on your total income.",
    },
    {
