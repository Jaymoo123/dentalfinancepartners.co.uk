 ...
 * content/resources/<topic>.md). Mirrors lib/blog.ts: gray-matter to split the
 * frontmatter from the HTML body, then addHeadingIds() so the in-page anchors
 * and any table of contents work. SERVER-ONLY (reads the filesystem).
 *
 * The guides are rendered by the resources/[topic] route as NOINDEX pages — they
 * are the value behind the email gate, deliberately kept out of the index so they
 * never compete with the ranking blog pages.
 */
import fs from "fs";
