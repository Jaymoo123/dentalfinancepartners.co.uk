...
import { buildStory, buildActivityRows, summariseVitals } from "@/lib/analytics/journey";
...
const GROUPS: Array<{ title: string; events: Array<[string, string]> }> = [
  ...
  { title: "Resources & nudges", events: [
    ["gate_view", "Download gates seen"], ["resource_unlocked", "Downloads unlocked"],
    ["support_opened", "Asked a question"], ["subscribe_view", "Newsletter shown"], ["subscribe_submitted", "Subscribed"],
  ]},
  { title: "Personalisation", events: [
    ["personalization_shown", "Prompts shown"], ["personalization_clicked", "Clicked through"], ["personalization_dismissed", "Dismissed"],
  ]},
  { title: "Issues", events: [["rage_click", "Rage clicks"], ["dead_click", "Dead clicks"], ["client_error", "JS errors"]] },
];

/** Worst-rating → panel styling for a Core Web Vital row / verdict chip. */
...
/** Turn a raw referrer host / utm source into a recognisable channel name. */
function friendlySource(host: string | null, utm: string | null): string {
  ...
  const map: Array<[RegExp, string]> = [
    ...
    [/(^|\.)(t\.co|twitter\.com|x\.com)/, "X / Twitter"], [/facebook\.com|fb\.me/, "Facebook"],
    [/linkedin\.com|lnkd\.in/, "LinkedIn"], [/reddit\.com/, "Reddit"], [/instagram\.com/, "Instagram"],
    [/youtube\.com|youtu\.be/, "YouTube"], [/(^|\.)yahoo\./, "Yahoo"],
  ];
  for (const [re, name] of map) if (re.test(raw)) return name;
  return host || utm || "direct";
}

/** "hero_book" → "Hero book" for the CTA list (raw id kept as a tooltip). */
function prettyCta(id: string): string {
...
export default async function VisitorTimelinePage({
  ...
  // Real-user performance this visitor actually experienced (Core Web Vitals).
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      ...
      <div className={`mt-3 rounded-xl border p-5 ${journey?.converted ? "border-emerald-300 bg-emerald-50" : "border-slate-200 bg-white"}`}>
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex shrink-0 flex-col items-end gap-1">
            ...
            {vitals.verdict && (
              <span
                className={`rounded-full px-3 py-1 text-xs font-bold ${VERDICT_STYLE[vitals.verdict]}`}
                title="How fast the site felt for this visitor (Core Web Vitals)"
              >
      ...
      {/* Performance they experienced (real-user Core Web Vitals) */}
      {vitals.verdict && (
        <div className="mt-3 rounded-xl border border-slate-200 bg-white p-4">
          <div className="flex items-center justify-between gap-3">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500">Performance they experienced</h3>
            <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${VERDICT_STYLE[vitals.verdict]}`}>
              {vitals.verdict}
            </span>
          </div>
          <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {vitals.rows.map((v) => (
              <div key={v.metric} className="flex items-center justify-between gap-2 rounded-lg bg-slate-50 px-3 py-2">
                ...
                <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${RATING_STYLE[v.rating] || "bg-slate-100 text-slate-600"}`}>
                  {v.display}
                </span>
              </div>
            ))}
          ...
          <p className="mt-2 text-[11px] text-slate-400">
            Worst sample across {vitals.pageLoads} page load{vitals.pageLoads > 1 ? "s" : ""}. Green = good, amber = needs work, red = poor. This is the speed and stability the visitor actually felt, not a lab score.
          </p>
        </div>
      )}
          ...
          <div className="mt-2 space-y-1 text-sm">
            {ctas.size === 0 ? <p className="text-xs text-slate-300">none</p> :
              Array.from(ctas.entries()).sort((a, b) => b[1] - a[1]).map(([id, n]) => (
                <div key={id} className="flex justify-between gap-2"><span className="break-all text-slate-700" title={id}>{prettyCta(id)}</span><span className="font-mono text-slate-400">{n}</span></div>
              ))}
          </div>
        </div>
      </div>
