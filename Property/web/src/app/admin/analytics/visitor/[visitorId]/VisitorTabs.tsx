...
import type { StorySession, ActivityRow } from "@/lib/analytics/journey";
import { clockTime } from "@/lib/analytics/journey";
        ...
        >
          <header className="border-b border-slate-100 bg-slate-50 px-4 py-2.5">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500">
              Visit {si + 1} · {new Date(s.startTs).toLocaleDateString("en-GB", {
                weekday: "short",
                day: "numeric",
                month: "short",
              })}
              ...
              <li key={i} className="relative flex gap-3 pb-3 last:pb-0">
                ...
                <div className="min-w-0">
                  ...
                  <span
                    className={cn(
                      "text-sm",
                      l.kind === "convert" && "font-semibold text-emerald-800",
                      l.kind === "reading" && "italic text-slate-500",
                      l.kind === "close" && "text-slate-500",
                      l.kind === "open" && "font-medium text-slate-900",
                      l.kind === "step" && "text-slate-800",
                    )}
                  >
                    {l.text}
                  </span>
...
function Activity({ rows }: { rows: ActivityRow[] }) {
  if (rows.length === 0) {
    return <p className="text-slate-400">No events for this visitor.</p>;
  }
  return (
    <ol className="overflow-hidden rounded-xl border border-slate-200 bg-white">
      {rows.map((r, i) => (
        <li
          key={i}
          className={cn(
            "flex items-baseline gap-3 border-b border-slate-50 px-4 py-1.5 text-sm last:border-0",
            r.kind === "engaged" && "bg-slate-50/60",
          )}
        >
          <span className="w-14 shrink-0 font-mono text-[11px] text-slate-400">
            {clockTime(r.ts)}
          </span>
          <span className={cn(r.kind === "engaged" ? "italic text-slate-500" : "text-slate-800")}>
            {r.text}
          </span>
        </li>
      ))}
