/**
 * Daily lead-nurture digest cron: fires at 07:00 UTC and sends a plain-text
 * operator summary covering the last 24 hours (funnel, deliverability, stuck
 * leads, failed sends). Quiet on dormant days (no sends, no stuck, no
 * failures) so the operator inbox stays clean.
 *
 * Auth pattern mirrors /api/cron/lead-nurture exactly (CRON_SECRET bearer,
 * timing-safe comparison). Both GET and POST are accepted so Vercel Cron can
 * fire it without extra config.
 *
 * Dormancy: short-circuits with ok:true, sent:false when LEAD_NURTURE_ENABLED
 * is not set, so the endpoint is safe to call at any time.
 *
