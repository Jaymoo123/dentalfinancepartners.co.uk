...
// INVARIANT: this form must always be rendered on a white or light surface. Labels are
// text-slate-900, consent copy is text-slate-500 and the privacy link is emerald-700, so a
// dark parent makes them invisible or unreadable. Wrap the call site in a light card.
const fieldClass =
      ...
      {status === "success" && !redirectOnSuccess && (
        <div role="status" className="rounded-lg border-2 border-emerald-200 bg-emerald-50 p-4">
          ...
          <p className="mt-2 text-sm text-emerald-900">
            For specialist advisory work we partner with Aswatax, Chartered Tax Advisers, so it may be
            their team who gets in touch.
          </p>
        </div>
      )}
