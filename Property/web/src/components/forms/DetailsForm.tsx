...
export default function DetailsForm({
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Client-side validation on the rendered field(s) only, against the shared
    // floors (kept in step with the server via field-floors).
    let ok = true;
    ...
    // Read the honeypot straight off the form; the server decides what to do.
    const form = e.currentTarget;
    const honeypot = String(
      new FormData(form).get("enquiry_ref") || "",
    ).trim();

    // Send only the rendered fields (plus token + honeypot); never email/message.
    ...
    try {
      ...
      if (res.ok && json.success) {
        if (json.invalidPhone) {
          // The number saved but does not look callable. Keep the phone box so
          // they can correct it and submit again.
          setStatus("idle");
          setPhoneError(
            "That number does not look right. Please check it and submit again.",
          );
          return;
        }
  ...
  return (
    <form onSubmit={onSubmit} className="text-left" noValidate aria-busy={status === "loading"}>
      ...
      <p className="mt-3 text-xs text-slate-500">
        We only use this to arrange your free property tax review. See our{" "}
        <a
          href="/privacy-policy"
          target="_blank"
