      ...
      <div className={`${siteContainerLg} relative z-10`}>
        ...
        <div className="grid gap-6 sm:gap-8 md:grid-cols-3">
          {testimonials.map((t) => (
            <figure
              key={t.who + t.detail}
              className="rounded-xl bg-white/5 p-6 sm:p-8 ring-1 ring-white/10 backdrop-blur-sm flex flex-col transition-colors hover:ring-emerald-500/60"
            >
              <div className="flex items-center gap-1" aria-label="Rated 5 out of 5">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} aria-hidden className="h-4 w-4 fill-emerald-400 text-emerald-400" />
                ))}
              </div>
