/**
 * Daily digest composer and runner for the startups-tech lead-nurture system.
 *
 * Mirrors Property/web/src/lib/leads/nurture-digest.ts with:
 *   - gatherDigestData('startups-tech')
 *   - Footer: "Founder Tax Partners"
 *   - site_key filter on stuck-leads query
 *   - JS-filter on failed-send events: leads.source === 'startups-tech'
 *
 * CONVENTIONS
...
export async function gatherDigestData(siteKey: string): Promise<DigestData> {
  ...
  const [health, control, stuckRes, failedRes] = await Promise.all([
    ...
    adminSelect<StuckViewRow>("vw_lead_nurture_stuck", {
      ...
      // ponytail: filter stuck leads to startups-tech site via join on leads table;
  ...
  if (failedEvents.length > 0) {
    ...
    // JS-filter: only include events whose lead belongs to startups-tech.
    failedSends = failedEvents
      .filter((e) => nameMap.get(e.lead_id)?.source === "startups-tech")
      .map((e) => {
...
export function composeDigestEmail(d: DigestData): {
  ...
  lines.push(`Automated digest from the Founder Tax Partners lead-nurture system.`);
...
export async function runNurtureDigest(): Promise<{ sent: boolean }> {
  const d = await gatherDigestData("startups-tech");
