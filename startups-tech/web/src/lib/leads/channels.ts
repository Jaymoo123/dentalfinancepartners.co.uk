/**
 * startups-tech lead-nurture channel sender: implements the engine's ChannelSender
 ...
 *
 * Mirrors Property/web/src/lib/leads/channels.ts with Founder Tax Partners
...
function serviceFrom(): string {
  const name = process.env.LEAD_SERVICE_FROM_NAME || "Founder Tax Partners";
...
export function twilioAuth(): TwilioAuth | null {
  ...
  if (account.startsWith("SK")) {
    console.error(
      "[lead-nurture/startups-tech] TWILIO_ACCOUNT_SID looks like an API Key SID (SK...). " +
        "It must be the AC... Account SID.",
    );
    return null;
  }
