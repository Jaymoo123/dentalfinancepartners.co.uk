...
// nothing reads them, so rather than invent speculative startup keys the map is
// empty and only the neutral fallback is live. Populate it when the enrichment
// layer actually emits startup intent categories.
const CATEGORY_PHRASE_MAP: Record<string, string> = {};

/**
 * Neutral fallback phrase when no clean echo exists, derived from the
 * intent category then role. Falls back to a generic startup tax phrase.
...
export function categoryPhrase(
  ...
  return "get your startup's tax position clear";
}
