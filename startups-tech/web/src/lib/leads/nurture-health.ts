/**
 * Observability health reader and guardrail evaluator for the startups-tech
 * lead-nurture system.
 *
 * Mirrors Property/web/src/lib/leads/nurture-health.ts exactly, with
 * 'property' literals replaced by 'startups-tech' and the Founder Tax Partners
 * brand in operator alerts.
 *
 * CONVENTIONS
...
export async function runNurtureGuardrails(opts: {
  ...
  const health = await getNurtureHealth("startups-tech");
  if (!health) {
  ...
  if (!alreadyAlerted && process.env.RESEND_API_KEY) {
    ...
    try {
      await getResend().emails.send({
        from: getFromAddress(),
        to: resolveLeadTo("startups-tech"),
        subject,
        text,
      });
      ...
      console.error("[nurture-health/startups-tech] guardrail alert send failed", err);
    }
  }
  ...
  if (opts.autopauseEnabled && verdict.shouldPause && !control.paused) {
    try {
      ...
      console.error("[nurture-health/startups-tech] auto-pause failed", err);
    }
  }
