 ...
 *
 * House style: no em-dashes. British English. Founder Tax Partners brand.
 */
...
export function buildIcsForSlot(params: IcsSlotParams): string {
  ...
  const uid     = `${params.leadId}-${params.date}-${params.windowKey}@foundertaxpartners.co.uk`;
  const summary = `Startup finance review call (${params.label})`;
  const desc    = "A startup accountant will call you in this window. Nothing to prepare.";

  return [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Founder Tax Partners//Lead Review//EN",
    "BEGIN:VEVENT",
    `UID:${uid}`,
...
export async function runLeadAuxScans(): Promise<{ reminders: number; nudges: number }> {
  ...
  try {
        ...
        if (inT24) {
          ...
          if (sendRowId) {
            try {
              ...
              await markSendStatus(sendRowId, "failed");
              console.error("[lead-aux-cron/startups-tech] T-24 email error", err);
            }
          }
        }
          ...
          if (inT2 && inSendWindow(now, true)) {
            ...
            if (sendRowId && lead.phone) {
              try {
                ...
                await markSendStatus(sendRowId, "failed");
                console.error("[lead-aux-cron/startups-tech] T-2 SMS error", err);
              }
        ...
        if (eligibleIds.length > 0 && inSendWindow(now, true)) {
          ...
          for (const lead of nudgeLeadsRes.data) {
            ...
            try {
              ...
              await markSendStatus(sendRowId, "failed");
              console.error("[lead-aux-cron/startups-tech] abandoned nudge error", err);
            }
          }
        }
      }
    ...
    console.error("[lead-aux-cron/startups-tech] unexpected top-level error", err);
  }
