/**
 * "Complete your details" endpoint for Founder Tax Partners.
 *
 ...
 * (startups-tech has no Twilio Lookup in the submit path), so accept saved phone
...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error("[leads/complete/startups-tech] lead lookup failed", e);
  ...
  if (patchKeys.length > 0) {
    try {
      ...
      console.error("[leads/complete/startups-tech] lead update failed", e);
      return NextResponse.json({ success: false, error: "server" }, { status: 500 });
    }
  }
  ...
  try {
    ...
    console.error("[leads/complete/startups-tech] audit event failed (non-fatal)", e);
  }
  ...
  try {
    ...
    console.error("[leads/complete/startups-tech] confirmed event failed (non-fatal)", e);
  }
  try {
    await adminUpdate(
      "lead_nurture_state",
    ...
    console.error("[leads/complete/startups-tech] nurture state promote failed (non-fatal)", e);
  }
  try {
    await adminUpdate(
      "leads",
    ...
    console.error("[leads/complete/startups-tech] lead status promote failed (non-fatal)", e);
  }
