/**
 * Server chokepoint for Founder Tax Partners lead submission.
 *
...
const sharedHandler = createLeadSubmitHandler({ source: "startups-tech" });
...
export async function POST(req: NextRequest) {
  ...
  const source = String(body.source ?? "startups-tech").toLowerCase();
  ...
  if (!isResource) {
    try {
      ...
      console.error("[leads/submit/startups-tech] enrol/instant-touch failed", e);
    }
  }
