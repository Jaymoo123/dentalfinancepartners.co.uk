/**
 * startups-tech lead-nurture cron: fires due follow-up steps for leads that have
 * not yet responded. Driven by Vercel Cron (hourly).
 *
 * Dormancy (two layers):
 *   Auth: CRON_SECRET must match (bearer), or 401.
 *   Master arm: LEAD_NURTURE_ENABLED. Until that is set, the run short-circuits
 *   with zero processing so no lead is ever advanced while the system is dormant.
 *
...
import { buildStartupsTechLeadNurtureConfigs, buildLeadMessageContext } from "@/config/lead-nurture";
...
async function run(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.error("[lead-nurture-cron/startups-tech] heartbeat write failed", err);
  }

  const configs = buildStartupsTechLeadNurtureConfigs();
  ...
  if (cronArmed) {
    try {
      ...
      console.error("[lead-nurture-cron/startups-tech] guardrail scan failed", err);
    }
  }
