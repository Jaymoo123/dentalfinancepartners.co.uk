# Medical — Session C — start here

**You are Session C**, one of three parallel Claude Opus 4.7 sessions rewriting Medical site pages. Your assignment is fixed — Sessions B and C have their own pre-assigned lists, you do not coordinate with them in real time.

## Worktree

You run inside the worktree at `C:/Users/user/Documents/Accounting-wt-medical-c/` on branch `medical-rewrite-c`. **Stay inside that worktree.** Build commands run from that worktree's root. The other sessions are in `Accounting-wt-medical-b/` and `Accounting-wt-medical-c/` on their own branches — they will never collide with your `.next/` or mid-edit YAML.
...
5. **`docs/medical/page_rewrite_tracker.md`** — the master tracker. Find the "Session C pages" section. That's your work list.
...
## Your Session C pages (15 assigned)
...
| # | Slug |
|---|---|
| C1 | becoming-gp-partner-financial-implications |
| C2 | gp-accountant-bristol |
| C3 | gp-accountant-glasgow |
| C4 | gp-accountant-london |
| C5 | gp-accountant-services-complete-guide |
| C6 | gp-accounting-software |
| C7 | gp-financial-planning |
| C8 | gp-partner-vs-salaried-gp-tax-comparison |
| C9 | gp-payroll-services |
| C10 | gp-tax-deductions-complete-list-2026 |
| C11 | locum-doctor-expenses-what-you-can-claim |
| C12 | locum-doctor-self-assessment-filing-guide |
| C13 | medical-practice-incorporation-step-by-step |
| C14 | nhs-pension-for-locums-form-a-form-b |
| C15 | nottingham-gp-accountant |

**Pillars in your list:** C5 (GP accountant services complete), C10 (GP tax deductions complete), C13 (Medical practice incorporation). Treat with extra care — these are the comprehensive references that other pages link to.

## When you're done

Update the summary table at the top of `docs/medical/page_rewrite_tracker.md`, then append a `[SESSION_C_COMPLETE]` paragraph to `docs/medical/site_wide_flags.md` noting any pages where you could not get a clean rewrite done and why. Then stop.

Begin with C1 (`becoming-gp-partner-financial-implications`).
