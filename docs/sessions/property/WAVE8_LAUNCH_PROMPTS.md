# Wave 8 launch prompts

**Status as of 2026-05-25:** Wave 8 prelaunch complete. Main at `b48406e` (Stage 2 brief commit). Three worktrees on branches `property-wave8-{a,b,c}` standing up at `b48406e` per §16.25 ff-verify. HP-lock state: 8 commits since Wave 7 close — §21.A CT three-figure framework (cc00f68) + §17.8/9/10 FIG/TRF/rebasing (a170f02) + §22.5 refresh + §22.X IHT LTR (b5fc91b) + §28 NEW Transactions in UK land (adc33f4) + §25.12 LRR mini-lock (6c88708) + cannib re-check (2a01076) + Stage 1a brief seeds (692438b) + Stage 1b corrections + §29 NEW VAT cluster (431f62b) + artefact shells (2f0fd45) + Stage 2b HP corrections (a63a667) + Stage 2 briefs (b48406e). 29 full briefs at `briefs/property/wave8/`. 51 substantive drift catches surfaced across HP-lock + Stage 1a + Stage 1b + Stage 2 — sessions inherit a strongly-corrected baseline per §16.46.

**Manager-side Q&A watcher to be armed at launch.** Wave 8 watcher uses **`## Q-\d+` heading-count pattern per §16.41(d)** (template-immune; ran cleanly through Wave 7 with zero false fires). Heading count is monotonic; new Q-N headings trigger notifications; template baseline does not match.
...
**Open in:** `C:/Users/user/Documents/Accounting-wt-property-wave8-a/`

```
You are Session A for Wave 8 of the Property Net-New Program. Working in worktree `Accounting-wt-property-wave8-a` on branch `property-wave8-a` at HEAD `b48406e` (will refresh if more prep commits land before launch). Bucket: FIG regime + non-dom IHT + leaving-UK depth (10 briefs A1-A10).

**Q&A discipline (§16.15 + §16.37 critical):** when raising a Q to manager, append to `C:/Users/user/Documents/Accounting/docs/property/wave8_questions_session_A.md` via ABSOLUTE PATH from your worktree. NEVER append to the worktree's relative-path copy of the file. Same discipline for tracker, flags, discovery logs. Manager polls main's path only; relative-path edits are NOT seen.

Read first, in this order:
1. docs/sessions/property/WAVE8_SESSION_A_START_HERE.md (your task brief + bucket overview + 11 critical drift watchpoints)
...
3. docs/property/house_positions.md — for Session A: §17 (Wave 2 expat + Wave 8 §17.8 FIG + §17.9 TRF + §17.10 rebasing) + §15.6 (IHT LTR headline) + §22.5 (Stage 1b refresh — SPOUSAL s.267ZC not generic) + §22.X (NEW Wave 8 IHT LTR depth) + §16 (DTAs)
4. Your 10 briefs at briefs/property/wave8/:
...
5. docs/property/wave8_page_tracker.md (your A1-A10 rows; flip ⬜ → in_progress when you claim; ABSOLUTE PATH)
6. docs/property/wave8_site_wide_flags.md (cross-session flags; ABSOLUTE PATH)
7. docs/property/wave8_questions_session_A.md (your Q&A channel; ABSOLUTE PATH)
8. docs/property/wave8_discovery_log_session_A.md (your discoveries; ABSOLUTE PATH)
...
Critical §16.35 + §16.45 per-write verification (11 watchpoints from Wave 8 prep 51 drift catches):
...
**Open in:** `C:/Users/user/Documents/Accounting-wt-property-wave8-b/`

```
You are Session B for Wave 8 of the Property Net-New Program. Working in worktree `Accounting-wt-property-wave8-b` on branch `property-wave8-b` at HEAD `b48406e`. Bucket: Transactions in UK land + trading-vs-investment + developer tax (10 briefs B1-B10).

**Q&A discipline (§16.15 + §16.37 critical):** when raising a Q to manager, append to `C:/Users/user/Documents/Accounting/docs/property/wave8_questions_session_B.md` via ABSOLUTE PATH from your worktree. NEVER append to the worktree's relative-path copy. Same discipline for tracker, flags, discovery logs.

Read first, in this order:
1. docs/sessions/property/WAVE8_SESSION_B_START_HERE.md (your task brief + bucket overview + 13 critical drift watchpoints)
...
3. docs/property/house_positions.md — for Session B: §28 (NEW Wave 8 cluster Transactions in UK land + Stage 2b refresh) + §21 (Wave 4 LtdCo) + §21.A (Wave 8 CT three-figure framework + Stage 1b expansive s.18N(3)) + §25 (Wave 6 CAA 2001) + §25.11 (Wave 7 s.198 fixtures) + §25.12 (Wave 8 LRR mini-lock + Stage 2b s.1178 + s.1175 range corrections) + §22.1 (Pawson BPR investment line) + §5 (CGT) + §1 + §1.A (Wave 7 SDLT partnership)
4. Your 10 briefs at briefs/property/wave8/:
...
5. docs/property/wave8_page_tracker.md (your B1-B10 rows; ABSOLUTE PATH)
6. docs/property/wave8_site_wide_flags.md (ABSOLUTE PATH)
7. docs/property/wave8_questions_session_B.md (ABSOLUTE PATH)
8. docs/property/wave8_discovery_log_session_B.md (ABSOLUTE PATH)
...
Critical §16.35 + §16.45 per-write verification (13 watchpoints from Wave 8 prep — read all in START_HERE):
...
**Open in:** `C:/Users/user/Documents/Accounting-wt-property-wave8-c/`

```
You are Session C for Wave 8 of the Property Net-New Program. Working in worktree `Accounting-wt-property-wave8-c` on branch `property-wave8-c` at HEAD `b48406e`. Bucket: VAT operational depth on commercial property (9 briefs — C4 dropped at cannib audit; C5 + C6 reframed at Stage 1a; final lineup C1, C2, C3, C5, C6, C7, C8, C9, C10).

**Q&A discipline (§16.15 + §16.37 critical):** when raising a Q to manager, append to `C:/Users/user/Documents/Accounting/docs/property/wave8_questions_session_C.md` via ABSOLUTE PATH from your worktree. NEVER append to the worktree's relative-path copy. Same discipline for tracker, flags, discovery logs.

Read first, in this order:
1. docs/sessions/property/WAVE8_SESSION_C_START_HERE.md (your task brief + bucket overview + 7 critical drift watchpoints)
...
3. docs/property/house_positions.md — for Session C: §29 (NEW Wave 8 VAT architectural cluster — single primary anchor for all 9 picks; sub-sections §29.1-§29.11 with Stage 2b corrections on s.26B drop + Sch 7A Group 7 reframe + reg 107A SI 2002/1074 cite)
4. Your 9 briefs at briefs/property/wave8/:
...
5. docs/property/wave8_page_tracker.md (your C-bucket rows; ABSOLUTE PATH)
6. docs/property/wave8_site_wide_flags.md (ABSOLUTE PATH)
7. docs/property/wave8_questions_session_C.md (ABSOLUTE PATH)
8. docs/property/wave8_discovery_log_session_C.md (ABSOLUTE PATH)
...
Critical §16.35 + §16.45 per-write verification (7 watchpoints from Wave 8 prep — read all in START_HERE):
