# Property Track 1 — Session A — start here

**You are Session A.** One of three parallel Claude Opus 4.7 sessions writing NET-NEW Property pages from scratch. Your assignment is fixed — Sessions B and C have their own pre-assigned lists.
...
You run inside `C:/Users/user/Documents/Accounting-wt-property-track1-a/` on branch `property-track1-a`. **Stay inside that worktree.** Build commands run from that worktree's root. Sessions B and C are in their own worktrees on their own branches; you will never collide on `.next/` or mid-edit YAML.
...
6. **`docs/property/track1_page_tracker.md`** — master tracker. Your assigned pages are in the "Session A pages" table.
...
18. **Append discoveries** to `docs/property/track1_discovery_log_session_A.md` (append-only) — adjacent topics, calculator ideas, components, existing-page issues, cross-niche links.
...
**Log to `docs/property/track1_discovery_log_session_A.md`** (append-only; for observations that don't need immediate action but feed future waves):
...
- **File:** `docs/property/track1_questions_session_A.md` (append-only; only YOU and the manager write to it)
...
## Your Session A pages (10 assigned — SDLT depth)

In assignment order. Work top to bottom. Claim ONE at a time.

| # | Slug |
|---|---|
| A1 | sdlt-5-percent-surcharge-refund-claim-process |
| A2 | sdlt-six-dwellings-non-residential-election |
| A3 | sdlt-sub-sale-relief-mechanics |
| A4 | sdlt-shared-ownership-staircasing |
| A5 | sdlt-group-relief-for-corporate-landlord-portfolios |
| A6 | sdlt-on-probate-property-transfers |
| A7 | sdlt-non-resident-2-percent-surcharge |
| A8 | sdlt-refund-scams-how-to-avoid |
| A9 | sdlt-mixed-use-property-classification |
| A10 | sdlt-leasehold-extension-vs-fresh-purchase |

**SDLT bucket discipline:** All your pages are SDLT. They must NOT read like the same page with different headings. Each brief's framing differentiator defines a distinct angle. Honour it. If you find yourself writing the same H2 outline for two pages, stop and rework.

## When you're done with all 10

Update the summary at the top of `docs/property/track1_page_tracker.md` and append a `[SESSION_A_COMPLETE]` paragraph to `docs/property/track1_site_wide_flags.md`. Then stop.

Begin with A1 (`sdlt-5-percent-surcharge-refund-claim-process`).
