# Property Wave 8, Session C, start here (VAT operational depth on commercial property)
...
**Bucket C is statute-isolated** — no FA 2025 reform territory; the per-write §16.35 verification load is on rates, thresholds, SI numbers, and form numbers (which drift by Budget and SI amendment). The §29 Wave 8 NEW VAT cluster lock gives you architectural anchors; sessions write the operational depth.

## Q&A discipline (§16.15 + §16.37 CRITICAL — read this first)

When raising a Q to manager, append to `C:/Users/user/Documents/Accounting/docs/property/wave8_questions_session_C.md` via **ABSOLUTE PATH** from your worktree. **NEVER** append to your worktree's relative-path copy. Manager polls main's path only. Same discipline for tracker (`wave8_page_tracker.md`), flags (`wave8_site_wide_flags.md`), discovery (`wave8_discovery_log_session_C.md`).

## Worktree

You run inside `C:/Users/user/Documents/Accounting-wt-property-wave8-c/` on branch `property-wave8-c`. **Stay inside that worktree.** Sessions A/B are in their own worktrees.
...
3. **`docs/property/house_positions.md`** — for Session C: **§29** (NEW Wave 8 VAT architectural cluster — the SINGLE primary anchor for all 9 picks). Sub-sections §29.1 (VATA 1994 overview with Stage 2b corrections on s.26B drop + Sch 7A Group 7 reframe), §29.2 (Sch 9 Gr 1 + 14 carve-outs incl para (ka) self-storage standard-rated), §29.3 (Sch 10 option-to-tax + form numbers), §29.4 (TOGC), §29.5 (CGS — SI 1995/2518 Part XV regs 112-116), §29.6 (partial exemption with Stage 2b SI 2002/1074 reg 107A inserting cite correction), §29.7 (reduced rate 5%), §29.8 (registration thresholds).
4. **Your assigned briefs** at `briefs/property/wave8/*.md`.
5. **`docs/property/wave8_page_tracker.md`** — your C1-C10 rows (C4 dropped; 9 picks).
...
## CRITICAL drift watchpoints (Wave 8 prep surfaced 51 catches — 7 directly Bucket C; read all before writing)
...
| Pos | Slug | Brief at |
|---|---|---|
| C1 | `option-to-tax-vata-1994-schedule-10-commercial-property-20-year-lock` (PILLAR) | briefs/property/wave8/ |
| C2 | `option-to-tax-revocation-routes-6-month-cooling-off-20-year-exit-landlords` | briefs/property/wave8/ |
| C3 | `transfer-of-going-concern-togc-commercial-property-option-matching-vat-free` | briefs/property/wave8/ |
| C5 | `vat-partial-exemption-special-method-approval-landlords-standard-method-override-mechanics` | briefs/property/wave8/ |
| C6 | `vat-commercial-to-residential-conversion-5-percent-reduced-rate-developer-recovery-mechanics` | briefs/property/wave8/ |
| C7 | `disapplication-option-to-tax-schedule-10-paragraph-12-residential-conversion` | briefs/property/wave8/ |
| C8 | `vat-storage-facility-lettings-schedule-9-group-1-supply-categorisation-landlords` | briefs/property/wave8/ |
| C9 | `vat-registration-threshold-90k-landlords-april-2024-group-registration` | briefs/property/wave8/ |
| C10 | `landlord-vat-recovery-professional-fees-capital-costs-commercial-property` | briefs/property/wave8/ |
