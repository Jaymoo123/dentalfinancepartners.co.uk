...
**STALE-FIGURE CORRECTIONS — RESOLVED + DEPLOYED (2026-06-10, user-approved).** PR #6 merged (main CI green run 27311338275): SL plan 1/2/4 thresholds → 2025/26; R&D model → 30% ERIS threshold, intensive branch 1.86×14.5% payable (no CT haircut), RDEC branch unchanged, page copy updated; Employment Allowance label → £10,500. Golden tests updated deliberately with derivations; TOOLS.md findings marked RESOLVED with sources. **Production deploy via `vercel deploy --prod` (env-var project override from repo root — root `.vercel` link belongs to digital-agency; per-site deploys set `VERCEL_ORG_ID`/`VERCEL_PROJECT_ID`)**. Live verification: R&D page serves 30% threshold · Employer NI serves £10,500 · `/embed/*` routes live (200) · `/feed.xml` live (200) — this deploy also shipped GAP-8 + GAP-2 to production (embeds, registry pages, shared schema, RSS, llms-full).
...
**GAP-3 PRECONDITIONS PASSED (manager, 2026-06-11) — branch `phase-b-console`. Accrual calendar gate WAIVED by user 2026-06-11** ("generalist IS deployed and collecting real clicks, so we're building it now") — readiness query stands at 9 human sessions; OB-03 acceptance may be partial until volume accrues, noted not hidden.
- **Live vw_* audit (precondition 1):** 27 views in prod, ALL parameterise by `site_key`. ONE hardcode found: `vw_channel_conversion_geo` classified "internal" via `'%propertytaxpartners%'` only. Fixed by migration `20260611000002_channel_view_registry_internal.sql` — internal = referrer matches the site's OWN `sites.domain` (www-stripped, registry join). **Property byte-identical proven** by same-instant old-logic-vs-new-view EXCEPT diff (0 rows both directions; naive before/after md5 snapshots differ only because live traffic landed between queries — same-instant diff is the valid method).
- **Visits-to-conversion attribution fix applied:** uncommitted file `20260610000001_fix_visits_to_conversion_attribution.sql` found in tree, never applied to prod, and version-COLLIDED with `_add_generalist_to_leads_source.sql`. Renamed → `20260611000001`. First apply FAILED: it referenced `web_sessions.created_at`, which does not exist (live column is `started_at`) — the D2 lesson firing again; corrected and applied. Both migrations recorded in `supabase_migrations.schema_migrations`.
- **Precondition 2 (generalist populates views):** funnel_v2 = 5 rows · channel = 8 · visits_conv = 4 · page_engagement = 8 · calculator_conversion = 0 (expected — tools deployed 2026-06-10, no human calc sessions yet; view structure confirmed parameterised).
- Generalist accrual at check: 48 sessions total, 9 human_confirmed non-bot non-test.

**GAP-3 — ACCEPTED (2026-06-11, manager verification). Local Verify lines independently re-run; live-data lines (OB-03/04/05/06) deferred to post-deploy.**
- 192/192 tests reproduced (176 prior + 16 console: query layer vs fixture rows, auth gate timing-safe/cookie/denial paths). `next build` re-run by manager: GREEN, console routes dynamic.
- **OB-01 re-verified at RUNTIME** (built server on :3456, not just code review): no credential → 307 to login · wrong key POST → 303 `?error=1` · right key POST → 303 to dashboard + `Set-Cookie: __console_session=<sha256 token, not the raw key>; Max-Age=86400; Path=/admin/analytics; HttpOnly; SameSite=Strict; Secure` · login page emits `noindex, nofollow` · with cookie → 200 and the dashboard renders LIVE store data (session/lead panels populated; exactly 4 "not operated on this site" states: experiments, personalisation, nurture, lead-intent — per spec, no fake zeros).
- **OB-02 re-verified by independent grep**: zero `vw_*`/table names in components or pages outside `adminData.ts` (only hits are inside human-readable not-operated explanation strings). No hardcoded site key; pages read `niche.content_strategy.site_key`.
- Auth design note (within spec bounds, recorded): session token = sha256(key-derived constant) — deterministic until key rotation; Max-Age is client-enforced only. Acceptable for env-key+cookie posture; HMAC-with-expiry would be the upgrade if posture ever tightens.
- **Deferred to post-deploy (live data):** OB-03 (panel answers — partial until accrual; 9 human sessions at acceptance), OB-04 (lead→journey click-through needs a real converted lead), OB-05 (live error grouping), OB-06 (`web_vital` rows — capture already proven live in GAP-2). Test-row cleanup (v_mgrtest_2026 + roster) executes once OB panels verify against real data.
- **Deploy precondition for operator:** `ADMIN_DASHBOARD_KEY` must be set in Vercel env (generalist project) BEFORE deploy — console returns only the login page without it; no secret committed anywhere.

**GAP-3 Stage 2 — DONE (Sonnet, 2026-06-11, branch `phase-b-console`)**
- Generalist console mounted at `/admin/analytics` with shared auth (OB-01 design). 6 new route files created; `ADMIN_DASHBOARD_KEY` added to `generalist/web/.env.local.example` (PF-05).
- Routes: `src/app/api/admin/login/route.ts` (POST handler, in-process rate limiting 10 fail/10 min/IP via `ipFailures` Map, `verifyConsoleKey`, `buildSessionCookie`, redirect on success), `src/app/admin/analytics/login/page.tsx` (GET, noindex, error state for wrong-key + rate_limit), `src/app/admin/analytics/checkAuth.ts` (server-side helper: reads `__console_session` cookie, `verifySessionCookie` against `ADMIN_DASHBOARD_KEY`), `src/app/admin/analytics/page.tsx` (main dashboard, 5-tab DashboardTabs: overview/visitors/experiments/behaviour/conversion), `src/app/admin/analytics/trends/page.tsx` (Sparkline-based, no recharts — generalist has no recharts), `src/app/admin/analytics/leads/page.tsx` (paginated lead list), `src/app/admin/analytics/visitor/[visitorId]/page.tsx` (visitor detail with journey story + activity tabs), `src/app/admin/analytics/visitor/[visitorId]/VisitorTabs.tsx` (client tab switcher).
- **Site key pattern:** `niche.content_strategy.site_key` (not `source_identifier` — generalist config shape). No site literal hardcoded anywhere in console pages.
- **"Not operated on this site" panels:** Experiments tab: explicit NotOperatedPanel for A/B experiments + personalisation. Conversion tab: explicit NotOperatedPanel for lead-intent enrichment + nurture engine. These render explanatory text, never empty/zero panels that would read as data.
- **RSC boundary:** `DashboardTabs`, `VisitorsTable`, `CountrySelect` are `"use client"` components receiving only `ReactNode`/`VisitorRow[]`/`string[]` props. No function-bearing objects cross the server/client boundary (lesson from GAP-2 defect). Verified by `next build` (the only gate that catches RSC errors).
- **`next build` GREEN** — 679 pages, 0 type errors, 0 build errors. `ƒ` (dynamic) on all 5 console routes + login API.
- **OB-01 VERIFIED:** grep confirms no `?k=` or credential param in any console route file; CONSOLE_NOINDEX_META applied to all 5 pages (main, login, trends, leads, visitor detail).
- **OB-02 VERIFIED:** zero direct table/view names in any component file; only `adminData.ts` touches `vw_*` views and `web_events`/`leads`; `journey.ts` comment mention is a documentation string only.
- **OB-03/04/05/06:** deferred to manager live-data verification (require real accrued data / live browser session / form submission). OB-03 note: 9 human sessions at Stage 2 build; funnel + channel panels will render with partial data. Per spec STOP: if panels can't be answered, user decides whether to extend window; panels are built, not empty.

**GAP-3 Stage 1 — DONE (Sonnet, 2026-06-11, branch `phase-b-console`)**
- `packages/web-shared/console/` created: `adminData.ts` (all query fns parameterised by siteKey, zero site literals), `consoleAuth.ts` (OB-01 design: timing-safe compare via `crypto.timingSafeEqual`, `deriveSessionToken` SHA-256 of `console-session:<key>`, `buildSessionCookie` HttpOnly+Secure+SameSite=Strict+Path=/admin/analytics, `clearSessionCookie`, `verifySessionCookie`), `journey.ts` (site-agnostic version of Property's journey.ts — `ruleLabel` replaced with a default-label fallback + optional custom fn param, no Property-specific intent imports), `components/Sparkline.tsx`, `components/SnapshotCard.tsx`, `components/DashboardTabs.tsx`, `components/CountrySelect.tsx` (no `?k=` in nav URL — OB-01), `components/VisitorsTable.tsx` (no `?k=` in visitor hrefs — OB-01), `index.ts` barrel.
- Export map entries added to `packages/web-shared/package.json` (10 new entries for `./console*`).
- **48 new tests in `console/console.test.ts`** covering: verifyConsoleKey (6 tests: correct/wrong/empty/case), deriveSessionToken (3: hex format/deterministic/distinct), verifySessionCookie (4: valid/tampered/undefined/empty-key), buildSessionCookie (6: name/HttpOnly/SameSite/Path/Secure/token-value), clearSessionCookie (2), humanise (9: various event types + custom ruleLabel), readingTime (3), clockTime (1), formatVital (3), summariseVitals (3: no vitals/all-good/poor), buildActivityRows (3: merge pings/suppress vitals/preserve events), buildStory (5: sessions/open-line/convert/reading/empty). **192/192 tests green** (48 new + 144 prior).
- **Stage 1 acceptance confirmed:** suite green; all 6 sites tsc clean with zero consumers (no-coupling proof).
- OB-02 note: `adminData.ts` only touches `vw_*` views + `web_events` + `leads` (direct table, PII, service-role only); no table names in any component. OB-02 verify line will be run at Stage 2 acceptance.
