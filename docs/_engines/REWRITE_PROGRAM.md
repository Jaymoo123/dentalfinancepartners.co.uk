...
- **A rule that protects a measurement must never be allowed to preserve a defect (LOCKED 2026-08-26, applied six times in two days on Medical batch 3).** An EXTEND grade, a `monitored_pages` freeze and a byte-identity requirement all exist to protect equity that lives in structure and positioning. **They restrict STRUCTURE AND POSITIONING, never TRUTH.** Cleared inside frozen copy, in ascending order of what they permit: a stale year tag where the figure still holds; a superseded figure, corrected together with its tag; a live factual error; an ownership breach, deleted down to one sentence plus a link; and any narration of our own process to the reader. Not cleared: headings, ordering, and any phrasing the equity register shows the page actually matches. **The test before any such edit is "does the page still match every query it matched before", not "did I touch frozen copy".** Where the page carries no measured queries at all, the freeze is protecting nothing and the bar is lower still.
- **A search snippet is not a source, including when it arrives from the conductor (2026-08-26).** A figure passed down from a web-search summary was refused by a writer who could not reconcile it with the published specification, and the writer was right. Only the primary instrument counts. This applies with the same force to a figure quoted in a research pack, in a sibling page, or in this file.
