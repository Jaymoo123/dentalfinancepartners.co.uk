...
**IN FLIGHT (2026-07-06 late): Dentists R2 build (Workflow wf_028b58e6-fa6; brief `d67da830`; --primary token invariant is a BLOCKING QA check) ∥ Medical R1 build (Workflow wf_fa2453c9-e89; flat-routing divergences encoded: topic-as-prop for blog posts, NO dynamic [category], GA4 into ConsentedScripts, LeadForm full replacement, keyTakeaways surface added).** Owner authorization: "ship D2 and D3 when green then start medical" — all covered by standing ship-when-green; Medical BUILD started early in parallel (disjoint trees), its deploys queue behind Dentists'. Dentists R3 brief committed and ready (R2-before-R3 dependency respected). Then: cfp+cis twin → agency (AMBER: relink, house_positions, newsletter retirement first).
