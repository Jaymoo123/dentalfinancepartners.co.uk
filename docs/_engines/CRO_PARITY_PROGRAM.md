...
**(e) OWNER ACTIONS — RESPONSES RECEIVED 2026-07-06:**
1. Resend from-domains: **owner: fine under the Property domain for the time being, will revisit later.** Emailed resource delivery stays OFF (on-page delivery live) until then.
2. Phones: **owner directive: NO phone numbers displayed on any site.** VERIFIED SATISFIED 2026-07-06: estate-wide live sweep found zero rendered phones, zero tel: links, zero JSON-LD telephone on all 7 non-Property sites (config values exist but are never emitted; the only live match is the lead form's input placeholder "07700 000000", a format hint for the visitor's own number, which stays). No changes needed.
3. CI push-trigger extension: explained to owner ("not sure what that is") — NOT approved, not doing. prod<->origin/main reconciliation remains a future offer.
4. /newsletter disposition: **owner approved as-is** (delinked, cron disarmed, page live).
5. WS8 keyTakeaways backfills: **owner pre-approved ("okay")** — folded into the parked content backlog (it is content-engine work).
6. Wave-1 modal-cap backport (gen/sol DeepScroll vs ExitIntent shared cap, Dentists pattern) at next wave-1 touch — unchanged.
7. CONTENT SWEEPS: **owner: "park the content sweeps for a new agent"** — full self-sufficient handoff written at `docs/_engines/CONTENT_SWEEPS_BACKLOG.md` (R&D cluster, employer-NIC, blog-md credential sweeps incl. the src/-scope lesson, WS8 backfills, fleet labels/meta trim, OPEN-ITEM verifications, owner-decision context).
