...
- **Residual browser-only checks (~5 min manual, before any deploy sign-off):** AN-01 (opt-out toggle stops beacons live), AN-02 (ids in devtools), AN-04 (pagehide beacon + cold-load first event), AN-05 client-half (scrubProps in a real page), LD-03/LD-05 (real form submit: honeypot + stitched lead). All covered at unit-suite + code-review level; flagged, not assumed.
- Test rows in prod marked by naming convention (`s_mgrtest*`, `v_mgrtest_2026`, page `/mgr-verify`) — left in place per instruction.
