...
- Email routing rule: **no lead from any site is copied to any external firm.** `DEFAULT_PARTNER_CC` has been empty since 2026-07-17, and the production health probe returns `"ccSet": false` (checked 2026-08-17), so every lead goes to the internal inbox only. A CC only happens if `LEADS_NOTIFY_CC` is explicitly set in Property's Vercel env, and even then not for sources listed in `LEADS_NOTIFY_CC_EXCLUDE_SOURCES`. The rule is `Property/web/src/lib/lead-routing.ts` (`resolveLeadCc`); `GET /api/leads/notify` reports the live `ccSet` and `ccExcludeSources`. This is also why test leads must never traverse this pipeline (Phase C guardrail: trigger-skipped inserts only).
- **Consent copy is in-house, estate-wide (2026-08-17, owner instruction).** `leadConsentText` on all 18 sites, every privacy policy, and the post-submit thank-you / complete / SMS / nurture copy say only that we use the details to respond and contact the enquirer. The partner-sharing notice added 2026-08-15 (`d8b8b118`, up to six recipient firms plus a referral-fee disclosure) was reverted; `partner` is `null` in every `niche.config.json`. **If lead sharing is ever switched back on, the form notice and the privacy policy must both disclose it BEFORE the first lead is routed.** A per-site vitest guard asserts the notice contains no sharing claim.
