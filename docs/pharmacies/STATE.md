# pharmacies (Pharmacy Finance Partners) site state
...
> **Pharmacy Finance Partners** (`www.pharmacyfinancepartners.co.uk`) so the
...
> re-verify `pharmacyfinancepartners.co.uk` before buying. See
...
- site_key `pharmacies` | display **"Pharmacy Finance Partners"** | intended domain `www.pharmacyfinancepartners.co.uk`
...
- [ ] Buy the domain `pharmacyfinancepartners.co.uk` and point DNS at Vercel (A/CNAME per Vercel domain UI)
...
- [ ] Google Search Console: add property `sc-domain:pharmacyfinancepartners.co.uk`, verify (DNS TXT), submit `/sitemap.xml`, Request Indexing on key pages (discovery failure is the number 1 new-site risk)
