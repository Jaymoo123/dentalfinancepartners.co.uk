...
| Designer credit IS present in the footer, deliberately, and section C's legal row is correct | **SUPERSEDED 2026-09-11. This row previously read "No designer credit in the footer ... Trade was not designed by them, so the credit is correctly absent and section C's row is Property-specific. TAKEN 2026-09-11", and that conclusion is wrong: it was written from the phase 1 position and never re-checked against the owner reversal taken the same morning.** The owner decided at 10:42 on 2026-09-11 (`git log -1 --format='%ad%n%B' 6966c1f1`) that `showBuilderCredit` flips to TRUE on both kit-chrome consumers, because "the owner wants the Double Wired Creative credit estate-wide, not only on the site the studio designed". Trade's LOCAL footer carries its own credit block at `construction-cis/web/src/components/layout/SiteFooter.tsx:150-161`, added by `72fe3261` at 18:39 the same day (`git log -1 --format=%ad 72fe3261`), i.e. AFTER the reversal and consistent with it, not in violation of it. It renders: `curl -s http://localhost:3167/ \| grep -o 'Built by Double Wired Creative'` returns the string. It is a followed outbound link (`rel="noopener noreferrer"`, no `nofollow`), which the owner has accepted estate-wide. The mechanism note below remains accurate about the KIT and is kept. Mechanism: playbook trap T27 (`docs/_engines/DESIGN_PORT_PLAYBOOK.md:720`; the re-review looked for it in the rollout doc, which stops at 13, and wrongly reported it as nonexistent). `packages/web-shared/design/chrome/SiteFooter.tsx:132` defaults `showBuilderCredit` to TRUE, so the kit footer ships the studio's credit as a FOLLOWED outbound link; under the 10:42 decision that default is now the correct behaviour and a port that passes nothing is correct. TAKEN 2026-09-11, **REVERSED AND CORRECTED 2026-09-11** |
