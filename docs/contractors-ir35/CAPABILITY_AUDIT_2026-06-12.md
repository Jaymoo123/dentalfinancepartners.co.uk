...
| 2 | **Add CI matrix entry** — add `contractors-ir35` to the `matrix.include` list in `.github/workflows/ci-build-test.yml:32` with `url: https://www.contractortaxaccountants.co.uk`. Lift the "not yet buildable" stale comment. | `.github/workflows/ci-build-test.yml` (repo root) | MUST — PF-04 |
