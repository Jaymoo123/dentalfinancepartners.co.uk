# Wave 7 Q&A — Session B

**Created:** 2026-05-24 PM.

**§16.15 / §16.37 critical Q&A discipline:** when raising a Q to manager, append HERE via ABSOLUTE PATH from your worktree: `C:/Users/user/Documents/Accounting/docs/property/wave7_questions_session_B.md`. NEVER append to your worktree relative-path copy of this file. Manager polls main path only; questions written to the worktree relative-path copy are NOT seen by the monitor and require manual user-side propagation.
...
For everything else, use flags (`wave7_site_wide_flags.md`) or discovery log (`wave7_discovery_log_session_B.md`). Flags do not block; questions block; discovery logs are FYI.

**Watcher arming pattern (§16.41(d), Wave 7+):** the manager-side watcher counts `## Q-\d+` heading occurrences (monotonic; template-immune).
