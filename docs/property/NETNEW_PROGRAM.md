...
**Last updated:** 2026-05-23 PM — Wave 5 closed (30 pages shipped + pre-merge house-position corrections F-B7-1 + F-4 + audit trail + three bucket merges + post-merge brief corrections + C9 §16.32 hyperlinks).
...
**Wave 6 prep status:** not started. Wave 6 sequencing decisions pending:
- (a) **Deploy timing.** Two pending batches (W4 + W5 = 60 pages). Deploying both together unlocks ~6-8 weeks of GSC signal on combined W1-5 (151 pages live) before Wave 6 launches → data-led bucket rotation. Holding deploy → Wave 6 buckets selected by manager intuition (rotation discipline degrades).
- (b) **Wave 6 bucket selection.** Cleanest Wave 6+ signal logged in Wave 5 Session C summary: §24.7 adult/minor-child cohort + settlements legislation (s.624) + GROB family-home is a clean topical gap not covered by C1-C10. Other candidates: Trusts + Estates beyond §22; Capital allowances on commercial property + SBA + FYA; NRCGT operational mechanics depth; Property income tax 2027 surcharge depth §7; Wave 5 Session A discovery log D-2 "extraction sequence pillar" idea (LtdCo extraction multi-strand-sequence framing not covered by competitor sites).
- (c) **§16.36 Stage 2 brief-template statutory-citation cross-check gate** (recommended pre-Wave 6 process improvement per the 4 brief-quality catches in Wave 5 — A8 + A9 + B5 + B10). See §16.36.
- (d) **§16.37 launch-prompt Q&A path discipline hardening** (Wave 5 Session B's §16.15 repeat). See §16.37.
...
- **Deploy timing (W4 + W5 held).** Two batches pending: W4 (30 pages, held since Wave 4 close 2026-05-23 AM) and W5 (30 pages, held since Wave 5 close 2026-05-23 PM). User holds; combined or sequential deploy is user's call. Combined deploy unlocks ~6-8 weeks of GSC signal on W1-5 (151 live pages) → data-led Wave 6 rotation. Holding both → Wave 6 buckets selected by manager intuition. Path A (rotation-respecting) prefers deploy first.
- **Wave 6 bucket selection.** Strongest signal: §24.7 adult/minor-child cohort + settlements legislation (s.624) + GROB family-home — clean topical gap not covered by Wave 5 C1-C10 (logged Wave 5 Session C summary). Other candidates: Trusts + Estates beyond §22; Capital allowances on commercial property + SBA + FYA; NRCGT operational mechanics depth (§17.4 extension); Property income tax 2027 surcharge depth (§7 expansion); "extraction sequence pillar" for LtdCo extraction multi-strand framing (Wave 5 Session A discovery log D-2). Pool size: ~134 narrowed candidates + small W4 verified handful.
- **§16.36 Stage 2 brief-template statutory-citation cross-check gate** (pre-Wave-6 process improvement). 4 brief-quality errors caught at §16.35 per-write verification in Wave 5 (A8 + A9 + B5 + B10 = 13% error rate at brief level). Recommendation: add an explicit "WebFetch each statutory cite against legislation.gov.uk during brief generation" step to the Stage 2 sub-agent prompt for Wave 6. Reduces load-bearing weight on session-time correction. Manager-time investment: ~10 min adding the step to the Stage 2 prompt template.
- **§16.37 launch-prompt Q&A path discipline hardening** (pre-Wave-6 process improvement). §16.15 worktree-vs-main violation has appeared in Waves 4 + 5 (Session A in W4 propagated by continuation session; Session B in W5 propagated by manager). Recommendation: add a top-level "Q&A discipline" paragraph to Wave 6 launch prompts mandating absolute paths to main's Q&A file. Manager-time investment: ~5 min adding the paragraph to the launch prompt template.
