...
**Last updated:** 2026-05-23 PM — Wave 4 closed (30 pages shipped + F-18 §15.4 pre-merge correction + three bucket merges + F-1/F-2/F-3/F-5 post-merge back-patches landed).

**`main` is at `f11d801`.** 121 net-new pages from Wave 1 (31) + Wave 2 (30) + Wave 3 (30) + Wave 4 (30) sit on `main`. Wave 3 commit chain `727eebc` (audit-trail) / `d98c53d` (A) / `cf4a7fa` (B) / `f546964` (C) / `2b18321` (factual back-patches) / `6edc5d8` (cross-link back-patches). Post-Wave-3 hygiene `38b17b5` (F-1 RRA-2026 site-wide) + `5a49767` (§20 verification F-12 to F-17) + `00fc6f3` (NETNEW_PROGRAM update). Wave 4 prep `f9c838b` (Stage 1) + `f9778a4` (Stage 2) + `1dabfa6` (launch prompts). Wave 4 close `1e796df` (F-18 §15.4) / `e9b95fc` (audit-trail) / `c964ea4` (A merge) / `e461bb5` (B merge) / `1a4c211` (C merge) / `f11d801` (post-merge back-patches).
...
**Waves 1 + 2 + 3 are deployed; Wave 4 is held.** Wave 4 deploy deferred by user 2026-05-23 (to be deployed later as a separate batch). GSC signal for the 91 W1-3 pages is therefore accruing; Wave 6+ bucket rotation can be data-led on W1-3 once 6-8 weeks have passed since their deploy dates. Wave 4 GSC signal blocked on a later user-triggered deploy.

**Worktrees that can now be deleted:** `Accounting-wt-property-wave4-{a,b,c}/`. (Previous Wave 1-3 worktrees already deleted in the post-Wave-3 hygiene batch.) **Medical worktrees** parked, not part of this program.
...
- **AUTHORITY_GAP from Wave 2 Stage 2:** no Form 17 dedicated page on-site. Future-wave candidate.
...
- **VAT cluster re-clustering** with property-specific token filter before any VAT bucket is Wave-5-viable (Wave 4 discovery surfaced 296 raw candidates but mostly noise: Uber court cases, NHS locum doctors, childcare, AI/tech, Amazon; needs filter).
- **Weekly `serp_runner.py` re-run** to widen the 83-query single-day SERP corpus (Wave 4 discovery snapshot).
- **CGT deepen-existing sweep** (Wave 4 delta surfaced 150 partial > 62 net-new = legacy-rebuild track signal, not net-new program scope).
...
**Program totals (Wave 4 closed):** ~485 total target (~285 net-new + ~231 legacy rebuilds). After Wave 1 (31) + Wave 2 (30) + Wave 3 (30) + Wave 4 (30) = **121 net-new on `main`** (W1-3 deployed; W4 held). ~164 candidates remain in the user-narrowed pool. Wave 4 discovery (`docs/property/topic_gaps_delta_2026-05-23.md`) surfaced ~1,136 raw "net-new" candidates + 817 partial-overlap across 25 named clusters in the 243-domain v2 competitor universe; **user correction 2026-05-23: ~90% of the 1,136 carry meaningful cannibal risk against the 121 already-shipped + 285 legacy inventory, so the true net-new lift on top of the 164-pool is only a handful, not 1,136.** Subsequent waves mine the 164 narrowed pool + the small Wave 4 verified net-new lift; see §16.34 for the lesson and the verification-cannibalisation-check rule.

**Wave 5 prep status:** not started. Wave 5 sequencing decision-required: (a) which buckets per the rotation strategy (proposed default: VAT-re-clustered + CGT-deepen-existing + new theme, OR extend the best-performing Wave 4 bucket once GSC signal arrives); (b) whether to deploy Waves 1-4 first to gather ~6-8 weeks of GSC signal before launching Wave 5. Inter-wave queue items to clear before Wave 5: VAT re-clustering with property-specific filter, weekly SERP re-run, F-9 calendar-quarter verification, F-1 PART 2 lead-page rewrite (if user wants it sequenced into the program rather than parked).
