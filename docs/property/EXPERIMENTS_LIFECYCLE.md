...
All six historical Property experiments are concluded and locked to their winning treatment. `personalization` is locked ON for every visitor; `exit_intent_offer` is locked to the topic-aware offer; `calc_result_capture` / `gate_to_form` / `mobile_tool_capture` / `lead_form_length` were already shipped-as-default and have had their residual scaffolding removed. Zero experiments are running. The next test (`calc_cta_placement`, calculator→lead handoff) is the first of the new lead-capture program.
