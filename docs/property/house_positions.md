...
- **s.22(1)(c)**: capital sums for forfeiture/surrender of rights are a disposal timed at receipt (forfeited buyer's deposit; a retained modern-method reservation fee needs this analysis).
- Do not write: "you can deduct removal costs against the gain"; "the 60-day clock runs from exchange"; "auction sales are taxed at completion".
...
- **Para 2 (property trader, "assisted move")** and **para 3 (chain-break)**: parallel reliefs with refurbishment-cap, no-lease/licence and no-principal/employee-occupation conditions.
...
- Permitted-area collision: TCGA 1992 s.222(2) PRR permitted area = 0.5 hectare (s.222(3) extension for reasonable enjoyment); Sch 6A carries its own permitted-area condition. **Stage 2 must confirm whether the two tests are drafted identically before any page asserts they are.**
...
- **DIY-conveyancing boundary (A3/A4):** LSA 2007 s.12(1)(c) + Sch 2 para 5(1)(a) (preparing transfer instruments = reserved), s.13 (authorised/exempt only), **Sch 3 para 3(10)** (individual acting otherwise than for fee/gain/reward = exempt). CAUTION: Sch 3 has no express self-representation carve-out; do NOT inflate para 3(10) into "anyone may convey for anyone unpaid" without checking HMLR practice guidance.
- **Distance/off-premises contracts (A5):** SI 2013/3134 reg 30 (14-day cancellation for service contracts), reg 31 (extension for information breach), reg 36 (supply in cancellation period; reg 36(4) proportionate charge).
...
- **PRs' cost of establishing title:** allowable per the agreed scale in **HMRC SP2/04** (deaths after 5 Apr 2004), referenced via **CG30560**. **FIGURES NOT LOCKED: CG30560 does not reproduce the scale and the SP page 404'd at seed time. Lock is existence + manual reference only; do not publish scale figures until the current published scale is located.**
- **PR AEA fork (selling decision made before listing):** PRs get the estate AEA (year of death + two following years, per §39) at the PR rate; assenting to beneficiaries first gives each beneficiary their own AEA and possibly the 18% band. **Statutory basis for the PR AEA not yet pinned; cite on §39/HMRC-manual authority and say so until a section is located.**
