...
**DEPLOYED to production 2026-05-29** via Vercel CLI (env-var override pattern from repo root per `memory/vercel_cli_deploy_workflow.md`). Consolidated 120-page push (60 MW2 + 60 MW3) to www.propertytaxpartners.co.uk. Production URL: https://property-tax-partners-qw55fzk4u-sitenudge-projects.vercel.app. Build wallclock ~2 min on Vercel infra (Washington DC East, 4 cores / 8 GB). All 732 routes prerendered + middleware + serverless functions deployed cleanly. Aliased domain serving sample 5/5 with HTTP 200. IndexNow batch submitted: 120 URLs to Bing/Yandex/Seznam/Naver/Yep (HTTP 200, full submission complete).

**Deploy blocker hit + worked around (NEW PLATFORM-LIMIT EVENT):** First two deploy attempts failed with Vercel "Oversized Incremental Static Regeneration (ISR) page: blog.fallback (20.71 MB)" — Vercel rejects pre-rendered responses larger than 19.07 MB. Root cause: total blog markdown corpus crossed the threshold (18.47 MB of post content × ~12% bundle overhead = ~20.7 MB fallback artifact) for the first time after MW2 + MW3 push. Two-part fix landed mid-session: (a) `Property: dynamicParams=false on [category]/[slug] to fix deploy` (`c05f38f0`) — runtime safety, unknown params 404 at routing layer instead of triggering fallback; (b) `VERCEL_BYPASS_FALLBACK_OVERSIZED_ERROR=1` added to Property project env (API call, ID `tRI0ys85IkjmYWKq`, target=prod/preview/dev) — Vercel's official build-time bypass. With (a) in place, runtime `FALLBACK_BODY_TOO_LARGE` cannot fire. Third deploy attempt SUCCEEDED. **Behavioural side-effect to flag:** the page's runtime `permanentRedirect` for "correct slug under wrong category prefix" requests will no longer fire — those now 404 instead. Middleware already covers 50+ legacy slug redirects directly; any residual wrong-category 404s can be back-filled into middleware later. **Structural follow-up (separate scope, not this session):** the blog corpus will continue growing ~1.5 MB per wave. By MW6 the fallback artifact will be ~30 MB. Proper fix is either lazy per-request post loading in `lib/blog.ts` (avoid bundling all posts into the page module) or splitting blog routes into smaller dynamic segments. Track as inter-wave item.

**420 net-new pages now LIVE in production** (W1-7 209 + W8 30 + W9 9 + MW1 52 + MW2 60 + MW3 60 — minor 1-page reconciliation from earlier 421-on-main tally per MW1 B12 carry-over). Previous prod state was 300; this consolidated push added 120 = 420 live. **First fully-deployed state since 2026-05-26.** GSC signal for the 120 newly-live MW2 + MW3 pages will start accruing this week; 6-8 weeks of dwell time before bucket-rotation decisions can be data-led.
