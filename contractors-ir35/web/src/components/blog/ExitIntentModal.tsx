 ...
 *
 * cfp_ storage keys, petrol-cyan palette.
 * WS6 stand-down guard: bails when cfp_assistant_active is "1" so the
 * assistant and the exit modal never fire together.
 */
...
const STORAGE_KEY = "cfp-exit-intent-dismissed-v1";
...
export function ExitIntentModal() {
  ...
  useEffect(() => {
    ...
    const fire = () => {
      ...
      if (window.sessionStorage.getItem("cfp_assistant_active") === "1") return;
      try {
  ...
  const heading = topic?.ctaCopy || "Get a free review of your contractor tax position";
  const blurb =
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-lg border-l-4 border-cyan-700 bg-white p-6 shadow-2xl sm:p-8">
        <button
          type="button"
          onClick={() => setOpen(false)}
          className="absolute right-3 top-3 flex h-10 w-10 items-center justify-center rounded-lg text-neutral-400 hover:text-neutral-700 focus:outline-none focus:ring-2 focus:ring-cyan-700"
          aria-label="Close"
        >
        ...
        <p className="text-xs font-bold uppercase tracking-wider text-cyan-700">Before you go</p>
