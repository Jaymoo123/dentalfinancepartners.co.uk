...
/**
 * Email-gated download unlock for Contractor Tax Accountants (cfp).
 *
 ...
 * names a partner firm). Resource downloads are NOT shared with any partner.
 *
 * Submit: goes through submitContractorLead with captureMode "email_only" AS A FIELD
 * and extras { resource_gate: true } so the server can skip notify/enrich on these rows.
 *
 * Honeypot: enquiry_ref. Value is passed through to submitContractorLead; do NOT
 * early-return on a filled honeypot (F4 fix). The server stores the row flagged and
 * returns success so a real human caught by browser autofill is never silently dropped.
 *
 * Belt-and-braces: if neither the xlsx nor the guide is enabled for this topic,
 * renders nothing (hasEnabledResource guard).
 *
 * Storage prefix: cfp (FROZEN). Events: gate_view, resource_unlocked (allowlist only).
 */
...
// cfp token-safe input class (no --primary, no --gold, no --navy)
const inputClass =
...
export function ResourceGate({
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Pass through to submitContractorLead so the server stores the row flagged.
    ...
    const result = await submitContractorLead(
      {
        full_name: "",
        email,
        phone: "",
  ...
  return (
    <section
      ref={rootRef}
      className="not-prose @container my-12 overflow-hidden rounded-2xl border border-[var(--border)] bg-white shadow-[0_1px_2px_rgba(14,116,144,0.04),0_12px_28px_-16px_rgba(14,116,144,0.18)] ring-1 ring-[var(--accent)]/[0.03]"
      aria-labelledby="resource-gate-heading"
    >
      {/* Branded top accent: petrol-cyan */}
      ...
      <div className={["grid", split ? "@3xl:grid-cols-[1.1fr_0.9fr]" : ""].join(" ").trim()}>
        ...
        >
          {status === "success" ? (
            <div
              role="status"
              className="rounded-xl border border-[var(--accent-whisper)] bg-[var(--surface-elevated)] p-5"
            >
              ...
              <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                ...
                {guideReady && resource?.guide && (
                  <a
                    ...
                    className="inline-flex items-center justify-center rounded-full border-2 border-[var(--accent)] px-5 py-3 text-base font-semibold text-[var(--accent)] transition-colors hover:bg-[var(--accent-whisper)]"
                  >
