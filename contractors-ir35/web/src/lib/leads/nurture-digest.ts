/**
 * Daily digest composer and runner for the Contractor Tax Accountants
 * lead-nurture system.
 *
 * gatherDigestData: reads vw_lead_nurture_health, vw_lead_nurture_stuck, and
 * last-7d send_failed events keyed on site='contractors-ir35'.
 *
...
export async function gatherDigestData(siteKey: string): Promise<DigestData> {
  ...
  // Enrich failed sends with lead names. Filter by source='contractors-ir35'
  // to prevent cross-site bleed.
  ...
  if (failedEvents.length > 0) {
    ...
    const nameMap = new Map<string, string | null>(
      // ponytail: JS-filter by source='contractors-ir35' to prevent cross-site bleed
      (nameRes.data ?? [])
        .filter((r) => r.source === "contractors-ir35")
...
export function composeDigestEmail(d: DigestData): {
  ...
  lines.push(`LEAD NURTURE DAILY DIGEST (contractors-ir35)`);
  ...
  lines.push(`Automated digest from the Contractor Tax Accountants lead-nurture system.`);
