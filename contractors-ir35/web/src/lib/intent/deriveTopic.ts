import { topicForBlogSlug, topicForCalcSlug, topicForForSlug, type TopicKey } from "./taxonomy";
 ...
 *   /for/<type>              -> contractor type slug (mapped via FOR_SLUG_TO_TOPIC)
...
export function deriveTopic(pathname: string): TopicKey | null {
  ...
  // Contractor type pages: derive topic from the contractor type slug.
  if (parts[0] === "for" && parts[1]) return topicForForSlug(parts[1]);
  return null;
}
