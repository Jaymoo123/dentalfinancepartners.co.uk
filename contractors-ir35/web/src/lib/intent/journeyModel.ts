/**
 * Journey model for Contractor Tax Accountants (contractors-ir35) - deterministic,
 * client-side, instant. Ported from Property/generalist/Dentists with the `cfp`
 * storage prefix.
 *
 ...
 *
 * Storage key: `cfp_journey` (site prefix `cfp`, never `dfp_`/`ptp_`/`hd_`).
 * Special-page flags: /about, /services, /contact - all three routes exist on
 * contractors-ir35 (confirmed), so all flags are kept.
 */
