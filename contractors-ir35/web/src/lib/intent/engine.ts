...
function specialistOffer(topicKey: TopicKey): IntentOffer {
  const t = getTopic(topicKey);
  const label = (t?.label ?? "contractor tax").toLowerCase();
  return {
    kind: "specialist",
    title: "Speak to a contractor tax specialist",
    blurb: `Get your specific ${label} position checked by a specialist contractor accountant.`,
    href: "/contact",
  ...
  switch (surface) {
    ...
    case "returning_bar": {
      ...
      const offer =
        reviewOffer(
          resume,
          "Pick up where you left off, get your contractor tax position reviewed",
        ) ??
        toolOffer(resume) ??
