/**
 * Contractor Tax Accountants lead-nurture composition: the contactability sequence
 * and detail-capture sequence for IR35 and contractor tax enquiries.
 *
 ...
 *
 * Audience: PSC contractors, umbrella workers, inside/outside IR35 enquirers,
 * day-rate workers, limited company directors, MSC-risk cases.
 * Themes: IR35 status, umbrella vs limited, day-rate/take-home, MSC risk,
 * limited-company tax, contractor pensions.
 *
 * House style: no em-dashes. British English. Faceless team voice.
 * No named individual, no credential claims, no cross-vertical bleed.
 *
...
const SEQUENCE_NAME = "contractors_ir35_contactability";
const DETAIL_CAPTURE_SEQUENCE_NAME = "contractors_ir35_detail_capture";
...
// ponytail: CALC_MAP seeds are ready to fill; ir35 has several relevant calcs.
// Wire them here when calculator-specific follow-up copy is written.
const CALC_MAP: Record<string, { name: string; path: string }> = {
  // umbrella_vs_limited: { name: "Umbrella vs Limited Calculator", path: "/calculators/umbrella-vs-limited" },
  // ir35_take_home: { name: "IR35 Take-Home Calculator", path: "/calculators/ir35-take-home" },
};
...
// Audience: PSC contractors, umbrella workers, IR35-status and take-home enquirers.
...
const STEPS: LeadNurtureStep[] = [
  ...
  // ── Step 1: T0 SMS + WhatsApp ──────────────────────────────────────────────
  {
    ...
    buildMessages: (c) => [
      smsMsgWithGen(
        c,
        "t0_sms",
        `Hi ${c.firstName}, this is Contractor Tax Accountants. Thanks for your enquiry. Reply YES and a member of the team will call you. ${c.optOutText}`,
      ),
  ...
  // ── Step 3: Day 1 SMS + WhatsApp ──────────────────────────────────────────
  {
    ...
    buildMessages: (c) => [
      smsMsgWithGen(
        c,
        "day1_sms",
        `Hi ${c.firstName}, following up on your contractor tax enquiry. We have kept some time free this week for a short call. Reply YES and we will call you. ${c.optOutText}`,
      ),
  ...
  // ── Step 4: Day 2 give-value email (theme: umbrella vs limited) ───────────
  {
    ...
    buildMessages: (c) => [
      emailMsg(
        c,
        `One thing contractors often overlook, ${c.firstName}`,
  ...
  // ── Step 5: Day 4 SMS + WhatsApp ──────────────────────────────────────────
  {
    ...
    buildMessages: (c) => [
      smsMsgWithGen(
        c,
        "day4_sms",
        `Hi ${c.firstName}, Contractor Tax Accountants here. Most contractors we speak to came with a question just like yours, and one short call usually clears up months of uncertainty. Reply YES and we will call you. ${c.optOutText}`,
      ),
      whatsappTemplate("lead_reminder", [c.firstName, c.bookingUrl]),
    ],
  },
    ...
    buildMessages: (c) => [
      emailMsg(
        ...
        [
          ...
          "For what it is worth, the moments a contractor tax review tends to matter most are a new contract, a change in IR35 status, an umbrella-to-limited switch, a Self Assessment deadline approaching, or a tax bill you want a second opinion on. If one of those lands, just reply to this email, even months from now, and the team will pick it straight back up.",
          "All the best with the contracting.",
        ],
        "breakup_day11",
        { cta: null, secondary: null },
      ),
    ...
    buildMessages: (c) => {
      ...
      return [
        emailMsg(
          ...
          [
            detailIntro(c),
            `Just reply to this email with ${ask} and a member of the team will call you. It is free, there is no obligation, and there is nothing to prepare beforehand.`,
          ...
          "detail_capture_t0",
          {
            cta: null,
            secondary: null,
            ...detailGreeting(c, "Hi, thanks for getting in touch with Contractor Tax Accountants."),
          },
        ),
      ];
    },
      ...
      return [
        emailMsg(
          ...
          "detail_capture_day1",
          {
            cta: null,
            secondary: null,
            ...detailGreeting(c, "Hi, following up from Contractor Tax Accountants."),
          },
        ),
      ];
    },
      ...
      return [
        emailMsg(
          ...
          "detail_capture_day3",
          {
            cta: null,
            secondary: null,
            ...detailGreeting(c, "Hi, from the team at Contractor Tax Accountants."),
          },
        ),
      ];
    },
      ...
      return [
        emailMsg(
          ...
          "detail_capture_day7",
          {
            cta: null,
            secondary: null,
            ...detailGreeting(c, "Hi, one last message from Contractor Tax Accountants."),
          },
        ),
      ];
    },
...
export function buildIr35LeadNurtureConfig(
...
export function buildIr35LeadNurtureConfigs(): LeadNurtureConfig[] {
...
//   "I agree to be contacted about my enquiry. I understand that Contractor Tax Accountants " +
