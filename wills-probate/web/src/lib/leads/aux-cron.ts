...
export async function runLeadAuxScans(): Promise<{ reminders: number; nudges: number }> {
  ...
  try {
        ...
        if (inT24) {
          ...
          if (sendRowId) {
            try {
              ...
              await markSendStatus(sendRowId, "failed");
              console.error("[lead-aux-cron/wp] T-24 email error", err);
            }
          }
        }
          ...
          if (inT2 && inSendWindow(now, true)) {
            ...
            if (sendRowId && lead.phone) {
              try {
                ...
                await markSendStatus(sendRowId, "failed");
                console.error("[lead-aux-cron/wp] T-2 SMS error", err);
              }
        ...
        if (eligibleIds.length > 0 && inSendWindow(now, true)) {
          ...
          for (const lead of nudgeLeadsRes.data) {
            ...
            try {
              ...
              await markSendStatus(sendRowId, "failed");
              console.error("[lead-aux-cron/wp] abandoned nudge error", err);
            }
          }
        }
      }
    ...
    console.error("[lead-aux-cron/wp] unexpected top-level error", err);
  }
