/**
 * Daily digest composer and runner for the Probate Compass lead-nurture
 * system.
 *
 * gatherDigestData: reads vw_lead_nurture_health, vw_lead_nurture_stuck, and
 * last-7d send_failed events (filtered to source='wills-probate').
 *
...
export async function gatherDigestData(siteKey: string): Promise<DigestData> {
  ...
  if (failedEvents.length > 0) {
    ...
    const nameMap = new Map<string, string | null>(
      // ponytail: JS-filter by source='wills-probate' to prevent cross-site bleed
      (nameRes.data ?? [])
        .filter((r) => r.source === "wills-probate")
...
export function composeDigestEmail(d: DigestData): {
  ...
  lines.push(`Automated digest from the Probate Compass lead-nurture system.`);
...
export async function runNurtureDigest(): Promise<{ sent: boolean }> {
  const d = await gatherDigestData("wills-probate");
  ...
  try {
    await getResend().emails.send({
      from: getFromAddress(),
      to: resolveLeadTo("wills-probate"),
      subject,
      text,
    });
    return { sent: true };
  } catch (err) {
    console.error("[nurture-digest/wp] digest email send failed", err);
    return { sent: false };
  }
}
