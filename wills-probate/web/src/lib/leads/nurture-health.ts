 ...
 *
 * getNurtureHealth: reads vw_lead_nurture_health for site_key='wills-probate'.
...
export async function runNurtureGuardrails(opts: {
  ...
  const health = await getNurtureHealth("wills-probate");
  if (!health) {
  ...
  if (!alreadyAlerted && process.env.RESEND_API_KEY) {
    ...
    try {
      ...
      console.error("[nurture-health/wp] guardrail alert send failed", err);
    }
  }
  ...
  if (opts.autopauseEnabled && verdict.shouldPause && !control.paused) {
    try {
      ...
      console.error("[nurture-health/wp] auto-pause failed", err);
    }
  }
