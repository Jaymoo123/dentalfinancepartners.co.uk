/**
 * Probate Compass experiment assignment -- re-export shim.
 *
 * Preserves the original `assignVariant(visitorId, key)` signature that
 * site components rely on (key is a string, registry lookup is internal).
 * Delegates to the shared pure `assignVariant` + the site registry.
 *
...
const registry = siteRegistries["wills-probate"];
