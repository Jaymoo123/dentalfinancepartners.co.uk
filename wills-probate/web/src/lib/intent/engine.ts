...
function reviewOffer(
  ...
  return {
    kind: "specialist",
    title: t.ctaCopy,
    blurb: "A free, no-obligation review of this estate's position with a probate specialist.",
    href: "/contact",
    reason,
  };
}
...
function specialistOffer(topicKey: TopicKey): IntentOffer {
  const t = getTopic(topicKey);
  const label = (t?.label ?? "probate").toLowerCase();
  return {
    kind: "specialist",
    title: `Speak to a probate specialist`,
    blurb: `Get this estate's specific ${label} position checked by a probate specialist.`,
    href: "/contact",
  ...
  switch (surface) {
    ...
    case "returning_bar": {
      ...
      const offer =
        reviewOffer(
          resume,
          "Pick up where you left off, get this estate's position reviewed",
        ) ??
        toolOffer(resume) ??
