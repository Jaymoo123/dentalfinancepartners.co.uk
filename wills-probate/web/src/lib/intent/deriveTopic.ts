 ...
 *   /for/<audience>          -> probate-cost (generic entry, best match)
...
export function deriveTopic(pathname: string): TopicKey | null {
  ...
  // Audience pages: someone landing on /for/<audience> has clear probate-cost intent.
  if (parts[0] === "for" && parts[1]) return "probate-cost";
  return null;
}
