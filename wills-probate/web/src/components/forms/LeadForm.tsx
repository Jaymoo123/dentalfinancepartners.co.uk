...
/** Returns true when the role value signals an executor or administrator of an estate. */
function isExecutorOrAdmin(role: string): boolean {
  return role.toLowerCase().includes("executor-administrator");
}

/** Returns true when the role value signals inheritance tax planning. */
function isIhtPlanning(role: string): boolean {
  return role.toLowerCase().includes("iht-planning");
}

const ESTATE_VALUE_OPTIONS = [
  "Under £325,000",
  "£325,000 to £650,000",
...
export function LeadForm({
  ...
  const [selectedEstateStage, setSelectedEstateStage] = useState("");
  const [selectedEstateValue, setSelectedEstateValue] = useState("");
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const completedCount =
      ...
      + (consent ? 1 : 0)
      + (selectedEstateStage ? 1 : 0)
      + (selectedEstateValue ? 1 : 0);
    trackFormSubmit(completedCount);

    // Qualifiers go into extras (not crammed into message) — schema-clean.
    const extras: Record<string, unknown> = {};
    if (selectedEstateStage) extras.estate_stage = selectedEstateStage;
    if (selectedEstateValue) extras.estate_value = selectedEstateValue;
    ...
    setConsent(false);
    setSelectedRole("");
    setSelectedEstateStage("");
    setSelectedEstateValue("");
    setHoneypotValue("");
    setSituation("");
    setPrompted("");
    setCallGoal("");
  ...
  const showEstateStageSelect = isExecutorOrAdmin(selectedRole);
  const showEstateValueSelect = isIhtPlanning(selectedRole);
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6" noValidate aria-busy={status === "loading"}>
      ...
      {/* Segment field: probate stage — shown for executors and administrators */}
      {showEstateStageSelect && (
        <div>
          <label htmlFor="estateStage" className={labelClass}>
            Have you applied for the grant of probate yet?{" "}
          ...
          <select
            ...
            autoComplete="off"
            className={fieldClass}
            value={selectedEstateStage}
            onChange={(e) => setSelectedEstateStage(e.target.value)}
            onFocus={() => onFieldFocus("estateStage")}
            onBlur={(e) => onFieldBlur("estateStage", !!e.target.value)}
          >
            <option value="">Select...</option>
      ...
      {/* Segment field: estate value — shown for inheritance tax planning enquiries */}
      {showEstateValueSelect && (
        <div>
          <label htmlFor="estateValue" className={labelClass}>
          ...
          <select
            ...
            autoComplete="off"
            className={fieldClass}
            value={selectedEstateValue}
            onChange={(e) => setSelectedEstateValue(e.target.value)}
            onFocus={() => onFieldFocus("estateValue")}
            onBlur={(e) => onFieldBlur("estateValue", !!e.target.value)}
          >
            <option value="">Select...</option>
            {ESTATE_VALUE_OPTIONS.map((opt) => (
        ...
        <div className="space-y-4 px-4 pb-4 pt-2">
          <div>
            ...
            <textarea
              ...
              rows={3}
              maxLength={600}
              placeholder="e.g. I am the executor for my late mother's estate, worth around £400,000, and have not yet applied for probate."
              className={fieldClass}
              value={situation}
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              maxLength={600}
              placeholder="e.g. A relative passed away recently and I need to understand what probate involves."
              className={fieldClass}
              value={prompted}
