...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error("[leads/complete/probate] lead lookup failed", e);
  ...
  if (patchKeys.length > 0) {
    try {
      ...
      console.error("[leads/complete/probate] lead update failed", e);
      return NextResponse.json({ success: false, error: "server" }, { status: 500 });
    }
  }
  ...
  try {
    ...
    console.error("[leads/complete/probate] audit event failed", e);
  }
  ...
  try {
    ...
    console.error("[leads/complete/probate] confirmed event failed", e);
  }
  try {
    await adminUpdate(
      "lead_nurture_state",
    ...
    console.error("[leads/complete/probate] nurture state promote failed (non-fatal)", e);
  }
  try {
    await adminUpdate(
      "leads",
    ...
    console.error("[leads/complete/probate] lead promote failed (non-fatal)", e);
  }
