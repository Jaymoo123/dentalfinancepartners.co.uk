/**
 * Lead-nurture cron for Probate Compass: fires due follow-up steps for
...
async function run(req: NextRequest): Promise<NextResponse> {
  ...
  if (effectiveArmed) {
    try {
      ...
      console.error("[lead-nurture-cron/probate] aux-cron failed (non-fatal)", err);
    }
  }
  ...
  if (cronArmed) {
    try {
      ...
      console.error("[lead-nurture-cron/probate] guardrail scan failed", err);
    }
  }
