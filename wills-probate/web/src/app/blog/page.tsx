...
export const metadata: Metadata = {
  title: "Probate and Inheritance Tax Blog | Guides and Articles",
  description:
    "Practical probate and inheritance tax guides. Grant of probate, estate valuation, nil-rate band, intestacy, executor duties and lasting power of attorney. Written by specialist probate advisers.",
            ...
            <div className="grid gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {categories.map((cat) => (
                <Link
                  key={cat.slug}
                  href={`/blog/${cat.slug}`}
                  className="block min-h-[100px] p-5 border border-neutral-200 bg-white transition-all hover:border-orange-500 hover:shadow-md"
                >
                  <h3 className="text-base font-bold text-neutral-900">{cat.name}</h3>
                  <p className="mt-1 text-sm text-neutral-500">
                    {cat.count} article{cat.count !== 1 ? "s" : ""}
                  </p>
                  <span className="mt-2 inline-flex items-center text-sm font-medium text-orange-700">
                    View articles
