...
/**
 * The "what the advice covers" grid. A card per subject: brand icon badge,
 ...
 * and without it the brand rules land at six different heights across the
...
export function CoverageCards({
   ...
   * animate `border-color` and finish on a brand drop shadow, so a card whose
   * rest state is a slate ring and no shadow would animate a border it does not
   * have and then snap out of a shadow it never had. The glow variant carries
   * the same border and resting shadow as the /services tiles, which is what
   * the animation resolves to.
   *
  ...
  const surface = glow
    ? `border border-primary-100 shadow-[0_6px_20px_-8px_rgba(5,150,105,0.28)] hover:shadow-[0_16px_32px_-12px_rgba(5,150,105,0.4)] ${
