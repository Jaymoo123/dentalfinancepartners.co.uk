...
/**
 * Numbered argument list whose numerals light up in brand colour the first
 * time the section scrolls into view, one after another.
 *
 ...
 *
 * The numerals settle into brand colour and STAY there. Everything on this
 * site draws once and stops; a numeral that pulsed would compete with the
 * copy beside it, and the point of the mark is the arrival, not the loop.
 *
    ...
    >
      {items.map((item, i) => (
        <div key={item.title}>
          ...
          <span
            aria-hidden
            className="story-numeral-rule mt-3 block h-px w-10 bg-primary-600"
            style={{ transitionDelay: `${i * STAGGER_MS}ms` }}
          />
