 ...
 *
 * Phase B note: getMaxScrollPct() and getEngagedMs() are exported for the
 * intent engine to read live scroll/engagement state without coupling intent
 * imports into this module (dependency direction: intent reads SDK, never reverse).
 */
...
/** Live read of the current page's max scroll depth (%), for the Phase B intent engine. */
export function getMaxScrollPct(): number {
  return maxScrollPct;
}

/** Live read of cumulative engaged time (ms) this session, for the Phase B intent engine. */
