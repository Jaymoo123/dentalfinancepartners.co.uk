 ...
 *
 * Layered defence: this is the heuristic layer that runs at ingest. Events are
 * TAGGED is_bot (not dropped) so they can be audited, and the human-only rollup
 * views exclude them. Vercel BotID can be slotted in at the marked integration
 * point, and a Python reclassifier is the daily backstop.
 *
 * Never stores raw IP — callers pass it only for heuristics/rate-limiting.
 *
 * ED-04: BotID decision comments preserved verbatim from the Property reference.
 */
 ...
 * client challenge, so this runs in basic (signal-only) mode. Deep mode would
 * require routing the action through a BotIdClient-instrumented fetch (the lead
 * form is the candidate, deferred). The Python reclassifier remains the daily
 * backstop regardless.
 * -------------------------------------------------------------------------- */
...
/**
 * Platform-grade verification via Vercel BotID. Fail-open: any error returns
 * null so the caller falls back to the heuristic.
 *
 * ⚠️ DO NOT call this on /api/track or any navigator.sendBeacon endpoint.
 * sendBeacon cannot carry BotID's client challenge — it false-flagged 100% of
 * real visitors as bots when wired into ingest on 2026-06-08.
 */
