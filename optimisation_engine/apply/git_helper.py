...
def stage_and_commit(*, files: Iterable[str], message: str, allow_empty: bool = False) -> str:
    ...
    for f in files:
        p = ROOT / f
        if not p.exists():
            raise GitError(f"file does not exist: {f}")

    # Stage
    rc, out, err = _run(["git", "add", "--"] + files)
    if rc != 0:
        raise GitError(f"git add failed: {err}")

    # Verify something was staged (unless allow_empty)
    if not allow_empty:
        rc, out, err = _run(["git", "diff", "--cached", "--quiet"])
        if rc == 0:
            # Nothing staged after `git add`; bail
            raise GitError("nothing changed for these files (already at HEAD state)")
    ...
    if rc != 0:
        ...
        raise GitError(f"git commit failed: {err or out}")

    # Capture the new SHA
    rc, sha, err = _run(["git", "rev-parse", "HEAD"])
    if rc != 0:
        raise GitError(f"git rev-parse failed: {err}")
    return sha.strip()
