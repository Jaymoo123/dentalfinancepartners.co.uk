...
def build_brief(opportunity: dict) -> ChangeBrief:
    ...
    brief.change_diff = {
        ...
        "target_word_count": target_words,
        "_content_pending": "(generated by DeepSeek at apply time)",
    }
...
def apply(brief: ChangeBrief) -> dict:
    ...
    primary_h1 = np_patch.get("primary_h1") or ""
    section_outline = np_patch.get("section_outline") or []
    schema_to_include = np_patch.get("schema_to_include") or []
    target_words = int(np_patch.get("target_word_count") or 1500)
    page_type = np_patch.get("page_type") or "blog_post"
    ...
    if not gen.auto_applicable:
        raise ApplyError(f"new_page content generation failed validators: {gen.notes}")

    content = gen.output
    body_html = content.get("body_html") or ""
    word_count = estimate_word_count(body_html)
    if word_count < target_words * 0.5:
        raise ApplyError(f"generated body too short: {word_count} words vs target {target_words}")
