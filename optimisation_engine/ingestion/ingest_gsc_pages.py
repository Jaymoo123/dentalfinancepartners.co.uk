"""
Ingest GSC page+date data for all (or one) active site.

Uses dimensions=['page', 'date'] — unthresholded, accurate totals.
Idempotent: ON CONFLICT (niche, page_url, date) DO UPDATE SET.
"""
...
from optimisation_engine.clients.gsc_page_client import GSCPageFetcher  # noqa: E402
...
def run(site_keys: list[str] | None = None, days: int = 28) -> dict[str, int]:
    ...
    for site_key in site_keys:
        print(f"\n=== GSC page ingestion: {site_key} ===")
        try:
            fetcher = GSCPageFetcher(site_key)
