...
4. **Claims arrive.** When a firm replies wanting a lead, in arrival order:
   ...
   ```
   The cap is enforced automatically; a full lead rejects the claim. Watch for the mandate warning: a firm without an active Direct Debit mandate should not receive its first lead until the mandate is active.

5. **Decay check.** Once a day, last thing:
   ```
   python lead_engine/scripts/decay.py
   ```
   Leads unclaimed past 24h re-ping at the last-call price; past 48h they cascade to the adjacent lane (verified) or the raw batch (unverified). Follow up any `cascaded` lead with the adjacent-professions firms.
...
2. Eyeball each invoice HTML before anything ever goes out. Credits need a reason that matches the standard terms in `config/standard_terms.md`.

## When something changes

- Prices, decay timing or the claim cap: edit `config/tiers.json` (owner sign-off first), then re-run `python lead_engine/scripts/build_price_sheet.py` so the price sheet stays in step. Scripts read the config at runtime, nothing else to update.
