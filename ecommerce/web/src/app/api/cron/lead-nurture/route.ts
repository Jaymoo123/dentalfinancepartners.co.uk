/**
 * Ecommerce lead-nurture cron: fires due follow-up steps for leads that have
...
import { buildEcommerceLeadNurtureConfigs, buildLeadMessageContext } from "@/config/lead-nurture";
...
async function run(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.error("[lead-nurture-cron/ecommerce] heartbeat write failed", err);
  }

  const configs = buildEcommerceLeadNurtureConfigs();
  ...
  if (cronArmed) {
    try {
      ...
      console.error("[lead-nurture-cron/ecommerce] guardrail scan failed", err);
    }
  }
