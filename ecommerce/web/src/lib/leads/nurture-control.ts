/**
 * Kill-switch control library for the Ecommerce lead-nurture system.
 *
 * Reads and writes the lead_nurture_control table keyed on site_key='ecommerce'
...
/**
 * Read the ecommerce control row (site_key='ecommerce'). FAIL-OPEN: returns the
...
export async function getNurtureControl(): Promise<NurtureControl> {
  try {
    const res = await adminSelect<DbRow>("lead_nurture_control", {
      ...
      site_key: "eq.ecommerce",
      limit: "1",
    });
    if (!res.ok || res.data.length === 0) return DEFAULT_CONTROL;
    return toControl(res.data[0]);
  } catch (err) {
    console.error("[nurture-control/ecommerce] getNurtureControl error, failing open", err);
...
export async function pauseNurture(reason: string, by: string): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("[nurture-control/ecommerce] pauseNurture write failed", res.status);
    }
  } catch (err) {
    console.error("[nurture-control/ecommerce] pauseNurture error", err);
  }
}
...
export async function resumeNurture(by: string): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("[nurture-control/ecommerce] resumeNurture write failed", res.status);
    }
  } catch (err) {
    console.error("[nurture-control/ecommerce] resumeNurture error", err);
  }
}
...
export async function recordGuardrailAlert(key: string): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("[nurture-control/ecommerce] recordGuardrailAlert write failed", res.status);
    }
  } catch (err) {
    console.error("[nurture-control/ecommerce] recordGuardrailAlert error", err);
  }
}
...
export async function recordCronHeartbeat(): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("[nurture-control/ecommerce] recordCronHeartbeat write failed", res.status);
    }
  } catch (err) {
    console.error("[nurture-control/ecommerce] recordCronHeartbeat error", err);
  }
}
...
export async function recordDigestHeartbeat(): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("[nurture-control/ecommerce] recordDigestHeartbeat write failed", res.status);
    }
  } catch (err) {
    console.error("[nurture-control/ecommerce] recordDigestHeartbeat error", err);
  }
}
