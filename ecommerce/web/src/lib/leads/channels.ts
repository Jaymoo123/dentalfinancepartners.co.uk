/**
 * Ecommerce lead-nurture channel sender: implements the engine's ChannelSender
 ...
 *
 * Mirrors Property/web/src/lib/leads/channels.ts with Ecommerce Finance
...
function serviceFrom(): string {
  const name = process.env.LEAD_SERVICE_FROM_NAME || "Ecommerce Finance";
...
export function twilioAuth(): TwilioAuth | null {
  ...
  if (account.startsWith("SK")) {
    console.error(
      "[lead-nurture/ecommerce] TWILIO_ACCOUNT_SID looks like an API Key SID (SK...). " +
