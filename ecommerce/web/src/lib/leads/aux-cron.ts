 ...
 *
 * House style: no em-dashes. British English. Ecommerce Finance brand.
 */
...
export function buildIcsForSlot(params: IcsSlotParams): string {
  ...
  return [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Ecommerce Finance//Lead Review//EN",
    "BEGIN:VEVENT",
    `UID:${uid}`,
...
export async function runLeadAuxScans(): Promise<{ reminders: number; nudges: number }> {
  ...
  try {
        ...
        if (inT24) {
          ...
          if (sendRowId) {
            try {
              ...
              await markSendStatus(sendRowId, "failed");
              console.error("[lead-aux-cron/ecommerce] T-24 email error", err);
            }
          }
        }
          ...
          if (inT2 && inSendWindow(now, true)) {
            ...
            if (sendRowId && lead.phone) {
              try {
                ...
                await markSendStatus(sendRowId, "failed");
                console.error("[lead-aux-cron/ecommerce] T-2 SMS error", err);
              }
        ...
        if (eligibleIds.length > 0 && inSendWindow(now, true)) {
          ...
          for (const lead of nudgeLeadsRes.data) {
            ...
            try {
              ...
              await markSendStatus(sendRowId, "failed");
              console.error("[lead-aux-cron/ecommerce] abandoned nudge error", err);
            }
          }
        }
      }
    ...
    console.error("[lead-aux-cron/ecommerce] unexpected top-level error", err);
  }
