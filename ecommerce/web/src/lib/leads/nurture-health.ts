/**
 * Observability health reader and guardrail evaluator for the Ecommerce
 * lead-nurture system.
 *
 * Mirrors Property/web/src/lib/leads/nurture-health.ts exactly, with
 * 'property' literals replaced by 'ecommerce' and the Ecommerce Finance
...
export async function runNurtureGuardrails(opts: {
  ...
  const health = await getNurtureHealth("ecommerce");
  if (!health) {
  ...
  if (!alreadyAlerted && process.env.RESEND_API_KEY) {
    ...
    try {
      ...
      console.error("[nurture-health/ecommerce] guardrail alert send failed", err);
    }
  }
  ...
  if (opts.autopauseEnabled && verdict.shouldPause && !control.paused) {
    try {
      ...
      console.error("[nurture-health/ecommerce] auto-pause failed", err);
    }
  }
