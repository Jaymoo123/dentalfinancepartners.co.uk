...
/**
 * Client tab switcher for the visitor detail page in the estate console.
 *
 * Equivalent to the generalist VisitorTabs but without the @/lib/utils cn()
 * dependency (the console app does not bundle shadcn). Uses a simple string
 * concat instead.
 */
...
const DOT: Record<StorySession["lines"][number]["kind"], string> = {
  ...
  step: "bg-emerald-500",
  convert: "bg-emerald-600 ring-2 ring-emerald-200",
  close: "bg-slate-400",
};
    ...
    <div className="mt-8">
      <div className="flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 p-1 text-sm">
        ...
        <span className="ml-auto pr-2 text-xs text-slate-400">
          {tab === "story"
            ? "What this person did, in plain English"
...
function TabButton({
  ...
  const base = "rounded-md px-4 py-1.5 font-semibold transition-colors";
  const cls = active
...
function Story({ story }: { story: StorySession[] }) {
  if (story.length === 0)
    return <p className="text-slate-400">No activity for this visitor yet.</p>;
  return (
        ...
        <section key={s.sessionId} className="overflow-hidden rounded-xl border border-slate-200 bg-white">
          <header className="border-b border-slate-100 bg-slate-50 px-4 py-2.5">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500">
              Visit {si + 1} &middot;{" "}
              {new Date(s.startTs).toLocaleDateString("en-GB", {
                weekday: "short",
                day: "numeric",
                month: "short",
              })}
          ...
          <ol className="px-4 py-3">
            {s.lines.map((l, i) => (
              <li key={i} className="relative flex gap-3 pb-3 last:pb-0">
                ...
                <span
                  className={`relative z-10 mt-1.5 h-2.5 w-2.5 shrink-0 rounded-full ${DOT[l.kind]}`}
                  aria-hidden
                />
  ...
  return (
    <ol className="overflow-hidden rounded-xl border border-slate-200 bg-white">
      {rows.map((r, i) => (
        <li
          key={i}
          className={`flex items-baseline gap-3 border-b border-slate-50 px-4 py-1.5 text-sm last:border-0 ${r.kind === "engaged" ? "bg-slate-50/60" : ""}`}
        >
          <span className="w-14 shrink-0 font-mono text-[11px] text-slate-400">{clockTime(r.ts)}</span>
          <span className={r.kind === "engaged" ? "italic text-slate-500" : "text-slate-800"}>
            {r.text}
          </span>
        </li>
      ))}
