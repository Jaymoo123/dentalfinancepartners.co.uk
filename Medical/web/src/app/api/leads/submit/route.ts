 ...
 *
 * Environment isolation: the shared factory returns a success-shaped no-op
 * outside production so preview browsing never creates real leads. Set
 * LEADS_ALLOW_NONPROD_SUBMIT=1 to override during testing.
 */
...
const sharedHandler = createLeadSubmitHandler({ source: "medical" });
...
export async function POST(req: NextRequest) {
  ...
  const source = String(body.source ?? "medical").toLowerCase();
  ...
  if (!isResource && !json.merged) {
    try {
      ...
      console.error("[leads/submit/medical] enrol/instant-touch failed", e);
    }
  }
