...
export const nhsPensionTool: GenericTool = {
  ...
  intro:
    "The NHS pension annual allowance can be tapered for high earners, reducing from the standard Â£60,000 to as low as Â£10,000. Enter your threshold income, annual pension growth from your NHS Pension Savings Statement, and tax band to see your allowance and any potential charge.",
  fields: [
    {
      id: "thresholdIncome",
    ...
    return {
      headline: r.excess > 0
        ? {
            ...
            sub: `Allowance: ${gbp(r.annualAllowance)} Â· Excess: ${gbp(r.excess)}`,
            tone: "warn" as const,
          }
        : {
            label: "Annual allowance",
            value: gbp(r.annualAllowance),
            sub: r.isTapered ? "Tapered â€” no excess charge" : "Standard allowance â€” no excess charge",
            tone: "good" as const,
          },
      rows,
      note: r.isTapered
        ? "Your allowance is tapered because threshold income exceeds Â£200,000 and adjusted income exceeds Â£260,000. Allowance reduces by Â£1 for every Â£2 above Â£260,000, with a floor of Â£10,000."
        : "Tapering only applies when threshold income exceeds Â£200,000 AND adjusted income exceeds Â£260,000. Your position does not currently meet both tests.",
    };
  },
  explainer: {
    heading: "How the tapered annual allowance works",
    paragraphs: [
      "The annual allowance is the maximum pension growth (employer + employee contributions, plus DB accrual) you can have in a tax year without a tax charge. The standard allowance for 2025/26 is Â£60,000.",
      "Tapering reduces this allowance for high earners. It applies when both: your threshold income exceeds Â£200,000, AND your adjusted income (threshold income plus employer pension contributions) exceeds Â£260,000. The allowance reduces by Â£1 for every Â£2 of adjusted income above Â£260,000, down to a minimum of Â£10,000.",
      "NHS doctors are particularly exposed because the deemed employer contribution to the NHS Pension Scheme can be significant, pushing adjusted income well above the Â£260,000 limit even on a moderate NHS salary. Check your Pension Savings Statement from NHSBSA each year.",
    ],
  },
  faqs: [
    {
