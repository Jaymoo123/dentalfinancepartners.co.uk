/**
 * Journey model for Medical Accountants UK -- deterministic, client-side, instant.
 * Ported from the SHIPPED Dentists R3 journeyModel.ts with the `ma` storage prefix
 * and the Medical 5-topic taxonomy.
 *
 * Reads the in-app analytics bus + route changes and accumulates THIS session's
 * trail (pages + topic, sections read, scroll depth, special-page visits,
 * calculator use, friction). From the trail it derives an intent PROFILE that
 * the proactive assistant's opener uses to pick a tailored message.
 *
 ...
 *
 * Storage key: `ma_journey` (site prefix `ma`, never `dfp_`/`hd_`/`ptp_`).
 * Special-page flags: /about, /services, /contact -- all three routes exist on
 * Medical (niche.navigation has /services, /about, /contact), so all flags are kept.
 * Medical's deriveTopic does NOT itself track /about/services/contact (returns null
 * for them); the journey model reads the raw pathname prefix for these flags.
 */
...
export type JourneyProfile = {
  ...
  depth: number;
  /** Combination flags: multi-topic, visited-about, visited-services, visited-contact,
   *  used-calculator, returning, friction, deep-read. */
  signals: string[];
  pageCount: number;
};
...
export function getJourneyProfile(): JourneyProfile {
  ...
  // Stage ladder (verbatim from Property/Dentists):
