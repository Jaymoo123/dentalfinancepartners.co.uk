/**
 * Client-side helper: submit a Medical Accountants UK lead through the server
...
export interface MedicalLeadPayload {
  full_name: string;
  email: string;
  phone: string;
  role: string;
...
export interface MedicalLeadResult {
  success: boolean;
  error?: string;
  leadId?: string | null;
  /** Signed token for the native booking picker (thank-you page embed). */
  bookingToken?: string;
  verify?: {
 ...
 */
export async function submitMedicalLead(
  payload: MedicalLeadPayload,
  enquiryRef = "",
): Promise<MedicalLeadResult> {
  try {
    ...
    let json: MedicalLeadResult = { success: false };
    try {
      json = (await res.json()) as MedicalLeadResult;
    } catch {
      /* non-JSON body */
    }
    if (!res.ok) {
