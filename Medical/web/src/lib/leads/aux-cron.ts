/**
 * Auxiliary scheduled sends for Medical Accountants UK: booked-slot reminders
...
export function buildIcsForSlot(params: IcsSlotParams): string {
  ...
  const uid     = `${params.leadId}-${params.date}-${params.windowKey}@medicalaccounts.co.uk`;
  const summary = `Medical Accountants UK review call (${params.label})`;
  const desc    = "A specialist medical accountant will call you in this window. Nothing to prepare.";

  return [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Medical Accountants UK//Lead Review//EN",
    "BEGIN:VEVENT",
    `UID:${uid}`,
