/**
 * Observability health reader and guardrail evaluator for the Medical
 * Accountants UK lead-nurture system.
 *
 * getNurtureHealth: reads vw_lead_nurture_health (service role, scoped to
 * site_key='medical') and maps the result to NurtureHealth. Returns null if
...
export async function runNurtureGuardrails(opts: {
  ...
  const health = await getNurtureHealth("medical");
  if (!health) {
