...
/**
 * Email-gated download unlock for Medical Accountants UK.
 *
 ...
 * Uses Medical-defined tokens: --navy, --copper, --copper-soft, --copper-strong,
 ...
 *   - does NOT 400 on empty name/phone (D-3: email_only relaxes validation)
 *   - excludes these rows from the partner-CC notify path (migration 20260706000001)
 *
 * Honeypot: enquiry_ref. Value is passed through to submitMedicalLead; do NOT
...
export function ResourceGate({
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Pass through to submitMedicalLead so the server stores the row flagged.
    ...
    const result = await submitMedicalLead(
      {
        full_name: "",
        email,
        phone: "",
    ...
    if (typeof window !== "undefined" && "gtag" in window) {
      ...
      if (gtag) {
        gtag("event", "generate_lead", {
          ...
          event_label: `medical_resource_${topic}`,
          value: 1,
        });
      }
    }
      ...
      <div className={["grid", split ? "@3xl:grid-cols-[1.1fr_0.9fr]" : ""].join(" ").trim()}>
        ...
        >
          {status === "success" ? (
            <div
              role="status"
              className="rounded-xl border border-[var(--copper-soft)] bg-[var(--surface-elevated)] p-5"
            >
              ...
              <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                ...
                {guideReady && resource?.guide && (
                  <a
                    ...
                    className="inline-flex items-center justify-center rounded-full border-2 border-[var(--copper)] px-5 py-3 text-base font-semibold text-[var(--navy)] transition-colors hover:bg-[var(--copper-soft)]"
                  >
