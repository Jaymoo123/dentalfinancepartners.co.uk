...
/**
 * Curated specialist widget for Medical Accountants UK.
 *
 ...
 * (gated via the intent context). Styled with Medical CSS-variable tokens.
 *
...
const AUTO_OPEN_KEY = "ma_assistant_autoopened";
...
export function SpecialistWidget() {
  ...
  useEffect(() => {
    ...
    initJourneyModel();
    try {
      window.sessionStorage.setItem("ma_assistant_active", "1");
    } catch {
      /* ignore */
    }
  }, [active]);
          ...
          <div className="flex-1 space-y-3 overflow-y-auto bg-[var(--surface-elevated)] p-4">
            {peekLine && (
              <div className="flex items-start gap-2">
                <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[var(--copper)] text-white">
