...
/**
 * Dependency-free premium calculator renderer for the Medical Accountants UK site.
 *
 ...
 *
 * TOKEN DISCIPLINE: Medical uses navy #001b3d + copper #b87333. var(--gold) and
 * var(--accent) are aliases onto copper. NEVER var(--primary).
 *   - var(--gold) / var(--accent): brand accent (active states, focus rings, bars)
 *   - var(--navy): dark brand fill (toggle ON state, accent chip background)
...
const inputCls =
  "w-full min-h-10 rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm text-[var(--ink)] focus:border-[var(--gold)] focus:outline-none focus:ring-2 focus:ring-[var(--gold)]/25";
            ...
            return (
              <button
                ...
                className={[
                  ...
                  active
                    ? "border-[var(--gold)] bg-[var(--gold)]/10 text-[var(--navy)] font-semibold"
      ...
      >
        ...
        <span
          className={[
            ...
            value ? "bg-[var(--navy)]" : "bg-[var(--border)]",
          ].join(" ")}
        >
          <input
            id={`pf-${field.id}`}
      ...
      {scenarios.map((s) => (
        ...
        >
          <div className="flex items-start justify-between gap-1">
            ...
            {s.best && (
              <span className="rounded-full bg-[var(--gold)]/10 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-[var(--navy)]">
                Better
              </span>
            )}
          </div>
...
export function PremiumCalculator({
  ...
  return (
    <>
      ...
      >
        {/* Top accent bar -- gold/copper brand token. NEVER var(--primary). */}
