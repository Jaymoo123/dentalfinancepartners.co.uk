...
/**
 * Exit-intent lead capture for Medical Accountants UK blog and calculator
 * pages. Self-contained: mounted once globally, arms only on the relevant
 * routes (blog + calculator pages), derives its topic from the URL or the
 * TopicOverrideCtx (for flat-routed blog posts), and triggers on desktop
 * (mouse leaves the top) OR mobile (leave-style scroll back to the top after
 * going deep). Uses the qualified MiniCapture (name + phone + email + message)
 * with a topic-aware offer.
 *
 * This is a qualified-lead capture modal, not a newsletter.
 *
 * Shared per-session cap with DeepScrollModal: at most ONE topic-offer modal
 * per session, whichever surface fires first (key: ma_modal_shown).
 *
 * R3-equivalent stand-down guard: when `ma_assistant_active` is set to "1"
...
import { useTopicOverride } from "@/components/intent/IntentProvider";

const STORAGE_KEY = "ma-exit-intent-dismissed-v1";
...
export function ExitIntentModal() {
  ...
  // URL-derived topic (covers calculators, hubs, /for-*, /nhs-pension)
  const urlTopicKey = deriveTopic(pathname);
  // For flat blog posts, the renderer injects the category-resolved topic
  const overrideTopicKey = useTopicOverride();
  const topicKey = overrideTopicKey ?? urlTopicKey;
  ...
  useEffect(() => {
    ...
    const fire = () => {
      ...
      // ma_assistant_active is set by the assistant component (not yet built).
      if (window.sessionStorage.getItem("ma_assistant_active") === "1") return;
      try {
  ...
  const blurb =
    "Tell us where to reach you and a specialist medical accountant will review your position and the sensible next step, with no obligation.";
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-lg border-l-4 border-[var(--copper)] bg-white p-6 shadow-2xl sm:p-8">
        <button
          type="button"
