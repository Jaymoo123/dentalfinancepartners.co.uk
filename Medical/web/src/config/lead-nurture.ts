/**
 * Medical Accountants UK lead-nurture composition: the contactability
 * sequence + per-lead message context. This is the ONLY place Medical copy
 ...
 * carve-out.
 *
 * Audience: GPs, salaried doctors, locums, hospital consultants.
 * Themes: NHS pension / annual allowance, superannuation, locum vs salaried,
 * practice / partnership finance, private-practice tax.
 * Style: faceless team voice, no named individual, no credential claims,
 * British English, no em-dashes.
 *
 * Cadence (contactability): instant email (step 0) fires synchronously at
 * submit; instant SMS (step 1) at the same time if in the send window. Then
 * 6 escalating follow-ups over about 11 days. Any two-way response (reply /
...
const SEQUENCE_NAME = "medical_contactability";
const DETAIL_CAPTURE_SEQUENCE_NAME = "medical_detail_capture";
...
const COMPANY = "Medical Accountants UK";
const SIGNOFF = `Speak soon, the team at ${COMPANY}`;
const FOOTER =
  "You are receiving this because you submitted an enquiry on medicalaccounts.co.uk.";
const OPT_OUT = "Reply STOP to opt out.";

// ponytail: CALC_MAP maps intent categories to Medical tools at /tools/[slug].
// Populated for the highest-traffic tools; extend as usage data confirms click-through.
const CALC_MAP: Record<string, { name: string; path: string }> = {
  nhs_pension:      { name: "NHS Pension Calculator", path: "/tools/nhs-pension-calculator" },
  annual_allowance: { name: "Annual Allowance Calculator", path: "/tools/nhs-pension-calculator" },
  locum_tax:        { name: "Locum Tax Calculator", path: "/tools/locum-tax-calculator" },
  incorporation:    { name: "Incorporation Calculator", path: "/tools/incorporation-calculator" },
};
...
function emailMsg(
  ...
  // Medical has no /book flow: CTA defaults to confirm link (reply-only style).
  const cta =
    opts?.cta === undefined
      ? null
      : opts.cta;
...
const STEPS: LeadNurtureStep[] = [
  ...
  // ── Step 3: Day 1 SMS + WhatsApp ──────────────────────────────────────────
  {
    ...
    buildMessages: (c) => [
      smsMsgWithGen(
        c,
        "day1_sms",
        `Hi ${c.firstName}, following up on your medical tax enquiry. We have kept some specialist time free this week for a short call. Reply YES and we will call you. ${c.optOutText}`,
      ),
  ...
  // ── Step 4: Day 2 give-value email ────────────────────────────────────────
  {
    ...
    buildMessages: (c) => {
      ...
      const calcLine = c.calculatorName && c.calculatorUrl
        ? `In the meantime, our ${c.calculatorName} is worth a look: ${c.calculatorUrl}`
        : null;
      ...
      return [
        emailMsg(
          c,
          `One thing doctors often overlook, ${c.firstName}`,
          "The annual allowance charge catches many. A short call often surfaces the gap.",
          paras,
          "day2_give_email",
          { cta: null, secondary: null },
        ),
    ...
    buildMessages: (c) => [
      emailMsg(
        ...
        [
          ...
          "For what it is worth, the moments a medical tax review really earns its keep are when an annual allowance charge lands, when you move from salaried to locum work, or when private practice income starts to complicate the picture. If one of those applies later, reply to this email even months from now and we will pick it straight back up.",
          "All the best.",
        ],
        "breakup_day11",
        { cta: null, secondary: null },
      ...
      return [
        emailMsg(
          ...
          "detail_capture_t0",
          {
            cta: null,
            secondary: null,
            ...detailGreeting(c, "Hi, the team at Medical Accountants UK here."),
          },
        ),
      ];
    },
      ...
      return [
        emailMsg(
          ...
          "detail_capture_day1",
          {
            cta: null,
            secondary: null,
            ...detailGreeting(c, "Hi, the team at Medical Accountants UK here again."),
          },
        ),
      ];
    },
      ...
      return [
        emailMsg(
          ...
          "detail_capture_day7",
          {
            cta: null,
            secondary: null,
            ...detailGreeting(c, "Hi, one last message from the team at Medical Accountants UK."),
          },
        ),
      ];
    },
...
export function buildMedicalLeadNurtureConfig(
...
export function buildMedicalLeadNurtureConfigs(): LeadNurtureConfig[] {
...
//   "I agree to be contacted about my enquiry. I understand that Medical Accountants UK " +
//   "may follow up by email and phone over the next 11 days to arrange a free call. " +
