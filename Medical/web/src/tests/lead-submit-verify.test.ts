 ...
 * lead_verification with a verify_pass/verify_fail event, resource-download
 * skip, fail-open on verifier errors, and enrolment wiring.
 *
 * The resource-gate case is the regression test for the live bug found
 * 2026-08-19: ResourceGate.tsx submits role:"resource" + extras.resource_gate,
 * and the submit route was enrolling every one of those inserts into
 * medical_detail_capture. A resource submission must be neither verified nor
 * enrolled.
 *
...
vi.mock("@/config/lead-nurture", () => ({
  routePrimarySequence: () => "medical_contactability",
}));
...
const BASE_BODY = {
  ...
  email: "sarah.ahmed@gppractice.nhs.uk",
  phone: "+447700900123",
  role: "GP partner",
  message: "Looking for advice on NHS pension annual allowance and incorporating the practice.",
  source: "medical",
};
...
describe("submit tail: resource downloads (2026-08-19 live bug regression)", () => {
  it("role='resource' is neither verified nor enrolled, but still succeeds", async () => {
    const res = await POST(
      submitReq({
        ...
        phone: "",
        role: "resource",
        message: "[Resource: nhs_pension] NHS Pension Annual Allowance model",
