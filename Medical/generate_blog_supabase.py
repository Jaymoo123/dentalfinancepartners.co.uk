...
def fetch_unused_topic():
    ...
    print(f"Selected topic (priority {topic.get('publish_priority', 'N/A')}, difficulty {topic.get('keyword_difficulty', 'N/A')}): {topic['keyword']}")
...
"""
    ...
    user_prompt = f"""Generate a comprehensive blog post for UK medical professionals (GPs, consultants, and locum doctors).
...
def main():
    ...
    if not topic_row:
        print("No topics to process. Add topics to the blog_topics_medical table.")
        return
    
    topic_id = topic_row["id"]
    
    print(f"[OK] Found topic: {topic_row['keyword']}")
    if topic_row.get("keyword"):
        print(f"     Primary keyword: {topic_row['keyword']}")
