 ...
 *   points to /contact.
 * - Submit uses submitGeneralistLead (not submitPropertyLead) with
 *   captureMode: "email_only" and extras: { capture_channel: "assistant", trigger }.
 * - Opener uses pickOpener / exitOpener / frictionOpener from opener.ts (generalist).
...
const AUTO_OPEN_KEY = "hd_assistant_autoopened";
...
export function SpecialistWidget() {
  ...
  // Suppress for converted visitors (no booking-concierge mode in R3).
  ...
  // The visitor engaged: stop this session's proactive cadence and clear the badge.
  ...
  useEffect(() => {
    ...
    initJourneyModel();
    try {
      window.sessionStorage.setItem("hd_assistant_active", "1");
    } catch {
      /* ignore */
    }
  }, [active]);
  ...
  // Auto-open once per session (desktop: open panel; mobile: peek).
  useEffect(() => {
    if (!active || suppressed || typeof window === "undefined") return;
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Honeypot: non-semantic name so autofill/password managers never target it.
    // Passed through (not silently dropped) so a real human caught by autofill is never lost.
    ...
    ft.onLead({ source: niche.content_strategy.source_identifier, role: "specialist_widget" });
    setStatus("success");
  }
  ...
  return (
    <div className="fixed bottom-24 right-4 z-[55] flex flex-col items-end print:hidden">
      {open && (
        <div
          role="dialog"
          aria-label="Holloway Davies specialist"
        ...
        >
          ...
          <div className="flex items-center gap-3 bg-slate-900 px-4 py-3 text-white">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-bold leading-tight">Holloway Davies</p>
              <p className="truncate text-[11px] text-slate-300">A specialist replies within one working day</p>
            </div>
