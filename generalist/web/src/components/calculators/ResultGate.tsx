 ...
 * interstitial until the reader either submits or skips. Ported from Property.
 *
 ...
 *   what makes the gate feel worth answering.
 */
      ...
      {showResult ? (
        ...
        <div>
          ...
          <button type="button" onClick={reopen} className={`${btnSecondary} mt-4 w-full`}>
            Get this figure checked by an accountant
          </button>
        </div>
      ) : (
        <HeldResult ground={ground} onReveal={requestReveal}>
          {children}
        </HeldResult>
      )}
