...
const ENGAGED_GUIDE_MS = 60_000; // engaged reader -> offer a free review
...
function reviewOffer(
  ...
  return {
    kind: "specialist",
    title: t.ctaCopy,
    blurb: "A free, no-obligation review of your position with a specialist accountant.",
    href: "/contact",
    reason,
  };
}
...
function specialistOffer(topicKey: TopicKey): IntentOffer {
  const t = getTopic(topicKey);
  const label = (t?.label ?? "your accounting").toLowerCase();
  return {
    kind: "specialist",
