...
export function buildIcsForSlot(params: IcsSlotParams): string {
  ...
  const uid     = `${params.leadId}-${params.date}-${params.windowKey}@hollowaydavies.co.uk`;
  const summary = `Accounting review call (${params.label})`;
  const desc    = "An accountant will call you in this window. Nothing to prepare.";

  return [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Holloway Davies//Lead Review//EN",
    "BEGIN:VEVENT",
    `UID:${uid}`,
...
export async function runLeadAuxScans(): Promise<{ reminders: number; nudges: number }> {
  ...
  try {
          ...
          if (sendRowId) {
            try {
              ...
              const { html, text } = renderLeadServiceEmail({
                ...
                paragraphs: [
                  `Your free accounting review call is tomorrow, ${windowPhrase}.`,
                  "A qualified accountant from our partner team will ring you then. They will have read your enquiry before they call, the call takes about 20 minutes, and there is nothing to prepare.",
        ...
        if (!isMorning) {
          ...
          if (inT2 && inSendWindow(now, true)) {
            ...
            if (sendRowId && lead.phone) {
              try {
                const body =
                  `Hi ${firstName}, your free accounting review call is later today, ${label}. ` +
              ...
              // Claimed but no phone: mark skipped so it does not loop.
              await markSendStatus(sendRowId, "skipped");
            }
          }
        }
    ...
    // Apply the upper bound (>= 2h ago) in JS since adminSelect carries only one value per key.
    ...
    if (viewedLeadIds.length > 0) {
      // Only leads active in the standard contactability chase.
      ...
      if (candidateIds.length > 0) {
        // Exclude those with any booked event.
