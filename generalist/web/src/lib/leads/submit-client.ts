/**
 * Client-side helper: submit a Holloway Davies lead through the server
...
export interface GeneralistLeadPayload {
  full_name: string;
  email: string;
  phone: string;
  role: string;
...
export interface GeneralistLeadResult {
  success: boolean;
  error?: string;
  leadId?: string | null;
...
export async function submitGeneralistLead(
  payload: GeneralistLeadPayload,
  honeypot = "",
): Promise<GeneralistLeadResult> {
  try {
    ...
    let json: GeneralistLeadResult = { success: false };
    try {
      json = (await res.json()) as GeneralistLeadResult;
    } catch {
      /* non-JSON body */
    }
    if (!res.ok) {
