/**
 * Daily digest composer and runner for the Holloway Davies lead-nurture system.
 *
...
export async function gatherDigestData(siteKey: string): Promise<DigestData> {
  ...
  // Filter by source='generalist' to prevent cross-site bleed.
  ...
  if (failedEvents.length > 0) {
    ...
    const nameMap = new Map<string, string | null>(
      // ponytail: JS-filter by source='generalist' to prevent cross-site bleed
      (nameRes.data ?? [])
        .filter((r) => r.source === "generalist")
        .map((r) => [r.id, r.full_name ?? null]),
    );

    // Only include events whose lead belongs to generalist (nameMap membership)
...
export function composeDigestEmail(d: DigestData): {
  ...
  lines.push(`Automated digest from the Holloway Davies lead-nurture system.`);
...
export async function runNurtureDigest(): Promise<{ sent: boolean }> {
  const d = await gatherDigestData("generalist");
