 ...
 * partner names, no action buttons besides the offer-to-buyers link below.
 *
...
export async function sendContactableHandoff(
  ...
  // Offer-to-buyers link: the value-score/teaser stack is estate-central and
  // lives only in Property/web, so this site links straight out to the
  // Property-hosted offer confirm page (which renders the teaser) rather than
  // building a teaser here. Requires LEAD_NURTURE_TOKEN_SECRET to be shared
  // across deployments so the minted token verifies on the Property side.
  // Best-effort, never blocks the handoff.
  try {
    const offerBase = process.env.LEAD_OFFER_BASE_URL || "https://www.propertytaxpartners.co.uk";
    const offerUrl = `${offerBase}/api/leads/offer/${mintLeadToken(leadId, "offer")}`;
    html += `<p style="margin:20px 0 0;"><a href="${offerUrl}" style="display:inline-block;font-size:15px;font-weight:700;color:#ffffff;text-decoration:none;padding:11px 22px;border-radius:6px;background-color:#0f172a;">Offer to buyers</a></p>`;
    text += `\n\nOffer to buyers: ${offerUrl}`;
  } catch (err) {
    console.error("[handoff] offer-to-buyers link failed", err);
  }
