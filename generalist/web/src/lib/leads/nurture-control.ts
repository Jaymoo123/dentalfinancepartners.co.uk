/**
 * Kill-switch control library for the Holloway Davies lead-nurture system.
 *
 * Reads and writes the lead_nurture_control table, keyed by site_key='generalist'
...
/**
 * Read the control row for site_key='generalist'. FAIL-OPEN: returns the
 * all-false/null default on any error (missing table, missing row, network
...
/**
 * Pause all nurture sends. Upserts site_key='generalist' with paused=true.
 * Fails silently if the table is missing.
 */
...
/**
 * Resume nurture sends. Upserts site_key='generalist' with paused=false and
 * clears all pause metadata. Fails silently if the table is missing.
 */
...
/**
 * Record that a guardrail alert was fired. Upserts site_key='generalist'
 * updating only last_alert_at, last_alert_key, and updated_at (merge-duplicates
 * strategy means paused state is untouched on conflict). Fails silently.
 */
...
/**
 * Record that the hourly lead-nurture cron has fired. Upserts site_key='generalist'
 * updating only last_cron_run_at and updated_at. Fails silently. Never throws.
 */
 ...
 * site_key='generalist' updating only last_digest_run_at and updated_at.
