 ...
 * reachable AND has actively responded, so it is safe to hand to the specialist
 * partner firm. This is the direct fix for "only 3 of 9 were contactable".
 *
 ...
 *   is not known-bad, so the specialist firm can still reach the lead by phone.
 *
...
export async function promoteIfContactable(leadId: string): Promise<PromoteResult> {
  try {
    ...
    if (handoff.sent === true || (handoff.skipped && handoff.skipped !== "db-error")) {
      ...
      // The contactable -> forwarded flip is OPERATOR-driven: it happens when the
      // operator clicks "I have forwarded this to the partner firm" in the handoff
      // email (POST /api/leads/forwarded/[token]), so 'forwarded' means a real
      // partner-firm hand-over, not merely that our brief email was delivered.
    } else {
