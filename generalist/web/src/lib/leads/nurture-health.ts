/**
 * Observability health reader and guardrail evaluator for the Holloway Davies
 * lead-nurture system.
 *
 * getNurtureHealth: reads vw_lead_nurture_health for site_key='generalist'.
...
export async function runNurtureGuardrails(opts: {
  ...
  const health = await getNurtureHealth("generalist");
  if (!health) {
