 ...
 *     answers (e.g. "after 6pm, works in property") are never lost.
...
function buildAckBody(firstName: string): string {
  ...
  return `Great, thank you${name}. A member of the team will call you shortly. Nothing to prepare. Speak soon.`;
}
...
export async function acknowledgeEmailReply(opts: { leadId: string }): Promise<boolean> {
  try {
    ...
    const { html, text } = renderLeadServiceEmail({
      preheader: "Thanks, that is everything we need. A member of the team will call you.",
      greeting: `Hi ${ctx.firstName},`,
      paragraphs: [
        "Got your reply, thank you. That is everything we need.",
        "A member of the team will call you, and if you mentioned a day or time that suits, we will aim for it.",
      ...
      signoff: "Speak soon, the team at Holloway Davies",
      footerNote:
