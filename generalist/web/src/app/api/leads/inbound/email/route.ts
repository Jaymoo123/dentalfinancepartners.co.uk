/**
 * Inbound email webhook for Resend: processes replies to Holloway Davies nurture outreach.
 *
 * When a lead replies to a nurture email, Resend posts an email.received event here.
 * The route verifies the Svix signature, resolves the sender to a generalist lead,
 ...
 *   Inbound routing domain: hollowaydavies.co.uk (or a reply.* subdomain)
 *   Endpoint URL: https://www.hollowaydavies.co.uk/api/leads/inbound/email
 *   Events: email.received (inbound only)
 *   Signing secret (whsec_...): set as LEAD_RESEND_INBOUND_SECRET
 *
 * Classification: no AI/LLM classify step. This site has no copy/classify AI
 * layer wired (Opus-only content, Sonnet mechanical build only), so
 * classification is deterministic: opt-out phrases are checked first
 * (classifyEmailReplyIntent), and anything left over defaults straight to a
 * genuine reply, the same safe fallback other sites use whenever their AI
 * classifier is unconfigured or gated off.
 *
...
export async function POST(req: NextRequest) {
  ...
  // Resolve sender to a generalist lead (most recent match wins).
  ...
  try {
    // Detect opt-out deterministically. No AI classify step on this site (see
    // file header): every non-opt-out reply is treated as a genuine reply.
    ...
    // reply landed and a member of the team will call. Idempotent per lead,
    // dormancy and test gated, fail-soft inside (never throws out of the webhook).
