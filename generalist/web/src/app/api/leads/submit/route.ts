/**
 * Server chokepoint for Holloway Davies lead submission.
 *
...
const sharedHandler = createLeadSubmitHandler({ source: "generalist" });
...
export async function POST(req: NextRequest) {
  ...
  const source = String(body.source ?? "generalist").toLowerCase();
  ...
  if (!isResource && !json.merged) {
    try {
      ...
      console.error("[leads/submit/generalist] enrol/instant-touch failed", e);
    }
  }
