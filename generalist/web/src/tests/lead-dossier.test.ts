...
vi.mock("@/config/lead-nurture", () => ({
  ...
  LEAD_SEQUENCE_NAME: "generalist_contactability",
}));
...
vi.mock("@/lib/resend", () => ({
  ...
  getFromAddress: () => "Holloway Davies <ops@x>",
}));
...
function seedLead(source = "generalist") {
  db.leads.push({
    id: LID,
    full_name: "Sam Doe",
...
describe("acknowledgeReply", () => {
  it("sends exactly one ack per lead (idempotent)", async () => {
    ...
    expect(msg.body).toBe(
      "Great, thank you Sam. A member of the team will call you shortly. Nothing to prepare. Speak soon.",
    );
