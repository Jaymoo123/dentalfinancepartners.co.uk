/**
 * Excel model generator for the Holloway Davies premium resources system.
 *
 ...
 *
 * The generated files are COMMITTED (with fixed workbook timestamps for
 * byte-stable diffs). The generator imports the SAME locked constants the
 * site calculators use, so the spreadsheet rates and the on-site math derive
 * from one source and cannot drift; a golden test (scripts/resources/builders/*.test.ts)
 * asserts the TS compute === the xlsx formula result before a category's
 * `enabled` flag is flipped.
 */
...
// scripts/resources -> generalist/web/public/resources
