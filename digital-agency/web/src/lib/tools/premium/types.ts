/**
 * Types for the premium calculator tier on the Agency Founder Finance site (aff).
 *
 * Ported from Dentists/web/src/lib/tools/premium/types.ts and adapted for the
 * agency site token system. The type shapes are identical; only token names differ.
 *
 ...
 *
 * Storage prefix: aff (FROZEN). Grid keys: aff:grid:<toolId>. Never ptp:/dfp:/cfp:/bfp:.
 *
 * Token system: brand accent = var(--accent) #4f46e5 (indigo); dark fill /
 * second chart series = var(--ink) #0f172a (slate). The aff site does NOT define
 * --gold, --navy or --dark; use only --accent and --ink for chart series colours.
 *
 ...
 * already available across the agency site.
 */
import type {
  CalcField,
  CalcValues,
...
/* ---------------------------------------------------------------------------
 * Scenario comparison (e.g. two VAT schemes side by side)
 * ------------------------------------------------------------------------- */
...
export interface ChartSeries {
  ...
  label: string;
  /**
   * CSS custom property token name or a hex colour.
   * Use the aff token system only: var(--accent) for the primary series,
   * var(--ink) for the secondary series.
   * Never use var(--gold), var(--navy), var(--dark) or var(--primary) --
   * those are not defined on this site.
   */
  color: string;
}
