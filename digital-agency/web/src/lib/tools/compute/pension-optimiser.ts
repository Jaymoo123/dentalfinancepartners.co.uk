...
export type PensionOptimiserInput = {
  profit: number;
  salary: number;
  contribution: number;
...
export type PensionOptimiserOutput = {
  allowance: number;
  contribution: number;
  capped: boolean;
...
export function calcPensionOptimiser(input: PensionOptimiserInput): PensionOptimiserOutput {
  const allowance = taperedAnnualAllowance(input.adjustedIncome);
  const c = Math.min(input.contribution, allowance);
  const profitAfter = Math.max(0, input.profit - input.salary - c);
  const ctWithPension = corporationTax(profitAfter);

  const profitNoPension = Math.max(0, input.profit - input.salary);
  ...
  const dividendTaxIfTakenInstead = dividendTaxOnAmount(input.salary, dividendIfTakenInstead);
  ...
  return {
    allowance,
    contribution: c,
    capped: input.contribution > allowance,
    ctWithPension,
    ctNoPension,
    ctSaved,
    marginal,
