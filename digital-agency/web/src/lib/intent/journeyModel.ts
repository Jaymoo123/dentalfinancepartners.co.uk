/**
 * Journey model for Agency Founder Finance (digital-agency) -- deterministic,
 * client-side, instant. Ported from the Dentists/Property pattern with the
 * `aff` storage prefix.
 *
 ...
 *
 * Storage key: `aff_journey` (site prefix `aff`, FROZEN).
 * Special-page flags: /about, /services, /contact -- all three routes exist on
 * the agency site (confirmed), so all flags are kept.
 */
...
export function getJourneyProfile(): JourneyProfile {
  ...
  // Stage ladder (verbatim from Property/Dentists pattern):
