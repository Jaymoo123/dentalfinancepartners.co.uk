/**
 * Agency Founder Finance lead-nurture channel sender: implements the engine's
 ...
 *
 * Mirrors Property/web/src/lib/leads/channels.ts with Agency Founder Finance
...
function serviceFrom(): string {
  const name = process.env.LEAD_SERVICE_FROM_NAME || "Agency Founder Finance";
...
export function twilioAuth(): TwilioAuth | null {
  ...
  if (account.startsWith("SK")) {
    console.error(
      "[lead-nurture/agency] TWILIO_ACCOUNT_SID looks like an API Key SID (SK...). " +
