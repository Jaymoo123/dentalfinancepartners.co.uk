/**
 * Shared lead-nurture enrolment for the Agency Founder Finance site.
 *
 * Extracted from the submit route so all enrolment paths (submit, retro-enrol,
 * reconciliation) route through ONE idempotent, arming-aware, sequence-aware
 * code path. Mirrors Property/web/src/lib/leads/enroll.ts with Agency Founder
 * Finance sequence names.
 */
import { adminInsert, adminUpdate } from "@/lib/supabase/admin";
import { buildLeadChannelSender, leadNurtureArmed } from "@/lib/leads/channels";
import {
  buildAgencyLeadNurtureConfig,
...
export async function enrollLead(
  ...
  if (deferFirstTouch) {
    ...
    const config = buildAgencyLeadNurtureConfig(variant);
