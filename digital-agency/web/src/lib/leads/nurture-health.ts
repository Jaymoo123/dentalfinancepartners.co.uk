/**
 * Observability health reader and guardrail evaluator for the Agency Founder
 * Finance lead-nurture system.
 *
 * Mirrors Property/web/src/lib/leads/nurture-health.ts exactly, with
 * 'property' literals replaced by 'agency' and the Agency Founder Finance
...
export async function runNurtureGuardrails(opts: {
  ...
  const health = await getNurtureHealth("agency");
  if (!health) {
  ...
  if (!alreadyAlerted && process.env.RESEND_API_KEY) {
    ...
    try {
      ...
      console.error("[nurture-health/agency] guardrail alert send failed", err);
    }
  }
  ...
  if (opts.autopauseEnabled && verdict.shouldPause && !control.paused) {
    try {
      ...
      console.error("[nurture-health/agency] auto-pause failed", err);
    }
  }
