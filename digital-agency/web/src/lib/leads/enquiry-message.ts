...
// nothing reads them, so rather than invent speculative agency keys the map is
// empty and only the neutral fallback is live. Populate it when the enrichment
// layer actually emits agency intent categories.
const CATEGORY_PHRASE_MAP: Record<string, string> = {};

/**
 * Neutral fallback phrase when no clean echo exists, derived from the
 * intent category then role. Falls back to a generic agency finance phrase.
...
export function categoryPhrase(
  ...
  return "get your agency's numbers straight";
}
