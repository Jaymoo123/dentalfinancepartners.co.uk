/**
 * Client-side helper: submit an Agency Founder Finance lead through the server
 ...
 * an unverified row (matches Property and Dentists, 2026-08).
 *
...
export interface AffLeadPayload {
  full_name: string;
  email: string;
  phone: string;
  role: string;
...
export interface AffLeadResult {
  success: boolean;
  error?: string;
  leadId?: string | null;
 ...
 *
 * honeypot: the honeypot field value, pass whatever the hidden input holds.
 ...
 */
export async function submitAffLead(
  payload: AffLeadPayload,
  honeypot = "",
): Promise<AffLeadResult> {
  try {
    ...
    let json: AffLeadResult = { success: false };
    try {
      json = (await res.json()) as AffLeadResult;
    } catch {
      /* non-JSON body */
    }
    if (!res.ok) {
