...
/**
 * Curated specialist widget for Agency Founder Finance.
 *
 * A topic-aware opener plus an honest "ask a specialist" lead capture. Not a
 ...
 * (gated via the intent context). Styled with Agency Founder Finance tokens.
 *
 * TOKEN HARDENING: no var(--gold), no var(--navy), no var(--dark), no var(--primary).
 * Uses: var(--accent) = #4f46e5 (indigo), var(--ink) = #0f172a (slate).
 *
 ...
 *
 * Stand-down: sets aff_assistant_active so ExitIntentModal knows the assistant
 ...
 *
 * Chip: "Book a free call" -> /contact (no /book path on the agency site).
 * Widget capture: email + message via email_only captureMode through submitAffLead.
 * Consent: uses siteConfig.leadConsentText (the widget IS an enquiry, not a resource gate).
 *
 * aff divergence B: mounted in app/layout.tsx (NOT PageShell).
 */
...
// aff token-safe input class: --accent for focus ring, --ink for text
const inputClass =
  "mt-1 w-full rounded-lg border-2 border-slate-200 bg-white px-3 py-2 text-base text-[var(--ink)] focus:border-[var(--accent)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/20 min-h-12 touch-manipulation";
...
const AUTO_OPEN_KEY = "aff_assistant_autoopened";
...
export function SpecialistWidget() {
  ...
  useEffect(() => {
    ...
    initJourneyModel();
    try {
      window.sessionStorage.setItem("aff_assistant_active", "1");
    } catch {
      /* ignore */
    }
  }, [active]);
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // The widget IS an enquiry, so leadConsentText is correct here (not resourceConsentText).
    ...
    const result = await submitAffLead(payload, honeypot);
    if (!result.success) {
      setStatus("error");
  ...
  return (
    <div className="fixed bottom-24 right-4 z-[55] flex flex-col items-end print:hidden">
      {open && (
        <div
          role="dialog"
          aria-label="Agency Founder Finance assistant"
        ...
        >
          {/* Agent header: indigo background, white text */}
          <div
            className="flex items-center gap-3 px-4 py-3 text-white"
            style={{ background: "var(--accent)" }}
          >
            <span
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white/20 ring-2 ring-white/15"
            >
              <svg
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-bold leading-tight">Agency Founder Finance</p>
            ...
            <button
              ...
              aria-label="Close"
              onClick={closePanel}
              className="flex min-h-11 min-w-11 shrink-0 items-center justify-center text-2xl leading-none text-white/70 hover:text-white"
            >
              &times;
            </button>
          </div>
          ...
          <div className="flex-1 space-y-3 overflow-y-auto bg-slate-50 p-4">
            {peekLine && (
              <div className="flex items-start gap-2">
                ...
                <div className="max-w-[82%] rounded-2xl rounded-tl-sm border border-slate-200 bg-white px-3 py-2 text-sm leading-relaxed text-[var(--ink)] shadow-sm">
                  {peekLine}
                </div>
              </div>
            )}
            {status === "success" ? (
              <div className="flex items-start gap-2">
                ...
                <div className="max-w-[82%] rounded-2xl rounded-tl-sm border border-indigo-100 bg-indigo-50 px-3 py-2 text-sm font-medium text-[var(--ink)] shadow-sm">
                  Thanks, a specialist has your message and will be in touch by email. For specialist
                  advisory work we partner with Aswatax, Chartered Tax Advisers, so it may be their
                  team who replies. Please keep an eye on your inbox, and your spam or junk folder,
                  so our reply is not missed.
                </div>
              </div>
            ) : !composing ? (
              <div className="flex flex-wrap gap-2 pl-9">
                {calcSlug && (
                  <a
                    ...
                    className="inline-flex items-center rounded-full border border-indigo-200 bg-white px-3 py-3 text-sm font-medium text-[var(--ink)] hover:bg-indigo-50"
                  >
                    See your numbers
                  </a>
                )}
              ...
              <form
                ...
                onBlurCapture={(e) => {
                  ...
                  if (
                    ...
                    ft.onFieldBlur(
                      t.name,
                      Boolean(t.value && t.value.trim()),
                      t.name === "email" ? t.value.length : undefined,
                    );
                }}
              >
      ...
      {!open && peekVisible && peekLine && (
        <div className="mb-3 flex w-[min(88vw,20rem)] items-start gap-2 rounded-2xl border border-indigo-100 bg-white p-3 shadow-2xl">
          <button
            type="button"
          ...
          <button
            ...
            aria-label="Dismiss"
            onClick={dismissPeek}
            className="-mr-1 -mt-1 flex min-h-11 min-w-11 shrink-0 items-center justify-center rounded p-1 text-slate-400 hover:text-[var(--ink)]"
          >
            <svg
              className="h-4 w-4"
              fill="none"
