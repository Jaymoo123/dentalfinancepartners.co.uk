...
/**
 * Dependency-free premium calculator renderer for Agency Founder Finance (aff).
 *
 ...
 * and var(--ink) #0f172a (slate). This site does NOT define --gold, --navy or
 * --dark. All Dentists tokens have been re-tokened:
 *   var(--gold)  -> var(--accent)   brand accent (active states, focus rings, bars)
 *   var(--navy)  -> var(--ink)      dark brand fill (toggle ON state, chip background)
 *   var(--border), var(--surface), var(--surface-elevated): keep as-is (defined on aff)
 *   var(--ink), var(--ink-soft), var(--muted): keep as-is (defined on aff)
 * Do NOT use var(--primary) in classNames (estate portability rule); use var(--accent).
 *
 ...
 * Shows at most once per session (module-level flag, NEVER the aff_modal_shown key
 * which belongs to ExitIntentModal.tsx:85,96 -- do NOT touch that key).
 *
 * CalcResultCta: uses slug={config.id} (not campaign=; the aff CalcResultCta takes
 * slug, not campaign per the brief §4 prop fix).
 */
...
// This is the CANONICAL module flag (mirrors Dentists/Property exactly).
// It is NOT the aff_modal_shown sessionStorage key (that belongs to ExitIntentModal).
            ...
            return (
              <button
                ...
                className={[
                  ...
                  active
                    ? "border-[var(--accent)] bg-[var(--accent)]/10 text-[var(--ink)] font-semibold"
      ...
      >
        ...
        <span
          className={[
            ...
            value ? "bg-[var(--ink)]" : "bg-[var(--border)]",
          ].join(" ")}
        >
          <input
            id={`pf-${field.id}`}
      ...
      {scenarios.map((s) => (
        ...
        >
          <div className="flex items-start justify-between gap-1">
            ...
            {s.best && (
              <span className="rounded-full bg-[var(--accent)]/10 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-[var(--ink)]">
                Better
              </span>
            )}
          </div>
...
export function PremiumCalculator({
  ...
  return (
    <>
      ...
      >
        {/* Top accent bar -- indigo brand token (var(--accent)) */}
        ...
        {placement === "blog" && !gated && (
          <div className="border-t border-[var(--border)] bg-white px-5 py-4 sm:px-7">
            {/* CalcResultCta takes slug= (not campaign=) per brief §4 aff prop fix. */}
            <CalcResultCta slug={config.id} />
          </div>
        )}
      </div>
