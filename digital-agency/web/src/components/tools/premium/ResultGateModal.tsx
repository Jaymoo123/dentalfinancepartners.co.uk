...
/**
 * Result-gate interstitial for the Agency Founder Finance premium calculator tier.
 *
 ...
 * --dark. Every Dentists token has been re-tokened:
 *   --gold  -> --accent  (accent border on the modal card)
 *   --navy  -> --ink     (backdrop scrim bg-[var(--ink)]/60, heading colour)
 *   --accent remains --accent (focus rings)
 *   --muted, --ink used for text hierarchy
 *
 ...
 *   3. Once per session (module-level flag gateModalShownThisSession in PremiumCalculator).
 *
 ...
 *
 * R2 gate is a FULL MiniCapture capture (name + phone + email + message).
 * captureMode:"email_only" and extras.resource_gate are R3 only.
 */
...
export function ResultGateModal({
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-[60] flex items-center justify-center bg-[var(--ink)]/60 p-4"
      onClick={(e) => {
    ...
    >
      ...
      >
        ...
        <MiniCapture
          ...
          blurb="A calculator gives the shape of the answer. Agency salary and dividend planning, VAT, exit relief and R&D claims are unforgiving in the detail. Tell us your situation and an agency finance specialist will confirm your exact figure and the sensible next step, with no obligation."
          submitLabel="Get my figure confirmed"
          successText="Thanks, we will be in touch within one working day. Your result is below."
          className="mt-2"
          messagePlaceholder="The more detail the better. Tell us about your agency situation, rough figures, and what you are trying to work out. A couple of sentences is ideal."
          messageMinLength={40}
          messageMinWords={8}
          onSuccess={onReveal}
        />
