...
/**
 * Returning-visitor greeting for Agency Founder Finance. A dismissible bottom
 ...
 *
 * Storage key uses aff_ prefix (FROZEN). Indigo palette to match brand.
 */
import { useEffect, useRef, useState } from "react";
import Link from "next/link";
import { useIntent, trackPersonalization } from "./IntentProvider";

const DISMISS_KEY = "aff_returning_bar_dismissed";
...
export function ReturningBar() {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-indigo-700 bg-indigo-900 text-white shadow-2xl">
      <div className="mx-auto flex max-w-5xl items-center justify-between gap-3 px-4 py-3 text-sm">
        <span className="min-w-0">
          ...
          <span className="hidden text-indigo-200 sm:inline">{offer.blurb}</span>
        </span>
        <div className="flex shrink-0 items-center gap-3">
          <Link
            ...
            className="rounded bg-white px-3 py-1.5 font-semibold text-indigo-900 hover:bg-indigo-50"
          >
            {offer.title}
          </Link>
          <button
