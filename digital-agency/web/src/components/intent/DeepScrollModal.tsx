...
/**
 * Deep-scroll topic offer for Agency Founder Finance. When a not-yet-converted
 * reader gets deep into a page with a clear topic, surfaces that topic's
 * calculator or a free specialist review. Modal overlay = no layout shift.
 * Frequency-capped: one per session (module flag) and a 30-day per-topic
 * suppress. Measured via personalization_* events.
 *
 * Indigo palette (#4f46e5) to match the aff brand.
 */
...
const suppressKey = (topic: string) => `aff_deepscroll_${topic}`;
      ...
      >
        ...
        <div className="mt-5 flex flex-col gap-2">
          <Link
            ...
            className="rounded-lg bg-indigo-600 px-4 py-2.5 text-center font-semibold text-white hover:bg-indigo-700"
          >
            {primaryLabel}
          </Link>
          <Link
