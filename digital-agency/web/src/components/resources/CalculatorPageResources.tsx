/**
 * Additive resource-gate island for Agency Founder Finance calculator pages.
 *
 ...
 * pages are unchanged until a resource is onboarded. Topics with no asset:
 *   - take-home-pay-calculator / employer-ni-calculator -> structure (no asset)
 *   - rd-tax-credit-estimator -> rnd (no asset, honesty boundary)
 *
 * Mount once in src/app/calculators/[slug]/page.tsx after <CalculatorClient slug={slug}>.
 * aff uses a single dynamic [slug] route (no per-slug static pages), so this is
 * a single wiring point.
 *
 * TOKEN HARDENING: no var(--gold), no var(--navy), no var(--dark), no var(--primary).
 * Label eyebrow chip uses bg-[var(--accent)] text-white.
 */
