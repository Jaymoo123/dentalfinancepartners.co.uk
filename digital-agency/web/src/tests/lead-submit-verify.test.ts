 ...
 * (2026-08-19 estate lead-stack port to Agency Founder Finance): real-time
 * verification written to lead_verification + verify_pass/verify_fail event,
 * resource-download skip, fail-open on verifier errors, and enrolment wiring.
 *
...
vi.mock("@/config/lead-nurture", () => ({
  routePrimarySequence: () => "agency_contactability",
}));
...
const VERIFY_OK = {
  ...
  email: { status: "deliverable", domain: "northlightcreative.co.uk" },
  verify_pass: true,
  provider: "twilio",
  raw: {},
};
...
const BASE_BODY = {
  ...
  email: "jane@northlightcreative.co.uk",
  phone: "+447700900123",
  role: "Marketing or digital agency",
  message:
    "We are ten people, mostly on retainers, and I want to know whether our current setup still makes sense before we take on two more hires.",
  source: "agency",
};
...
describe("submit tail: verification", () => {
  ...
  it("enrols onto the agency contactability sequence", async () => {
    await POST(submitReq(BASE_BODY));
    const [lead, opts] = mockEnrollLead.mock.calls[0];
    expect(lead).toMatchObject({ id: LEAD_ID, source: "agency" });
    expect(opts).toMatchObject({ sequenceName: "agency_contactability", live: true });
  });
...
describe("submit tail: resource downloads", () => {
  // Mirrors exactly what components/resources/ResourceGate.tsx posts: an
  // email-only download unlock carrying in-house consent. It must never spend a
  // Twilio Lookup and must never enter a nurture sequence (estate rule).
  it("role='resource' is neither verified nor enrolled, but still succeeds", async () => {
    const res = await POST(
      submitReq({
        ...
        phone: "",
        role: "resource",
        extras: { resource_gate: true, resource_slug: "agency-margin-checklist" },
