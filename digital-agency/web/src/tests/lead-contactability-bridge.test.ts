/**
 * Tests for the pool-intake bridge in promoteIfContactable (2026-08-19 estate
 * lead-stack port to Agency Founder Finance): on promotion the site calls
 * Property's internal /api/leads/pool-intake so the Telegram grade + ping fire
 * from the single estate bot instance. The call is fail-open by contract: a
 * bridge outage must never block promotion or the handoff email.
 *
...
vi.mock("@/lib/resend", () => ({
  ...
  getFromAddress: () => "team@agencyfounderfinance.co.uk",
}));
