...
const GROUPS: Array<{ title: string; events: Array<[string, string]> }> = [
  ...
  { title: "Resources", events: [
    ...
    ["subscribe_view", "Newsletter shown"], ["subscribe_submitted", "Subscribed"],
  ]},
...
export default async function VisitorTimelinePage({
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      <Link href="/admin/analytics" className="text-sm text-indigo-700 underline">All visitors</Link>

      <div className={`mt-3 rounded-xl border p-5 ${isConverted ? "border-indigo-300 bg-indigo-50" : "border-slate-200 bg-white"}`}>
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex shrink-0 flex-col items-end gap-1">
            ...
            {isConverted ? (
              <span className="rounded-full bg-indigo-600 px-3 py-1 text-xs font-bold text-white">CONVERTED</span>
            ) : (
      ...
      {sessionEngagement.length > 0 && (
        <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4">
          <p className="text-sm text-slate-700">
            ...
            {calcs.size > 0 ? ` · ${calcs.size} calculator${calcs.size !== 1 ? "s" : ""}` : ""}
            {isConverted ? " · converted" : ""}
          </p>
          <div className="flex items-end gap-1" title="Engaged time per session">
            {sessionEngagement.map((ms, i) => (
              <div key={i} className="w-2 rounded-t bg-indigo-500" style={{ height: `${Math.max(3, (ms / maxEng) * 32)}px` }} />
            ))}
          </div>
        </div>
      )}
        ...
        <div className="mt-2 flex flex-wrap items-center gap-1.5 text-xs">
          {miniFunnel.map((s, i) => (
            <span key={s.label} className="flex items-center gap-1.5">
              ...
              <span className={`rounded-full px-2.5 py-1 font-semibold ${s.reached ? s.branch ? "bg-sky-100 text-sky-700" : "bg-indigo-100 text-indigo-800" : "bg-slate-100 text-slate-400"}`}>
