  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10">
      <div className="flex items-baseline justify-between">
        ...
        <Link href="/admin/analytics" className="text-sm text-indigo-700 underline">Overview</Link>
      </div>
                ...
                return (
                  <tr key={l.id} className="border-t border-slate-100">
                    ...
                    <td className="px-3 py-2">
                      {l.visitor_id ? (
                        <Link href={`/admin/analytics/visitor/${l.visitor_id}`} className="text-indigo-700 underline">view</Link>
                      ) : (
      ...
      <div className="mt-4 flex items-center justify-between text-sm">
        {pageNum > 0 ? (
          <Link href={`/admin/analytics/leads?country=${country}&page=${pageNum - 1}`} className="text-indigo-700 underline">Newer</Link>
        ) : <span />}
        <span className="text-xs text-slate-500">Page {pageNum + 1}</span>
        {hasNext ? (
          <Link href={`/admin/analytics/leads?country=${country}&page=${pageNum + 1}`} className="text-indigo-700 underline">Older</Link>
        ) : <span />}
      </div>
    </div>
  );
