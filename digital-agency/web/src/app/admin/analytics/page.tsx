/**
 * Agency analytics console — main dashboard.
 *
 ...
 * console routes.
 *
 * Agency-specific system state:
 *   - No A/B experiments operated: experiments tab shows explicit "not operated"
 *   - Newsletter surface IS present (unlike Medical): nurture panel notes Phase 5 play
 *   - No lead-intent enrichment yet: intent panel shows explicit "not operated"
 *   - No personalisation: offer panel shows explicit "not operated"
 *   - Calculator fleet: 8 tools (agency-specific fleet, deployed 2026-06-11)
 *
          ...
          rows.map(([label, n]) => (
            <div key={label}>
              ...
              <div className="mt-1 h-1.5 rounded bg-slate-100">
                <div className="h-1.5 rounded bg-indigo-500" style={{ width: `${(n / total) * 100}%` }} />
              </div>
            </div>
          ))
        )}
...
function CalcStepBar({ rate, kind }: { rate: number | null; kind: "compute" | "lead" }) {
  ...
  return (
    <div>
      ...
      <div className="mt-1 h-1.5 rounded bg-slate-100">
        <div className={`h-1.5 rounded ${isWeak ? "bg-rose-400" : "bg-indigo-500"}`} style={{ width: `${widthPct}%` }} />
      </div>
    </div>
  );
}
...
function CalculatorsPanel({
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-xs text-slate-500">Tool funnel: Viewed to Computed to Lead. 8-tool fleet deployed 2026-06-11.</p>
      ...
      {rows.length === 0 ? (
        <p className="mt-3 text-sm text-slate-400">No calculator data yet. Tools were deployed 2026-06-11 and are accruing sessions.</p>
      ) : (
...
function CtaPerformancePanel({ rows }: { rows: CtaPerformance[] }) {
    ...
    return (
      <tr key={c.cta_id} className={`border-t border-slate-100 ${weak ? "bg-rose-50/50" : ""}`}>
        ...
        <td className="w-28 px-3 py-2">
          ...
          <div className="mt-1 h-1.5 rounded bg-slate-100">
            <div className={`h-1.5 rounded ${weak ? "bg-rose-400" : "bg-indigo-500"}`} style={{ width: `${widthPct}%` }} />
          </div>
        </td>
  ...
  return (
    <div>
      ...
      <div className="mt-3 overflow-x-auto rounded-xl border border-slate-200 bg-white">
        <div className="border-b border-slate-100 bg-indigo-50/40 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wider text-indigo-800">Form-bound CTAs</div>
                    ...
                    <td className="w-1/3 px-3 py-2 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <div className="hidden h-1.5 w-24 rounded bg-slate-100 sm:block">
                          <div className={`h-1.5 rounded ${flood ? "bg-amber-400" : "bg-indigo-500"}`} style={{ width: `${widthPct}%` }} />
                        </div>
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              rows.map((r) => {
                ...
                return (
                  <tr key={r.visits_bucket} className={`border-t border-slate-100 ${r.converted_visitors > 0 ? "bg-indigo-50/40" : ""}`}>
...
export default async function AdminAnalyticsPage({
  ...
  const overviewSection = (
    <div>
                ...
                <tr key={r.label} className={`border-b border-slate-100 last:border-0 ${r.branch ? "bg-slate-50/60" : ""}`}>
                  ...
                  <td className="hidden w-1/3 px-4 py-2.5 sm:table-cell">
                    <div className="h-2 rounded bg-slate-100">
                      <div className={`h-2 rounded ${r.branch ? "bg-sky-400" : "bg-indigo-500"}`} style={{ width: `${barPct}%` }} />
                    </div>
                  </td>
      ...
      <div className="mt-8 flex flex-wrap items-baseline justify-between gap-2">
        ...
        <Link href="/admin/analytics/trends" className="text-xs text-indigo-700 underline">View trends</Link>
      </div>
  ...
  // Agency does not operate A/B experiments or personalisation yet.
  const experimentsSection = (
    <div className="space-y-6">
      <NotOperatedPanel
        feature="A/B experiments"
        reason="Not operated on this site. The experiments engine requires a running experimentation registry. Agency does not currently run A/B tests."
      />
      <NotOperatedPanel
        feature="Personalisation"
        reason="Not operated on this site. Behavioural personalisation (intent-matched offers) requires the personalisation engine. Agency does not currently run it."
      />
    </div>
  );
  ...
  const conversionSection = (
    <div className="space-y-8">
      <NotOperatedPanel
        ...
        reason="Not operated on this site. The lead-intent classifier (Opus enrichment + vw_lead_intent_mix) has not been enabled for Agency. This is a Phase 5 / Phase 8 play."
      />
      <NotOperatedPanel
      ...
      <div>
        <Link href="/admin/analytics/leads" className="inline-block text-xs text-indigo-700 underline">View all leads</Link>
      </div>
    </div>
  );
    ...
    <div className="mx-auto max-w-6xl px-3 py-6 sm:px-4 sm:py-10">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs sm:gap-4">
          ...
          <Link href="/admin/analytics/trends" className="text-indigo-700 underline">Trends</Link>
          <Link href="/admin/analytics/leads" className="text-indigo-700 underline">All leads</Link>
