/**
 * "Complete your details" endpoint for Agency Founder Finance.
 *
 ...
 * (Agency Founder Finance has no Twilio Lookup in the submit path), so accept saved phone
...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error("[leads/complete/agency] lead lookup failed", e);
  ...
  if (patchKeys.length > 0) {
    try {
      ...
      console.error("[leads/complete/agency] lead update failed", e);
      return NextResponse.json({ success: false, error: "server" }, { status: 500 });
    }
  }
  ...
  try {
    ...
    console.error("[leads/complete/agency] audit event failed (non-fatal)", e);
  }
  ...
  try {
    ...
    console.error("[leads/complete/agency] confirmed event failed (non-fatal)", e);
  }
  try {
    await adminUpdate(
      "lead_nurture_state",
    ...
    console.error("[leads/complete/agency] nurture state promote failed (non-fatal)", e);
  }
  try {
    await adminUpdate(
      "leads",
    ...
    console.error("[leads/complete/agency] lead status promote failed (non-fatal)", e);
  }
