/**
 * Server chokepoint for Agency Founder Finance lead submission.
 *
 ...
 *
 * Not to be confused with /api/nurture/* , which is the separate opt-in
 * newsletter engine. This route only ever handles enquiry submissions.
 */
...
const sharedHandler = createLeadSubmitHandler({ source: "agency" });
...
export async function POST(req: NextRequest) {
  ...
  const source = String(body.source ?? "agency").toLowerCase();
  ...
  if (!isResource && !json.merged) {
    try {
      ...
      console.error("[leads/submit/agency] enrol/instant-touch failed", e);
    }
  }
