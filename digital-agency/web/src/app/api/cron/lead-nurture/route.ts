/**
 * Agency Founder Finance lead-nurture cron: fires due follow-up steps for leads that have
...
import { buildAgencyLeadNurtureConfigs, buildLeadMessageContext } from "@/config/lead-nurture";
...
async function run(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.error("[lead-nurture-cron/agency] heartbeat write failed", err);
  }

  const configs = buildAgencyLeadNurtureConfigs();
  ...
  if (cronArmed) {
    try {
      ...
      console.error("[lead-nurture-cron/agency] guardrail scan failed", err);
    }
  }
