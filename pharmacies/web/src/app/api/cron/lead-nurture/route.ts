/**
 * Pharmacies lead-nurture cron: fires due follow-up steps for leads that have
...
import { buildPharmaciesLeadNurtureConfigs, buildLeadMessageContext } from "@/config/lead-nurture";
...
async function run(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.error("[lead-nurture-cron/pharmacies] heartbeat write failed", err);
  }

  const configs = buildPharmaciesLeadNurtureConfigs();
  ...
  if (cronArmed) {
    try {
      ...
      console.error("[lead-nurture-cron/pharmacies] guardrail scan failed", err);
    }
  }
