/**
 * Server chokepoint for Pharmacy Tax lead submission.
 *
...
const sharedHandler = createLeadSubmitHandler({ source: "pharmacies" });
...
export async function POST(req: NextRequest) {
  ...
  const source = String(body.source ?? "pharmacies").toLowerCase();
  ...
  if (!isResource) {
    try {
      ...
      console.error("[leads/submit/pharmacies] enrol/instant-touch failed", e);
    }
  }
