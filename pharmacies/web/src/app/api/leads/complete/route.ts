/**
 * "Complete your details" endpoint for Pharmacy Tax.
 *
 ...
 * (Pharmacies has no Twilio Lookup in the submit path), so accept saved phone
...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error("[leads/complete/pharmacies] lead lookup failed", e);
  ...
  if (patchKeys.length > 0) {
    try {
      ...
      console.error("[leads/complete/pharmacies] lead update failed", e);
      return NextResponse.json({ success: false, error: "server" }, { status: 500 });
    }
  }
  ...
  try {
    ...
    console.error("[leads/complete/pharmacies] audit event failed (non-fatal)", e);
  }
  ...
  try {
    ...
    console.error("[leads/complete/pharmacies] confirmed event failed (non-fatal)", e);
  }
  try {
    await adminUpdate(
      "lead_nurture_state",
    ...
    console.error("[leads/complete/pharmacies] nurture state promote failed (non-fatal)", e);
  }
  try {
    await adminUpdate(
      "leads",
    ...
    console.error("[leads/complete/pharmacies] lead status promote failed (non-fatal)", e);
  }
