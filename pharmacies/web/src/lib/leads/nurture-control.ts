/**
 * Kill-switch control library for the Pharmacies lead-nurture system.
 *
 * Reads and writes the lead_nurture_control table keyed on site_key='pharmacies'
...
/**
 * Read the pharmacies control row (site_key='pharmacies'). FAIL-OPEN: returns the
...
export async function getNurtureControl(): Promise<NurtureControl> {
  try {
    const res = await adminSelect<DbRow>("lead_nurture_control", {
      ...
      site_key: "eq.pharmacies",
      limit: "1",
    });
    if (!res.ok || res.data.length === 0) return DEFAULT_CONTROL;
    return toControl(res.data[0]);
  } catch (err) {
    console.error("[nurture-control/pharmacies] getNurtureControl error, failing open", err);
...
export async function pauseNurture(reason: string, by: string): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("[nurture-control/pharmacies] pauseNurture write failed", res.status);
    }
  } catch (err) {
    console.error("[nurture-control/pharmacies] pauseNurture error", err);
  }
}
...
export async function resumeNurture(by: string): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("[nurture-control/pharmacies] resumeNurture write failed", res.status);
    }
  } catch (err) {
    console.error("[nurture-control/pharmacies] resumeNurture error", err);
  }
}
...
export async function recordGuardrailAlert(key: string): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("[nurture-control/pharmacies] recordGuardrailAlert write failed", res.status);
    }
  } catch (err) {
    console.error("[nurture-control/pharmacies] recordGuardrailAlert error", err);
  }
}
...
export async function recordCronHeartbeat(): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("[nurture-control/pharmacies] recordCronHeartbeat write failed", res.status);
    }
  } catch (err) {
    console.error("[nurture-control/pharmacies] recordCronHeartbeat error", err);
  }
}
...
export async function recordDigestHeartbeat(): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("[nurture-control/pharmacies] recordDigestHeartbeat write failed", res.status);
    }
  } catch (err) {
    console.error("[nurture-control/pharmacies] recordDigestHeartbeat error", err);
  }
}
