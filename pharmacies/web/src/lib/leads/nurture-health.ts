/**
 * Observability health reader and guardrail evaluator for the Pharmacies
 * lead-nurture system.
 *
 * Mirrors Property/web/src/lib/leads/nurture-health.ts exactly, with
 * 'property' literals replaced by 'pharmacies' and the Pharmacy Tax
...
export async function runNurtureGuardrails(opts: {
  ...
  const health = await getNurtureHealth("pharmacies");
  if (!health) {
  ...
  if (!alreadyAlerted && process.env.RESEND_API_KEY) {
    ...
    try {
      ...
      console.error("[nurture-health/pharmacies] guardrail alert send failed", err);
    }
  }
  ...
  if (opts.autopauseEnabled && verdict.shouldPause && !control.paused) {
    try {
      ...
      console.error("[nurture-health/pharmacies] auto-pause failed", err);
    }
  }
