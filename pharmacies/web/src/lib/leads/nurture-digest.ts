/**
 * Daily digest composer and runner for the Pharmacies lead-nurture system.
 *
 * Mirrors Property/web/src/lib/leads/nurture-digest.ts with:
 *   - gatherDigestData('pharmacies')
 *   - Footer: "Pharmacy Tax"
 *   - site_key filter on stuck-leads query
 *   - JS-filter on failed-send events: leads.source === 'pharmacies'
 *
 * CONVENTIONS
...
export async function gatherDigestData(siteKey: string): Promise<DigestData> {
  ...
  const [health, control, stuckRes, failedRes] = await Promise.all([
    ...
    adminSelect<StuckViewRow>("vw_lead_nurture_stuck", {
      ...
      // ponytail: filter stuck leads to pharmacies site via join on leads table;
  ...
  if (failedEvents.length > 0) {
    ...
    // JS-filter: only include events whose lead belongs to pharmacies.
    failedSends = failedEvents
      .filter((e) => nameMap.get(e.lead_id)?.source === "pharmacies")
      .map((e) => {
...
export function composeDigestEmail(d: DigestData): {
  ...
  lines.push(`Automated digest from the Pharmacy Tax lead-nurture system.`);
...
export async function runNurtureDigest(): Promise<{ sent: boolean }> {
  const d = await gatherDigestData("pharmacies");
