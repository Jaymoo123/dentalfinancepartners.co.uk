 ...
 *
 * House style: no em-dashes. British English. Pharmacy Tax brand.
 */
...
export function buildIcsForSlot(params: IcsSlotParams): string {
  ...
  const uid     = `${params.leadId}-${params.date}-${params.windowKey}@pharmacytax.co.uk`;
  const summary = `Pharmacy finance review call (${params.label})`;
  const desc    = "A pharmacy finance specialist will call you in this window. Nothing to prepare.";

  return [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Pharmacy Tax//Lead Review//EN",
    "BEGIN:VEVENT",
    `UID:${uid}`,
...
export async function runLeadAuxScans(): Promise<{ reminders: number; nudges: number }> {
  ...
  try {
          ...
          if (sendRowId) {
            try {
              ...
              const { html, text } = renderLeadServiceEmail({
                ...
                paragraphs: [
                  `Your free pharmacy finance review call is tomorrow, ${windowPhrase}.`,
                  "One of our pharmacy finance specialists will ring you then. They will have read your enquiry before they call, the call takes about 20 minutes, and there is nothing to prepare.",
              ...
              await markSendStatus(sendRowId, "failed");
              console.error("[lead-aux-cron/pharmacies] T-24 email error", err);
            }
          }
        }
        ...
        if (!isMorning) {
          ...
          if (inT2 && inSendWindow(now, true)) {
            ...
            if (sendRowId && lead.phone) {
              try {
                const body =
                  `Hi ${firstName}, your free pharmacy finance review call is later today, ${label}. ` +
                  `Your pharmacy finance specialist will call you then. Need a different time? Just reply and we will rearrange. Reply STOP to opt out.`;
                ...
                await markSendStatus(sendRowId, "failed");
                console.error("[lead-aux-cron/pharmacies] T-2 SMS error", err);
              }
        ...
        if (eligibleIds.length > 0 && inSendWindow(now, true)) {
          ...
          for (const lead of nudgeLeadsRes.data) {
            ...
            try {
              ...
              await markSendStatus(sendRowId, "failed");
              console.error("[lead-aux-cron/pharmacies] abandoned nudge error", err);
            }
          }
        }
      }
    ...
    console.error("[lead-aux-cron/pharmacies] unexpected top-level error", err);
  }
