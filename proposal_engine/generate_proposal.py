...
def main():
    ...
    html = Template(open(os.path.join(HERE, "template.html"), encoding="utf-8").read()).safe_substitute(
        ...
        price_all=PROSPECT.get("price_all_leads", ""), price_top=PROSPECT.get("price_top_tiers", ""),
        select_window=PROSPECT.get("select_window_days", ""),
        price_medium=PROSPECT.get("price_medium", 40), price_high=PROSPECT.get("price_high", 85),
        price_very_high=PROSPECT.get("price_very_high", 150),
        offer_expiry_hours=PROSPECT.get("offer_expiry_hours", 24),
