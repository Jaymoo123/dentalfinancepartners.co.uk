/**
 * Types for the premium calculator tier on the Dentists site.
 *
 * Ported verbatim from Solicitors/web/src/lib/tools/premium/types.ts and adapted
 * for the Dentists site, which has no radix/recharts/shadcn stack. The type shapes
 * are identical to the Solicitors wave-1 types; only the UI rendering differs.
 *
 ...
 * already available across the Dentists site.
 */
import type {
  CalcField,
  CalcValues,
...
/* ---------------------------------------------------------------------------
 * Editable mini-grid (not used in R2 but present for type parity with Solicitors)
 * ------------------------------------------------------------------------- */
...
export interface ChartSeries {
   ...
   * Use the Dentists token system: var(--gold), var(--navy), var(--accent).
   * Never use var(--primary) -- Dentists does not define --primary.
   */
  color: string;
}
