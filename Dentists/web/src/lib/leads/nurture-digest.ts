/**
 * Daily digest composer and runner for the Dentists lead-nurture system.
 *
 * Mirrors Property/web/src/lib/leads/nurture-digest.ts with:
 *   - gatherDigestData('dentists') and resolveLeadTo('dentists')
 *   - Footer: "Dental Finance Partners"
 *   - site_key filter on stuck-leads query
 *   - JS-filter on failed-send events: leads.source === 'dentists'
 *
 * CONVENTIONS
 *   No em/en dashes in any operator-facing text.
...
export async function gatherDigestData(siteKey: string): Promise<DigestData> {
  ...
  const [health, control, stuckRes, failedRes] = await Promise.all([
    ...
    adminSelect<StuckViewRow>("vw_lead_nurture_stuck", {
      ...
      // ponytail: filter stuck leads to dentists site via join on leads table;
      // if the view exposes site_key directly, add site_key: `eq.${siteKey}` here.
  ...
  if (failedEvents.length > 0) {
    ...
    // JS-filter: only include events whose lead belongs to dentists.
    failedSends = failedEvents
      .filter((e) => nameMap.get(e.lead_id)?.source === "dentists")
      .map((e) => {
...
export function composeDigestEmail(d: DigestData): {
  ...
  if (d.health) {
    ...
    if (h.complaints24h >= 1) {
      notes.push(`${h.complaints24h} complaint(s) in 24 h. Review content and unsubscribe flow.`);
    }
    if (h.optouts7d >= 3) {
      notes.push(
        `${h.optouts7d} opt-out(s) in 7 days. Review sending frequency and message relevance.`,
      );
    }
  ...
  lines.push(`Automated digest from the Dental Finance Partners lead-nurture system.`);
...
export async function runNurtureDigest(): Promise<{ sent: boolean }> {
  const d = await gatherDigestData("dentists");
  ...
  try {
    await getResend().emails.send({
      from: getFromAddress(),
      to: resolveLeadTo("dentists"),
      subject,
      text,
    });
    return { sent: true };
  } catch (err) {
    console.error("[nurture-digest/dentists] digest email send failed", err);
    return { sent: false };
  }
}
