/**
 * Observability health reader and guardrail evaluator for the Dentists
 * lead-nurture system.
 *
 * Mirrors Property/web/src/lib/leads/nurture-health.ts exactly, with
 * 'property' literals replaced by 'dentists' and the Dental Finance Partners
...
export async function runNurtureGuardrails(opts: {
  ...
  const health = await getNurtureHealth("dentists");
  if (!health) {
  ...
  if (!alreadyAlerted && process.env.RESEND_API_KEY) {
    ...
    try {
      ...
      console.error("[nurture-health/dentists] guardrail alert send failed", err);
    }
  }
  ...
  if (opts.autopauseEnabled && verdict.shouldPause && !control.paused) {
    try {
      ...
      console.error("[nurture-health/dentists] auto-pause failed", err);
    }
  }
