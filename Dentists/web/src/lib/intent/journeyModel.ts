/**
 * Journey model for Dental Finance Partners (Dentists site) — deterministic,
 * client-side, instant. Ported from Property/generalist with the `dfp` storage
 * prefix.
 *
 ...
 *
 * Storage key: `dfp_journey` (site prefix `dfp`, never `hd_`/`ptp_`).
 * Special-page flags: /about, /services, /contact — all three routes exist on
 * the Dentists site (confirmed), so all flags are kept (no drops).
 */
...
export function getJourneyProfile(): JourneyProfile {
  ...
  // Stage ladder (verbatim from Property):
