/**
 * Dentists lead-nurture cron: fires due follow-up steps for leads that have
...
import { buildDentistsLeadNurtureConfigs, buildLeadMessageContext } from "@/config/lead-nurture";
...
async function run(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.error("[lead-nurture-cron/dentists] heartbeat write failed", err);
  }

  const configs = buildDentistsLeadNurtureConfigs();
  ...
  if (cronArmed) {
    try {
      ...
      console.error("[lead-nurture-cron/dentists] guardrail scan failed", err);
    }
  }
