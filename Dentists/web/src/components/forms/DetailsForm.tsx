 ...
 *
 * House style: navy/gold tokens, no em-dashes in copy.
 */
...
const inputClass =
  "mt-1 w-full min-h-12 touch-manipulation rounded-lg border-2 border-[var(--navy)]/20 bg-white px-3.5 py-3 text-base text-[var(--navy)] shadow-sm focus:border-[var(--gold)] focus:outline-none focus:ring-2 focus:ring-[var(--gold)]/25 transition-colors";
...
export default function DetailsForm({
  ...
  if (status === "success") {
    return (
      <div className="border-2 border-[var(--gold)] bg-[var(--gold-soft)] p-6 text-center">
        <p className="text-lg font-bold text-[var(--navy)]">Thank you, that is everything we need</p>
        <p className="mt-2 text-base text-[var(--navy)]/70">
          A dental accountant will be in touch shortly. If you would like to pick a time that suits
  ...
  if (status === "partial") {
    return (
      <div className="border-2 border-[var(--gold)] bg-[var(--gold-soft)] p-6 text-center">
        <p className="text-lg font-bold text-[var(--navy)]">Thank you</p>
        <p className="mt-2 text-base text-[var(--navy)]/70">
  ...
  return (
    <form onSubmit={onSubmit} className="text-left" noValidate aria-busy={status === "loading"}>
      ...
      {needsName && (
        <div className="mb-4">
          <label htmlFor="complete-name" className="block text-sm font-semibold text-[var(--navy)]">
            Full name
          </label>
          <input
            type="text"
      ...
      {needsPhone && (
        <div className="mb-4">
          <label htmlFor="complete-phone" className="block text-sm font-semibold text-[var(--navy)]">
            Phone
          </label>
          <input
            type="tel"
      ...
      <p className="mt-3 text-xs text-[var(--navy)]/50">
        We only use this to arrange your free dental practice finance review. See our{" "}
        <a
          href="/privacy-policy"
          target="_blank"
          rel="noopener noreferrer"
          className="font-semibold text-[var(--gold-strong)] underline"
        >
          Privacy Policy
        </a>
        .
