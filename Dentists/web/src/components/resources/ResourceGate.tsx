...
/**
 * Email-gated download unlock for Dental Finance Partners.
 *
 ...
 *
 * Submit: goes through submitDentistLead with extras { resource_gate: true }
 * so the server chokepoint can exclude these rows from the partner-CC notify path.
 *
 * Honeypot: enquiry_ref. Value is passed through to submitDentistLead; do NOT
 * early-return on a filled honeypot (F4). The server stores the row flagged and
 * returns success so a real human caught by browser autofill is never silently
 * dropped (estate-standard fix, Solicitors posture).
 *
...
// Dentists token-safe input class (no --primary, no orange-*)
const inputClass =
  "mt-1 w-full min-h-11 touch-manipulation rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3.5 py-2.5 text-base text-[var(--ink)] placeholder:text-[var(--muted)] focus:border-[var(--gold)] focus:outline-none focus:ring-2 focus:ring-[var(--gold)]/25 transition-colors";
...
export function ResourceGate({
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Pass through to submitDentistLead so the server stores the row flagged.
    ...
    const result = await submitDentistLead(
      {
        full_name: "",
        email,
        phone: "",
    ...
    if (typeof window !== "undefined" && "gtag" in window) {
      ...
      if (gtag) {
        gtag("event", "generate_lead", {
          ...
          event_label: `dentists_resource_${topic}`,
          value: 1,
        });
      }
    }
      ...
      <div className={["grid", split ? "@3xl:grid-cols-[1.1fr_0.9fr]" : ""].join(" ").trim()}>
        ...
        >
          {status === "success" ? (
            <div
              role="status"
              className="rounded-xl border border-[var(--gold-soft)] bg-[var(--surface-elevated)] p-5"
            >
              ...
              <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                ...
                {guideReady && resource?.guide && (
                  <a
                    ...
                    className="inline-flex items-center justify-center rounded-full border-2 border-[var(--gold)] px-5 py-3 text-base font-semibold text-[var(--navy)] transition-colors hover:bg-[var(--gold-soft)]"
                  >
