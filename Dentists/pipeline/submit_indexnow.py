...
Dental Finance Partners site.

Usage:
  # Submit a single URL or several inline:
  python Dentists/pipeline/submit_indexnow.py https://www.dentalfinancepartners.co.uk/blog/some-post

  # Submit everything in the live sitemap (use sparingly, e.g. after big content drops):
  python Dentists/pipeline/submit_indexnow.py --from-sitemap

  # Submit URLs listed in a text file (one per line):
  python Dentists/pipeline/submit_indexnow.py --from-file urls.txt

  # Drain the pending-publish queue (URLs added by generators after each post):
  python Dentists/pipeline/submit_indexnow.py --from-queue

  # Add a URL to the queue without submitting (used by generators):
  python Dentists/pipeline/submit_indexnow.py --enqueue https://www.dentalfinancepartners.co.uk/blog/foo
