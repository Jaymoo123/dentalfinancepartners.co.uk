...
sys.path.insert(0, str(ROOT / 'Solicitors' / 'pipeline'))
...
# Alias for code that references DENTAL_ANCHOR_TERMS in the original file
DENTAL_ANCHOR_TERMS = SOLICITOR_ANCHOR_TERMS

DEEPSEEK_API_KEY = os.getenv('DEEPSEEK_API_KEY')
TAXONOMY_CSV = ROOT / 'Solicitors' / 'pipeline' / 'topic_taxonomy.csv'
...
# ---------------------------------------------------------------------------
# Hallucination catchers — solicitor-specific
# ---------------------------------------------------------------------------
HALLUCINATION_PATTERNS = [
    ...
    (r'\bP11D\b.{0,80}(?:employer|company|firm|partnership)\s*pension',
     'wrong: employer pension contributions are NOT P11D items'),
    (r'(?:employer|company|firm|partnership)\s*pension.{0,80}\bP11D\b',
     'wrong: employer pension contributions are NOT P11D / BIK items'),
    (r'\bAIA\b.{0,40}\bcars?\b',
     'wrong: AIA does not cover cars'),
    (r'LLP\s*(?:pays|paying)\s*corporation\s*tax',
     'wrong: LLPs are tax-transparent; members taxed personally'),
    (r'(?:LLP|limited liability partnership)\s*(?:is|are)\s*subject\s*to\s*corporation\s*tax',
     'wrong: LLPs are tax-transparent for income tax'),
    (r'partnership.{0,40}(?:pays|liable for|subject to)\s*corporation\s*tax',
     'wrong: partnerships do not pay corporation tax; members taxed personally'),
    (r'SRA\s*Accounts?\s*Rules?.{0,40}(?:require|mandate).{0,40}monthly\s*reconcil',
     'wrong: SRA Accounts Rules require reconciliation at least every five weeks (Rule 8.3), not monthly'),
    (r'conveyancing\s*(?:fees?\s*)?(?:is|are)\s*VAT.?exempt',
     'wrong: conveyancing legal services are standard-rated for VAT (20%)'),
    (r'(?:legal services|solicitor.?s?\s*fees?)\s*(?:is|are)\s*VAT.?exempt',
     'wrong: legal services including conveyancing are standard-rated for VAT'),
    ...
    (r'(?:selling|sale\s+of)\s+(?:your\s+|a\s+)?(?:law\s+firm|practice).{0,40}(?:trading\s+income|income\s+tax|self.assessment\s+rate)',
     'wrong: firm sale is a capital disposal, not trading income'),
    (r'(?:all\s+|every\s+)?salaried\s*partners?\s*(?:are\s+)?taxed\s*as\s*partners',
     'wrong: Salaried Member Rules (FA 2014) can deem LLP salaried members as employees for tax'),
    (r'CGT\s*(?:rate)?\s*(?:of|at)\s*28%',
     'outdated: residential CGT higher rate is 24% from 30 October 2024'),
    (r'PII\s*(?:premiums?\s*)?(?:is\s*|are\s*)?(?:not|non).?(?:deductible|tax.?deductible)',
     'wrong: PII premiums are allowable trade expenses'),
]
...
def validate_pre_write(fields: dict, slug: str, anchor_terms: list[str]) -> tuple[bool, list[str]]:
    ...
    if not any(a in h1_low for a in anchor_terms):
        errors.append(f'H1 missing legal anchor (need one of: {", ".join(anchor_terms[:5])}...)')

    # First 200 words must contain >= 2 distinct legal terms
    ...
    if anchors_found < 2:
        errors.append(f'first 200 words contain only {anchors_found} legal anchor(s), need >=2')

    # At least one H2 contains a legal term
    ...
    if not h2_anchored:
        errors.append('no H2 contains a legal anchor term')
