/**
 * Daily digest composer and runner for the Solicitors (Accounts for Lawyers)
 * lead-nurture system.
 *
 ...
 * site_key='solicitors' / source='solicitors'.
 *
...
export async function gatherDigestData(siteKey: string): Promise<DigestData> {
  ...
  // Enrich failed sends with lead names; filter to solicitors source.
  ...
  if (failedEvents.length > 0) {
    ...
    // ponytail: JS-filter failed-send events by leads.source==='solicitors' (vw_lead_nurture_stuck
    ...
    failedSends = failedEvents
      .filter((e) => {
        ...
        return entry?.source === "solicitors";
      })
      .map((e) => {
...
export function composeDigestEmail(d: DigestData): {
  ...
  lines.push(`Automated digest from the Accounts for Lawyers lead-nurture system.`);
...
export async function runNurtureDigest(): Promise<{ sent: boolean }> {
  const d = await gatherDigestData("solicitors");
