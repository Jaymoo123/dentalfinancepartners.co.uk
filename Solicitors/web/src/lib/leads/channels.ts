/**
 * Solicitors (Accounts for Lawyers) lead-nurture channel sender: implements the
 * engine's ChannelSender over Resend (email) and Twilio REST (SMS + WhatsApp).
 *
...
function serviceFrom(): string {
  const name = process.env.LEAD_SERVICE_FROM_NAME || "Accounts for Lawyers";
