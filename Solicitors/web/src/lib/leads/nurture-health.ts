/**
 * Observability health reader and guardrail evaluator for the Solicitors
 * (Accounts for Lawyers) lead-nurture system.
 *
 * getNurtureHealth: reads vw_lead_nurture_health (service role, scoped to
 * site_key='solicitors') and maps the result to NurtureHealth. Returns null if
 * the view is absent, the site has no row yet, or any error occurs. A missing
 * view must never block sends (fail-open).
 *
...
export async function runNurtureGuardrails(opts: {
  ...
  const health = await getNurtureHealth("solicitors");
  if (!health) {
