/**
 * Auxiliary scheduled sends for Accounts for Lawyers: booked-slot reminders
 * and abandoned-booking nudges.
...
export function buildIcsForSlot(params: IcsSlotParams): string {
  ...
  const uid     = `${params.leadId}-${params.date}-${params.windowKey}@accountsforlawyers.co.uk`;
  const summary = `Accounts for Lawyers review call (${params.label})`;
  const desc    = "A specialist accountant for solicitors will call you in this window. Nothing to prepare.";

  return [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Accounts for Lawyers//Lead Review//EN",
    "BEGIN:VEVENT",
    `UID:${uid}`,
      ...
      for (const [leadId, meta] of latestByLead) {
        ...
        if (inT24) {
          ...
          if (sendRowId) {
            try {
              const windowPhrase =
                WINDOW_PHRASES[meta.window] ?? `in your booked slot (${label})`;
              const { html, text } = renderLeadServiceEmail({
                preheader: "Booked for tomorrow. Reply if the time no longer works.",
                greeting: `Hi ${firstName},`,
                paragraphs: [
                  `Your free review call with a specialist accountant for solicitors is tomorrow, ${windowPhrase}.`,
        ...
        if (!isMorning) {
          ...
          if (inT2 && inSendWindow(now, true)) {
            ...
            if (sendRowId && lead.phone) {
              try {
                const body =
                  `Hi ${firstName}, your free review call with a specialist accountant for solicitors is later today, ${label}. ` +
