/**
 * Types for the premium calculator tier on the Solicitors site.
 *
 * Ported from Property/web/src/lib/calculators/premium/types.ts and adapted for
 * the Solicitors site, which has no radix/recharts/shadcn stack. The type shapes
 * are identical to Property's; only the UI rendering differs.
 *
 ...
 * already available across the Solicitors site.
 */
import type {
  CalcField,
  CalcValues,
...
/* ---------------------------------------------------------------------------
 * Editable mini-grid (not used in R2 but present for type parity with Property)
 * ------------------------------------------------------------------------- */
...
export interface ChartSeries {
  ...
  label: string;
  /**
   * CSS custom property token name (e.g. "var(--primary)") or a hex colour.
   * Use the Solicitors token system: var(--primary), var(--accent), var(--ink).
   */
  color: string;
}
