...
const sharedHandler = createLeadSubmitHandler({ source: "solicitors" });
...
export async function POST(req: NextRequest) {
  ...
  const source = String(body.source ?? "solicitors").toLowerCase();
  ...
  if (!isResource) {
    try {
      ...
      console.error("[leads/submit/solicitors] enrol/instant-touch failed", e);
    }
  }
