 ...
 *
 * F.6's four deliberate departures are honoured: no `LeadCTAPanel` (the reader
 * has just submitted the form, so the closing ask is the callback slot, not a
 * second enquiry form), no breadcrumb (noindex), a shallow hero with no CTA row,
 * and no tick badge - the old `CheckIcon` is deleted rather than restyled.
 *
 * Grounds on every branch: navy hero, then light. Nothing navy may land on the
 * navy footer, which is why even the two short branches carry a light section.
 *
 ...
 * so `WhatToExpectCard` is not mounted (its items would be net-new text here and
 * its four defaults are Property strings). Single column on `siteContainerLg`.
 *
        ...
        {nurtureArmed ? (
          <>
            ...
            <p className="mt-4 text-base leading-relaxed text-slate-300">
              For specialist tax advisory work, including complex structuring and tax planning,
