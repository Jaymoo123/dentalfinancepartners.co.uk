/**
 * Solicitors analytics console — main dashboard.
 *
 ...
 * console routes.
 *
 * Solicitors-specific system state:
 *   - No A/B experiments operated: experiments tab shows explicit "not operated"
 *   - No nurture/subscriber engine: Solicitors has no newsletter surface (n/a)
...
function CalculatorsPanel({
  ...
  return (
    <div>
      ...
      {rows.length === 0 ? (
        <p className="mt-3 text-sm text-slate-400">No calculator data yet. Tools are accruing sessions.</p>
      ) : (
...
export default async function AdminAnalyticsPage({
  ...
  // Solicitors does not operate A/B experiments, personalisation, or nurture (no newsletter surface).
  const experimentsSection = (
    <div className="space-y-6">
      <NotOperatedPanel
        feature="A/B experiments"
        reason="Not operated on this site. The experiments engine requires a running experimentation registry. Solicitors does not currently run A/B tests."
      />
      <NotOperatedPanel
        feature="Personalisation"
        reason="Not operated on this site. Behavioural personalisation (intent-matched offers) requires the personalisation engine. Solicitors does not currently run it."
      />
    </div>
  );
  ...
  const conversionSection = (
    <div className="space-y-8">
      <NotOperatedPanel
        ...
        reason="Not operated on this site. The lead-intent classifier (Opus enrichment + vw_lead_intent_mix) has not been enabled for Solicitors. This is a Phase 5 / Phase 8 play."
      />
      <NotOperatedPanel
        feature="Nurture engine"
        reason="Not operated on this site. Solicitors has no newsletter surface, so the marketing opt-in and drip nurture engine (Resend sequences) is n/a."
      />
