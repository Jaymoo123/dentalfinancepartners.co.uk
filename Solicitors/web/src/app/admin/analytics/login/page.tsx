...
export default async function LoginPage({
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[var(--surface)]">
      <div className="w-full max-w-sm rounded-xl border border-[var(--border)] bg-white p-8 shadow-sm">
        <h1 className="font-serif text-lg font-semibold text-[var(--ink)]">Analytics console</h1>
        <p className="mt-1 text-sm text-[var(--muted)]">Enter the admin key to access the dashboard.</p>
        ...
        <form action="/api/admin/login" method="POST" className="mt-5 space-y-4">
          <div>
            <label htmlFor="key" className="block text-xs font-semibold uppercase tracking-wider text-[var(--muted)]">
              Admin key
            </label>
            <input
              id="key"
          ...
          <button
            type="submit"
            className="w-full rounded-full bg-[var(--primary)] px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-[var(--primary-soft)]"
          >
            Sign in
          </button>
        </form>
