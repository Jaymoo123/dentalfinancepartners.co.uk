...
/**
 * Dependency-free inline SVG bar chart for the Solicitors premium calculator tier.
 *
 * Renders grouped bars from ChartResult.data + ChartSpec.series using only SVG and
 * the site's CSS variable tokens. Fixed height (180px) so there is no CLS. No
 * recharts, no radix, no new npm dependencies.
 *
 * Bar rendering:
 *   - Groups are spaced evenly across the SVG width.
 *   - Within a group, bars are rendered side by side, one per series.
 *   - A simple tooltip is shown on hover via title elements.
 *   - Values are formatted as GBP currency by default (valueFormat="currency"),
 *     or as raw numbers (valueFormat="number").
 */
import type { ChartSpec, ChartResult } from "@/lib/tools/premium/types";
