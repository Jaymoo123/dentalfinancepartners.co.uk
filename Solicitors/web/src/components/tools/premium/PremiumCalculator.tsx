...
/**
 * Dependency-free premium calculator renderer for the Solicitors site.
 *
 ...
 *
 * The comparison chart (full variant only) is a dependency-free inline SVG bar chart
 * via PremiumBarChart.tsx (~80 lines, no recharts).
 *
 * All styling uses the site's --* CSS variable tokens and btnPrimary from
 * layout-utils. No radix, no recharts, no tailwind-merge, no new npm dependencies.
 *
      ...
      {scenarios.map((s) => (
        ...
        >
          <div className="flex items-start justify-between gap-1">
            ...
            {s.best && (
              <span className="rounded-full bg-[var(--primary)]/10 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-[var(--primary)]">
                Better
              </span>
            )}
          </div>
...
export function PremiumCalculator({
  ...
  return (
    <>
      ...
      >
        {/* Top accent bar using site primary token */}
