...
/**
 * Exit-intent capture. Self-contained: mounted once globally, it arms only on
 * relevant routes (blog + calculator pages), derives its topic from the URL,
 * and triggers on desktop (mouse leaves the top) OR mobile (leave-style scroll
 * back to the top after going deep). Uses the qualified MiniCapture (name +
 * phone + email + message) with a topic-aware offer. No assistant guard.
 * Consent-gated. 30-day per-visit suppress.
 */
...
const STORAGE_KEY = "afl-exit-intent-dismissed-v1";
...
export function ExitIntentModal() {
  ...
  useEffect(() => {
    ...
    if (desktop) {
      ...
      // Mobile: they scrolled deep, then scrolled back up toward the top.
      let maxY = 0;
  ...
  const heading = topic?.ctaCopy || "Get a free review of your position";
  const blurb =
    "Tell us where to reach you and a specialist solicitor accountant will review your situation and suggest the next sensible step, with no obligation.";
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-lg border-l-4 border-[var(--primary)] bg-white p-6 shadow-2xl sm:p-8">
        <button
          type="button"
        ...
        <p className="text-xs font-bold uppercase tracking-wider text-[var(--primary)]">Before you go</p>
