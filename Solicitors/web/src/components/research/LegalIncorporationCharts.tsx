 ...
 *
 * ACCESSIBILITY (the phase-4 PremiumBarChart pattern, copied verbatim in shape).
 * Each chart used to put `aria-hidden="true"` on the wrapper AND a `role="img"`
 * + `aria-label` on the svg inside it. The wrapper removed the whole subtree
 * from the accessibility tree, so the role and the label were dead code and
 * every value was unreachable. Now: the bars stay decorative and stay
 * `aria-hidden`, and the values are published as a real `sr-only` table built
 * from THE SAME formatter the visible labels and the <title> tooltips use, off
 * the same array the bars are drawn from. No number is typed twice and no
 * second data array exists.
 */
...
/** The one count formatter. Feeds <title>, the sr-only table, nothing else. */
function fmtCount(n: number): string {
  return n.toLocaleString("en-GB");
}

/** The one percent formatter, full precision. Feeds <title> and the table. */
function fmtPctFull(n: number): string {
  return n.toFixed(1) + "%";
}
...
export function AnnualIncorporationChart({ annual }: { annual: IncorporationYear[] }) {
  ...
  return (
    <>
      <div style={{ height: CHART_H }} className="w-full overflow-hidden">
        <svg
          ...
          {[0.25, 0.5, 0.75, 1].map((f) => {
            ...
            return (
              <line key={f} x1={PAD.left} y1={y} x2={PLOT_W - PAD.right} y2={y}
                stroke="var(--border)" strokeWidth={1} strokeDasharray="4 3" aria-hidden="true" />
            );
          })}
          {data.map((d, i) => {
            ...
            return (
              <g key={d.year}>
                <rect x={x} y={y} width={barW} height={Math.max(0, barH)}
                  fill="var(--primary)" rx={2} ry={2} opacity={0.85} aria-hidden="true">
                  <title>{d.year}: {fmtCount(d.total)} incorporations</title>
                </rect>
                <text x={x + barW / 2} y={CHART_H - 6} textAnchor="middle"
                  fontSize={LABEL_SIZE} fill="var(--ink-soft)" aria-hidden="true">
                  {d.year}
                </text>
              </g>
            );
      ...
      <table className="sr-only">
        <caption>Annual law firm incorporations</caption>
        <thead>
          <tr>
...
export function SraStructureChart({ series }: { series: SraSnapshot[] }) {
  ...
  // Stacked bar segments in order: incorporated (primary), llp (violet), partnership, sole.
  // The LLP segment stays PURPLE on purpose: the page body calls it "the purple
  // band". violet-700 is a token and measures 7.10 on white; the #7c6ff7 it
  // replaces was the only non-token colour in this file and measured 3.84.
  const segments: { key: keyof SraSnapshot; color: string; label: string }[] = [
    { key: "incorporated", color: "var(--primary)", label: "Incorporated" },
    { key: "llp", color: "#6d28d9", label: "LLP" },
  ...
  return (
    <>
      <div style={{ height: CHART_H + 20 }} className="w-full overflow-hidden">
        <svg
        ...
        >
                ...
                {segments.map((seg) => {
                  ...
                  return (
                    <rect key={seg.key} x={x} y={stackY} width={barW} height={Math.max(0, segH)}
                      fill={seg.color} opacity={0.9} aria-hidden="true">
                      <title>{seg.label}: {fmtCount(val)} ({d.year})</title>
                    </rect>
                  );
                })}
          ...
          {/* Legend */}
          <g transform={`translate(${PAD.left}, ${PAD.top - 10})`} aria-hidden="true">
            {segments.map((seg, i) => (
              <g key={seg.key} transform={`translate(${i * 130}, 0)`}>
                <rect x={0} y={-8} width={10} height={10} fill={seg.color} rx={1} />
                <text x={14} y={0} fontSize={LABEL_SIZE} fill="var(--ink-soft)">{seg.label}</text>
              </g>
            ))}
          </g>
        </svg>
      ...
      <table className="sr-only">
        <caption>SRA-regulated firms by constitution type</caption>
        <thead>
          <tr>
            <th scope="col">Year</th>
            {segments.map((seg) => (
              <th key={seg.key} scope="col">{seg.label}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {data.map((d) => (
            <tr key={d.year}>
              <th scope="row">{d.year}</th>
              {segments.map((seg) => (
                <td key={seg.key}>{fmtCount(Number(d[seg.key] ?? 0))}</td>
              ))}
            </tr>
          ))}
        </tbody>
...
export function SurvivalComparisonChart({
  ...
  const scale = (v: number) => (v / maxVal) * ph;
  const seriesLabels = ["Legal activities (SIC 691)", "All industry"];
      ...
      <div style={{ height: CHART_H }} className="w-full overflow-hidden">
        ...
        >
            ...
            return (
              <g key={label} aria-hidden="true">
                {legalVal !== null && (
                  <g>
                    <rect x={groupX} y={PAD.top + ph - scale(legalVal)} width={barW}
                      ...
                      <title>Legal activities {label}: {fmtPctFull(legalVal)}</title>
                    </rect>
                    {/* Visible label is deliberately rounded to whole percent; the
                        table and the tooltip carry fmtPctFull. Same number, two
                        precisions, one formatter each. */}
                    <text x={groupX + barW / 2} y={PAD.top + ph - scale(legalVal) - 3}
                      textAnchor="middle" fontSize={LABEL_SIZE} fill="var(--ink)">
                      {legalVal.toFixed(0)}%
                    </text>
                  </g>
                )}
                {allVal !== null && (
                  <g>
                    <rect x={groupX + barW + barGap} y={PAD.top + ph - scale(allVal)} width={barW}
                      ...
                      <title>All industry {label}: {fmtPctFull(allVal)}</title>
                    </rect>
                ...
                <text x={groupX + groupW / 2} y={CHART_H - 6} textAnchor="middle"
                  fontSize={LABEL_SIZE} fill="var(--ink-soft)">{label}</text>
              </g>
            );
          })}
          ...
          <g transform={`translate(${PAD.left}, ${PAD.top - 10})`} aria-hidden="true">
            ...
            <text x={14} y={0} fontSize={LABEL_SIZE} fill="var(--ink-soft)">{seriesLabels[0]}</text>
            <rect x={190} y={-8} width={10} height={10} fill="#94a3b8" rx={1} />
            <text x={204} y={0} fontSize={LABEL_SIZE} fill="var(--ink-soft)">{seriesLabels[1]}</text>
          </g>
        </svg>
      </div>

      <table className="sr-only">
        <caption>Survival rate comparison</caption>
        <thead>
          <tr>
        ...
        <tbody>
          {labels.map((label, i) => (
            <tr key={label}>
              ...
              <td>{legalPcts[i] === null ? "n/a" : fmtPctFull(legalPcts[i] as number)}</td>
              <td>{allPcts[i] === null ? "n/a" : fmtPctFull(allPcts[i] as number)}</td>
            </tr>
          ))}
        </tbody>
      </table>
