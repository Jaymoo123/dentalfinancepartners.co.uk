...
/**
 * Curated specialist widget for Accounts for Lawyers.
 *
 ...
 * (gated via the intent context). Styled with CSS-variable tokens.
 *
 * Proactive layer (Phase 0, deterministic, no LLM): presents as a prominent
 * card. It auto-opens ONCE per session shortly after landing for immediacy,
 * and otherwise surfaces a badge "peek" with a journey-tailored one-liner that
...
const emailRe = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
const inputClass =
  "mt-1 w-full rounded-lg border-2 border-[var(--border)] bg-white px-3 py-2 text-base text-[var(--ink)] focus:border-[var(--primary)] focus:outline-none focus:ring-2 focus:ring-[var(--primary)]/20 min-h-12 touch-manipulation";
...
const AUTO_OPEN_KEY = "afl_assistant_autoopened";
...
export function SpecialistWidget() {
  ...
  // Suppress the proactive layer for converted visitors (booking-concierge exception
  // if they hold a live booking nudge and haven't picked a slot yet).
  ...
  useEffect(() => {
    ...
    initJourneyModel();
    try {
      window.sessionStorage.setItem("afl_assistant_active", "1");
    } catch {
      /* ignore */
    }
  }, [active]);
  ...
  return (
    <div className="fixed bottom-24 right-4 z-[55] flex flex-col items-end print:hidden">
      {open && (
        <div
          role="dialog"
          aria-label="Accounts for Lawyers assistant"
        ...
        >
          {/* Agent header */}
          <div className="flex items-center gap-3 bg-[var(--ink)] px-4 py-3 text-white">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[var(--primary)] ring-2 ring-white/15">
          ...
          <div className="flex-1 space-y-3 overflow-y-auto bg-[var(--surface-elevated)] p-4">
            {peekLine && (
              <div className="flex items-start gap-2">
                <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[var(--primary)] text-white">
            ...
            {status === "success" ? (
              <div className="flex items-start gap-2">
                ...
                <div className="max-w-[82%] rounded-2xl rounded-tl-sm border border-[var(--primary)]/20 bg-[var(--primary)]/5 px-3 py-2 text-sm font-medium text-[var(--primary-dark)] shadow-sm">
                  Thanks, a specialist has your message and will be in touch by email. For specialist advisory work we partner with Aswatax, Chartered Tax Advisers, so it may be their team who replies. Please keep an eye on your inbox, including spam or junk, so our reply is not missed.
                </div>
              </div>
            ) : !composing ? (
              <div className="flex flex-wrap gap-2 pl-9">
                {calcSlug && (
                  <a
                    ...
                    className="inline-flex items-center rounded-full border border-[var(--primary)]/30 bg-white px-3 py-3 text-sm font-medium text-[var(--primary)] hover:bg-[var(--primary)]/5"
                  >
                    See your numbers
                  </a>
                )}
          ...
          {!composing && status !== "success" && (
            <div className="border-t border-[var(--border)] bg-white p-3">
              <button
                ...
                className="w-full rounded-lg bg-[var(--primary)] px-4 py-3 text-sm font-semibold text-white hover:bg-[var(--primary-soft)]"
              >
                Ask a specialist
              </button>
            </div>
            ...
            <div className="border-t border-[var(--border)] bg-white p-3">
              ...
              >
                ...
                <button
                  ...
                  className="w-full rounded-lg bg-[var(--primary)] px-4 py-3 text-sm font-semibold text-white hover:bg-[var(--primary-soft)] disabled:opacity-60"
                >
                  {status === "loading" ? "Sending..." : "Send to a specialist"}
                </button>
                <p className="text-[11px] leading-relaxed text-[var(--muted)]">
                  {siteConfig.leadConsentText} See our{" "}
                  <a href="/privacy-policy" target="_blank" rel="noopener noreferrer" className="font-semibold text-[var(--primary)] underline">
                    Privacy Policy
                  </a>
                  .
                </p>
      ...
      {!open && peekVisible && peekLine && (
        <div className="mb-3 flex w-[min(88vw,20rem)] items-start gap-2 rounded-2xl border border-[var(--primary)]/20 bg-white p-3 shadow-2xl">
          <button
            ...
            className="flex-1 text-left text-sm font-medium leading-snug text-[var(--ink)] hover:text-[var(--primary)]"
          >
            {peekLine}
          </button>
          <button
      ...
      <button
        ...
        className="relative flex items-center gap-2 rounded-full bg-[var(--primary)] px-4 py-3 text-sm font-semibold text-white shadow-2xl hover:bg-[var(--primary-soft)]"
      >
        {!open && unread > 0 && (
