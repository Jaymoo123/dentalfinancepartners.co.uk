    ...
    <>
      ...
      <section className="border-b border-neutral-200 bg-[#fafaf7] py-12 sm:py-16 lg:py-20">
        <div className={siteContainerLg}>
          ...
          <div className="mt-10">
            <Link href="/services" className="inline-flex items-center gap-2 text-[#b0532f] hover:opacity-80 font-semibold text-sm sm:text-base transition-opacity">
              View all services
            </Link>
          </div>
        </div>
        ...
        <div className={siteContainerLg}>
          ...
          <div className="grid gap-3 sm:gap-4 grid-cols-2">
            {hospitalityHubs.filter((h) => h.slug !== slug).map((h) => (
              <Link key={h.slug} href={`/for/${h.slug}`} className="group block border border-neutral-200 bg-white p-4 transition-all hover:border-[#b0532f] hover:shadow-sm">
                <span className="text-sm font-semibold text-neutral-800 group-hover:text-[#b0532f] transition-colors">{h.title}</span>
              </Link>
            ))}
          </div>
          <div className="mt-5">
            <Link href="/for" className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#b0532f] hover:opacity-80 transition-opacity">
