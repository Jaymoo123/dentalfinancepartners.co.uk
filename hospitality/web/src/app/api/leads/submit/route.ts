/**
 * Server chokepoint for Hospitality Tax lead submission.
 *
...
const sharedHandler = createLeadSubmitHandler({ source: "hospitality" });
...
export async function POST(req: NextRequest) {
  ...
  const source = String(body.source ?? "hospitality").toLowerCase();
  ...
  if (!isResource && !json.merged) {
    try {
      ...
      console.error("[leads/submit/hospitality] enrol/instant-touch failed", e);
    }
  }
