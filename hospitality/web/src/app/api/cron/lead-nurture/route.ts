/**
 * Hospitality lead-nurture cron: fires due follow-up steps for leads that have
...
import { buildHospitalityLeadNurtureConfigs, buildLeadMessageContext } from "@/config/lead-nurture";
...
async function run(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.error("[lead-nurture-cron/hospitality] heartbeat write failed", err);
  }

  const configs = buildHospitalityLeadNurtureConfigs();
  ...
  if (cronArmed) {
    try {
      ...
      console.error("[lead-nurture-cron/hospitality] guardrail scan failed", err);
    }
  }
