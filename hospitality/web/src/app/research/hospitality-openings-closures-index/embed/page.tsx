/**
 * Embeddable iframe view of the Hospitality Openings & Closures Index.
...
export default function HospitalityIndexEmbed() {
      ...
      <div className="mb-4 flex flex-wrap gap-3">
        <div className="rounded-lg border border-[var(--border)] bg-white px-4 py-3">
          <div className="text-xl font-bold text-[var(--brand-primary)]">
            {fmt(headline.pub_count_proxy.count)}
          </div>
          <div className="text-xs text-[var(--muted)]">active pub/bar companies</div>
        </div>
        <div className="rounded-lg border border-[var(--border)] bg-white px-4 py-3">
          <div className="text-xl font-bold text-[var(--brand-primary)]">
            {fmtNet(latestNet.restaurants_cafes ?? 0)}
          </div>
          <div className="text-xs text-[var(--muted)]">restaurant net openings ({latestQ})</div>
        </div>
        <div className="rounded-lg border border-[var(--border)] bg-white px-4 py-3">
          <div className="text-xl font-bold text-[var(--brand-primary)]">
            {fmtNet(latestNet.pubs_bars ?? 0)}
          </div>
          <div className="text-xs text-[var(--muted)]">pub/bar net openings ({latestQ})</div>
        </div>
      </div>
