/**
 * Daily digest composer and runner for the Hospitality lead-nurture system.
 *
 * Mirrors Property/web/src/lib/leads/nurture-digest.ts with:
 *   - gatherDigestData('hospitality')
 *   - Footer: "Hospitality Tax"
 *   - site_key filter on stuck-leads query
 *   - JS-filter on failed-send events: leads.source === 'hospitality'
 *
 * CONVENTIONS
...
export async function gatherDigestData(siteKey: string): Promise<DigestData> {
  ...
  const [health, control, stuckRes, failedRes] = await Promise.all([
    ...
    adminSelect<StuckViewRow>("vw_lead_nurture_stuck", {
      ...
      // ponytail: filter stuck leads to hospitality site via join on leads table;
  ...
  if (failedEvents.length > 0) {
    ...
    // JS-filter: only include events whose lead belongs to hospitality.
    failedSends = failedEvents
      .filter((e) => nameMap.get(e.lead_id)?.source === "hospitality")
      .map((e) => {
...
export function composeDigestEmail(d: DigestData): {
  ...
  lines.push(`Automated digest from the Hospitality Tax lead-nurture system.`);
...
export async function runNurtureDigest(): Promise<{ sent: boolean }> {
  const d = await gatherDigestData("hospitality");
