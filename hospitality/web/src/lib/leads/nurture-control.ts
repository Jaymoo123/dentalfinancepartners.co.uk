/**
 * Kill-switch control library for the Hospitality lead-nurture system.
 *
 * Reads and writes the lead_nurture_control table keyed on site_key='hospitality'
...
/**
 * Read the hospitality control row (site_key='hospitality'). FAIL-OPEN: returns the
...
export async function getNurtureControl(): Promise<NurtureControl> {
  try {
    const res = await adminSelect<DbRow>("lead_nurture_control", {
      ...
      site_key: "eq.hospitality",
      limit: "1",
    });
    if (!res.ok || res.data.length === 0) return DEFAULT_CONTROL;
    return toControl(res.data[0]);
  } catch (err) {
    console.error("[nurture-control/hospitality] getNurtureControl error, failing open", err);
...
export async function pauseNurture(reason: string, by: string): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("[nurture-control/hospitality] pauseNurture write failed", res.status);
    }
  } catch (err) {
    console.error("[nurture-control/hospitality] pauseNurture error", err);
  }
}
...
export async function resumeNurture(by: string): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("[nurture-control/hospitality] resumeNurture write failed", res.status);
    }
  } catch (err) {
    console.error("[nurture-control/hospitality] resumeNurture error", err);
  }
}
...
export async function recordGuardrailAlert(key: string): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("[nurture-control/hospitality] recordGuardrailAlert write failed", res.status);
    }
  } catch (err) {
    console.error("[nurture-control/hospitality] recordGuardrailAlert error", err);
  }
}
...
export async function recordCronHeartbeat(): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("[nurture-control/hospitality] recordCronHeartbeat write failed", res.status);
    }
  } catch (err) {
    console.error("[nurture-control/hospitality] recordCronHeartbeat error", err);
  }
}
...
export async function recordDigestHeartbeat(): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("[nurture-control/hospitality] recordDigestHeartbeat write failed", res.status);
    }
  } catch (err) {
    console.error("[nurture-control/hospitality] recordDigestHeartbeat error", err);
  }
}
