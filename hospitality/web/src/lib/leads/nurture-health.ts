/**
 * Observability health reader and guardrail evaluator for the Hospitality
 * lead-nurture system.
 *
 * Mirrors Property/web/src/lib/leads/nurture-health.ts exactly, with
 * 'property' literals replaced by 'hospitality' and the Hospitality Tax
...
export async function runNurtureGuardrails(opts: {
  ...
  const health = await getNurtureHealth("hospitality");
  if (!health) {
  ...
  if (!alreadyAlerted && process.env.RESEND_API_KEY) {
    ...
    try {
      ...
      console.error("[nurture-health/hospitality] guardrail alert send failed", err);
    }
  }
  ...
  if (opts.autopauseEnabled && verdict.shouldPause && !control.paused) {
    try {
      ...
      console.error("[nurture-health/hospitality] auto-pause failed", err);
    }
  }
