...
/** The personalisation A/B: control gets the plain generic experience. */
const PERSONALIZATION_EXP = "personalization";

const Ctx = createContext<IntentContext | null>(null);
/** The visitor's personalisation A/B arm ("control" | "treatment" | null). */
...
export function IntentProvider({ children }: { children: React.ReactNode }) {
  ...
  // A/B arm for the personalisation experiment. null until assigned after mount
  // (SSR-safe). "control" => suppress personalisation entirely; "treatment" =>
  // run the behaviour-driven CIS offers.
  const [arm, setArm] = useState<string | null>(null);

  useEffect(() => setMounted(true), []);

  // Assign the visitor to the running personalisation experiment and register
