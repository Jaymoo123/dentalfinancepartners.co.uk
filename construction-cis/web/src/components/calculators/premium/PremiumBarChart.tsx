...
/**
 * Dependency-free inline SVG bar chart for the construction-cis premium calculator tier.
 *
 * Ported from Dentists/web/src/components/tools/premium/PremiumBarChart.tsx.
 * Re-tokened to orange/slate:
 *   var(--accent) = orange (#f97316)
 *   var(--dark)   = slate (#1e293b)
 *   var(--border), var(--ink), var(--ink-soft) are shared design tokens.
 *
 * The site does NOT define --gold or --navy. Any var(--gold) or var(--navy) from
 * the Dentists source has been replaced with var(--accent) / var(--dark).
 *
 * Renders grouped bars from ChartResult.data + ChartSpec.series using only SVG.
 * Fixed height (180px) to prevent CLS. No recharts, no new npm dependencies.
 */
