...
/**
 * Email-gated download unlock for Trade Tax Specialists (construction-cis).
 *
 ...
 * Uses construction-cis tokens: --accent (orange #f97316), --dark (slate #1e293b).
 *
 * Consent: uses siteConfig.resourceConsentText (in-house-only wording, never
 * names any partner firm). Resource downloads are NOT shared with any partner.
 *
 * Submit: goes through submitCisLead with captureMode:"email_only" and
 * extras { resource_gate: true } so the server chokepoint can exclude these
 * rows from the partner-CC notify path.
 *
 * Honeypot: enquiry_ref. Value is passed through to submitCisLead; do NOT
 * early-return on a filled honeypot (F4 fix). The server stores the row flagged
 * and returns success so a real human caught by browser autofill is never dropped.
 *
...
// Orange/slate token-safe input class (no --primary, no var(--gold), no var(--navy))
const inputClass =
...
export function ResourceGate({
  ...
  topic: TopicKey;
  /** Gate heading and blurb. When omitted, derived automatically from the topic. */
  copy?: GateCopy;
  ...
  const copy: GateCopy = copyProp ?? gateCopy(topic);
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Pass through to submitCisLead so the server stores the row flagged.
    ...
    const result = await submitCisLead(
      {
        full_name: "",
        email,
        phone: "",
        ...
        // would 400 every submission on the empty name/phone (QA BLOCKING
        // finding, D-3). The message prefix keeps the 10-char floor satisfied.
  ...
  return (
    <section
      ref={rootRef}
      className="not-prose @container my-12 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_1px_2px_rgba(30,41,59,0.04),0_12px_28px_-16px_rgba(30,41,59,0.18)] ring-1 ring-slate-900/[0.03]"
      aria-labelledby="resource-gate-heading"
    >
      {/* Branded top accent: orange */}
      <div className="h-1 bg-[var(--accent)]" style={{ background: "#f97316" }} />
              ...
              <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                ...
                {guideReady && resource?.guide && (
                  <a
                    ...
                    className="inline-flex items-center justify-center rounded-full border-2 border-orange-500 px-5 py-3 text-base font-semibold text-orange-700 transition-colors hover:bg-orange-50"
                  >
