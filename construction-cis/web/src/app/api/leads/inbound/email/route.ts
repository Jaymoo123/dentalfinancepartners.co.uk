/**
 * Inbound email webhook for Resend: processes replies to construction-cis
 * nurture outreach.
 *
 * When a lead replies to a nurture email, Resend posts an email.received event here.
 * The route verifies the Svix signature, resolves the sender to a construction-cis
 * lead, detects opt-outs, and classifies the reply before updating contactability state.
 *
 ...
 *   Inbound routing domain: tradetaxspecialists.co.uk (or a reply.* subdomain)
 *   Endpoint URL: https://www.tradetaxspecialists.co.uk/api/leads/inbound/email
 *   Events: email.received (inbound only)
 *   Signing secret (whsec_...): set as LEAD_RESEND_INBOUND_SECRET
 *
 * Classification: no AI classify/concierge layer on this site (estate-central,
 * Sonnet mechanical build only). Deterministic opt-out detection runs first;
 * anything left over is a genuine reply, the same safe fallback the other
 * niche sites use when their classifier is unconfigured or gated off.
 *
...
export async function POST(req: NextRequest) {
  ...
  // Resolve sender to a construction-cis lead (most recent match wins).
  ...
  try {
    // Deterministic opt-out check first: honour it and stop, no further work.
