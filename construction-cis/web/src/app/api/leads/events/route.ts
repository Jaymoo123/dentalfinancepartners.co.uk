/**
 * Resend webhook sink for lead-nurture email engagement events (construction-cis).
 *
 ...
 * subscriber one:
 *   Endpoint URL: https://www.tradetaxspecialists.co.uk/api/leads/events
 ...
 * message id in the payload. An unknown id is almost certainly a message from
 * one of the sibling sites sharing the same Resend account; those are
 * silently ignored (200 empty) so their own webhook endpoint handles them
...
async function hasRecentComplaintAlert(leadId: string): Promise<boolean> {
  ...
  const res = await adminSelect<ContactEventRow>("lead_contact_events", {
    ...
    // works instead of 400-ing and alerting on every complaint.
...
<p style="font-size:13px;color:#64748b;">Automated notification from the Trade Tax Specialists lead-nurture system.</p>
</div>`,
...
export async function POST(req: NextRequest) {
  ...
  // Resolve to a lead. An unknown id is probably a message from a sibling
  // site sharing the same Resend account; ignore it silently so that site's
  // own webhook endpoint handles it correctly.
