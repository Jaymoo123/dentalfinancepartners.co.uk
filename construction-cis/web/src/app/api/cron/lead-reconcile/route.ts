 ...
 *   - role is not 'resource' (resource downloads, in-house consent,
...
async function run(req: NextRequest): Promise<NextResponse> {
  ...
  // 1. Pull the recent construction-cis leads in the lookback window (newest first).
    ...
    for (const row of toEnrol) {
      try {
        const lead: NurtureLead = {
          ...
          source: row.source ?? "construction-cis",
