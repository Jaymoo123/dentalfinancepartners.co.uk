 ...
 *   /for/<trade>             -> cis-refund (generic CIS entry, best match)
...
export function deriveTopic(pathname: string): TopicKey | null {
  ...
  // Trade pages: a CIS subcontractor landing on /for/<trade> has clear refund intent.
  if (parts[0] === "for" && parts[1]) return "cis-refund";
  return null;
}
