...
function reviewOffer(
  ...
  return {
    kind: "specialist",
    title: t.ctaCopy,
    blurb: "A free, no-obligation review of your CIS position with a specialist accountant.",
    href: "/contact",
    reason,
  };
}
...
function specialistOffer(topicKey: TopicKey): IntentOffer {
  const t = getTopic(topicKey);
  const label = (t?.label ?? "CIS tax").toLowerCase();
  return {
    kind: "specialist",
    title: `Speak to a CIS specialist`,
    blurb: `Get your specific ${label} position checked by a CIS specialist accountant.`,
    href: "/contact",
  ...
  switch (surface) {
    ...
    case "returning_bar": {
      ...
      const offer =
        reviewOffer(
          resume,
          "Pick up where you left off, get your CIS position reviewed",
        ) ??
        toolOffer(resume) ??
