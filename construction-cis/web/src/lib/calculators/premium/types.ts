/**
 * Types for the premium calculator tier on the construction-cis site.
 *
 * Ported from Dentists/web/src/lib/tools/premium/types.ts and adapted for
 * construction-cis (bfp prefix; orange/slate tokens; calculators not tools).
 * The type shapes are identical; only the storage prefix and token comments differ.
 *
 * Import chain: imports only from the shared web-shared package's
 * tools/types.ts (CalcField, CalcValues, CalcResult, CalcResultRow), which
 * are already available across the construction-cis site.
 */
import type {
  CalcField,
  CalcValues,
...
/* ---------------------------------------------------------------------------
 * Scenario comparison (e.g. CIS vs PAYE)
 * ------------------------------------------------------------------------- */
...
export interface ChartSeries {
   ...
   * Use the construction-cis token system: var(--accent) = orange, var(--dark) = slate.
   * The site does NOT define --gold or --navy (those are Dentists-only).
   * Never use var(--primary) -- construction-cis does not define --primary.
   */
  color: string;
}
...
export interface PremiumToolConfig {
  /** stable id; matches resources.ts toolId. Storage prefix: bfp (FROZEN). */
  id: string;
  topic: TopicKey;
  title: string;
