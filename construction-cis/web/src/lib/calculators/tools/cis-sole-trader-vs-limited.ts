...
export const cisSoleTraderVsLimited: GenericTool = {
  ...
  explainer: {
    ...
    paragraphs: [
      ...
      "A limited company changes the mechanics entirely. The company registers as a CIS contractor and employer, and uses the Employer Payment Summary (EPS) to tell HMRC each month how much CIS it has suffered as a subcontractor. Those CIS credits are set against the company's own PAYE and employer NIC liability. Where the CIS deductions exceed the PAYE due, the surplus carries forward to the next month and keeps reducing the bill, so for subcontractors with significant deductions and modest salary bills the credit is being used up month by month rather than sitting with HMRC until a return is filed more than a year later. Note what the EPS is not: it is an offset the company applies itself, not a claim HMRC processes, so there is no HMRC turnaround attached to it. Only a credit still unabsorbed at the year end becomes a repayment claim, and HMRC says it will usually respond to one of those within 8 weeks.",
  ...
  faqs: [
    {
      ...
      answer:
        "When your limited company works as a CIS subcontractor, contractors deduct either 20% or 30% from your labour payments and pay it to HMRC on your behalf. Your company can offset these CIS credits against its own PAYE obligation every month through the Employer Payment Summary (EPS). Where the CIS credits exceed what the company owes in PAYE and employer NIC for that month, the surplus carries forward and reduces the following month's bill. The benefit is not a fast refund, it is that the money is being used against a liability you would otherwise have to pay in cash, month by month, instead of sitting with HMRC until you file a Self Assessment return more than a year later. The offset is something the company applies itself rather than a claim HMRC processes, so no HMRC turnaround applies. Only a credit left unabsorbed at the year end becomes a repayment claim, and HMRC says it will usually respond to one within 8 weeks.",
    },
    {
