/**
 * Daily digest composer and runner for the Trade Tax Specialists lead-nurture
 * system.
 *
 * gatherDigestData: reads vw_lead_nurture_health, vw_lead_nurture_stuck, and
 * last-7d send_failed events (filtered to source='construction-cis').
 *
...
export async function gatherDigestData(siteKey: string): Promise<DigestData> {
  ...
  if (failedEvents.length > 0) {
    ...
    const nameRes = await adminSelect<LeadNameRow & { source: string | null }>("leads", {
      id: `in.(${leadIds.join(",")})`,
      select: "id,full_name,source",
      limit: String(leadIds.length + 1),
    }).catch(() => ({ ok: false, status: 0, data: [] as Array<LeadNameRow & { source: string | null }> }));

    const nameMap = new Map<string, string | null>(
      // ponytail: JS-filter by source='construction-cis' to prevent cross-site bleed
      (nameRes.data ?? [])
        .filter((r) => r.source === "construction-cis")
        .map((r) => [r.id, r.full_name ?? null]),
    );

    failedSends = failedEvents
      .filter((e) => nameMap.has(e.lead_id))
      .map((e) => {
...
export function composeDigestEmail(d: DigestData): {
  ...
  lines.push(`Automated digest from the Trade Tax Specialists lead-nurture system.`);
...
export async function runNurtureDigest(): Promise<{ sent: boolean }> {
  const d = await gatherDigestData("construction-cis");
  ...
  try {
    await getResend().emails.send({
      from: getFromAddress(),
      to: resolveLeadTo("construction-cis"),
      subject,
      text,
    });
    return { sent: true };
  } catch (err) {
    console.error("[nurture-digest/cis] digest email send failed", err);
    return { sent: false };
  }
}
