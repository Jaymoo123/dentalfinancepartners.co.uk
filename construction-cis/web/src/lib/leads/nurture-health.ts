/**
 * Observability health reader and guardrail evaluator for the Trade Tax
 * Specialists lead-nurture system.
 *
 * getNurtureHealth: reads vw_lead_nurture_health for site_key='construction-cis'.
...
export async function getNurtureHealth(
  siteKey: string = "construction-cis",
...
export async function runNurtureGuardrails(opts: {
  ...
  const health = await getNurtureHealth("construction-cis");
  if (!health) {
  ...
  if (!alreadyAlerted && process.env.RESEND_API_KEY) {
    ...
    try {
      ...
      console.error("[nurture-health/cis] guardrail alert send failed", err);
    }
  }
  ...
  if (opts.autopauseEnabled && verdict.shouldPause && !control.paused) {
    try {
      ...
      console.error("[nurture-health/cis] auto-pause failed", err);
    }
  }
