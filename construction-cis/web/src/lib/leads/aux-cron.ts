...
export async function runLeadAuxScans(): Promise<{ reminders: number; nudges: number }> {
  ...
  try {
          ...
          if (sendRowId) {
            try {
              ...
              const { html, text } = renderLeadServiceEmail({
                ...
                paragraphs: [
                  `Your free CIS tax review call is tomorrow, ${windowPhrase}.`,
              ...
              await markSendStatus(sendRowId, "failed");
              console.error("[lead-aux-cron/cis] T-24 email error", err);
            }
          }
        }
        ...
        if (!isMorning) {
          ...
          if (inT2 && inSendWindow(now, true)) {
            ...
            if (sendRowId && lead.phone) {
              try {
                const body =
                  `Hi ${firstName}, your free CIS tax review call is later today, ${label}. ` +
                ...
                await markSendStatus(sendRowId, "failed");
                console.error("[lead-aux-cron/cis] T-2 SMS error", err);
              }
        ...
        if (eligibleIds.length > 0 && inSendWindow(now, true)) {
          ...
          for (const lead of nudgeLeadsRes.data) {
            ...
            try {
              ...
              await markSendStatus(sendRowId, "failed");
              console.error("[lead-aux-cron/cis] abandoned nudge error", err);
            }
          }
        }
      }
    ...
    console.error("[lead-aux-cron/cis] unexpected top-level error", err);
  }
