...
vi.mock("@/config/lead-nurture", () => ({
  ...
  LEAD_SEQUENCE_NAMES: { contactability: "cis_contactability", detail_capture: "cis_detail_capture" },
}));
...
vi.mock("@/lib/resend", () => ({
  ...
  getFromAddress: () => "Trade Tax Specialists <ops@x>",
}));
...
describe("formatting", () => {
  it("humanises blog slugs with acronyms", () => {
    expect(humanisePath("/blog/cis-tax-refund-guide")).toBe("CIS tax refund guide");
  });

  it("labels calculators", () => {
    expect(humanisePath("/tools/cis-refund-calculator")).toMatch(/\(calculator\)$/);
  });
...
function seedLead(source = "construction-cis") {
  db.leads.push({
    id: LID,
    full_name: "Sam Doe",
    email: "sam@example.com",
    phone: "+447700900123",
    role: "Sole trader subcontractor (CIS)",
    source,
    message: "hello",
  });
}
...
describe("acknowledgeReply", () => {
  it("sends exactly one ack per lead (idempotent)", async () => {
    ...
    expect(msg.body).toBe(
      "Great, thank you Sam. A specialist will call you shortly. Nothing to prepare. Speak soon.",
    );
