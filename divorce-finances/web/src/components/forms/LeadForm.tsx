...
/** Returns true when the role value signals someone starting a divorce or separation. */
function isStartingDivorce(role: string): boolean {
  return role.toLowerCase().includes("starting-divorce");
}

/** Returns true when the role value signals settlement negotiation. */
function isNegotiatingSettlement(role: string): boolean {
  return role.toLowerCase().includes("negotiating-settlement");
}
...
export function LeadForm({
  ...
  const [selectedDivorceStage, setSelectedDivorceStage] = useState("");
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const completedCount =
      ...
      + (selectedDivorceStage ? 1 : 0)
      + (selectedAssetValue ? 1 : 0);
    trackFormSubmit(completedCount);

    // Qualifiers go into extras (not crammed into message) — schema-clean.
    const extras: Record<string, unknown> = {};
    if (selectedDivorceStage) extras.divorce_stage = selectedDivorceStage;
    if (selectedAssetValue) extras.asset_value = selectedAssetValue;
    ...
    const result = await submitSiteLead(payload, honeypotValue);
  ...
  const showDivorceStageSelect = isStartingDivorce(selectedRole);
  const showAssetValueSelect = isNegotiatingSettlement(selectedRole);
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6" noValidate aria-busy={status === "loading"}>
      ...
      {/* Segment field: divorce stage — shown for people starting out */}
      {showDivorceStageSelect && (
        <div>
          <label htmlFor="divorceStage" className={labelClass}>
          ...
          <select
            ...
            autoComplete="off"
            className={fieldClass}
            value={selectedDivorceStage}
            onChange={(e) => setSelectedDivorceStage(e.target.value)}
            onFocus={() => onFieldFocus("divorceStage")}
            onBlur={(e) => onFieldBlur("divorceStage", !!e.target.value)}
          >
            <option value="">Select...</option>
            <option value="Not started">Not started</option>
            <option value="Application submitted">Application submitted</option>
            <option value="Conditional order">Conditional order</option>
      ...
      {/* Segment field: combined assets — shown for settlement negotiation enquiries */}
      {showAssetValueSelect && (
        <div>
          <label htmlFor="assetValue" className={labelClass}>
            Approximate combined assets{" "}
            <span className="font-normal text-neutral-500">(optional)</span>
          </label>
          <select
            id="assetValue"
            ...
            onFocus={() => onFieldFocus("assetValue")}
            onBlur={(e) => onFieldBlur("assetValue", !!e.target.value)}
          >
            <option value="">Select...</option>
            {ASSET_VALUE_OPTIONS.map((opt) => (
        ...
        <div className="space-y-4 px-4 pb-4 pt-2">
          <div>
            ...
            <textarea
              ...
              rows={3}
              maxLength={600}
              placeholder="e.g. We are separating after 12 years and I want to understand what a fair split of the house and pensions looks like."
              className={fieldClass}
              value={situation}
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              maxLength={600}
              placeholder="e.g. We have agreed to separate and I need to understand where I stand financially."
              className={fieldClass}
              value={prompted}
