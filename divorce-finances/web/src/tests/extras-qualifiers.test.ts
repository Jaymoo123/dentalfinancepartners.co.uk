/**
 * Tests for divorce-finances extras-qualifiers contract.
 *
...
describe("composeLeadMessage (used by MiniCapture / CalcResultCta surfaces)", () => {
  ...
  it("prepends Trade prefix when trade is given", () => {
    ...
    expect(result).toBe("Segment: Plumbers\n\nTest");
  });

  it("prepends Size prefix when subbieCount is given", () => {
    const result = composeLeadMessage({ subbieCount: "6 to 20", message: "Test" });
    expect(result).toBe("Size: 6 to 20\n\nTest");
  });
  ...
  it("prepends both prefixes when both segment fields are given", () => {
    ...
    expect(result).toBe("Segment: Roofers\nSize: 1 to 5\n\nTest");
  });
});
...
describe("extras qualifier object shape", () => {
  ...
  it("message stays clean (no Segment/Size prefix) when extras are used", () => {
    ...
    // The message should be the raw string, with no Segment: prefix.
    expect(rawMessage).not.toContain("Segment:");
    expect(rawMessage).not.toContain("Size:");
