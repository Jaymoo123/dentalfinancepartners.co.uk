...
import { getSiteUrl, niche } from "@/config/niche-loader";
...
export function buildIcsForSlot(params: IcsSlotParams): string {
  ...
  const uid     = `${params.leadId}-${params.date}-${params.windowKey}@${niche.domain}`;
  const summary = `Free review call (${params.label})`;
  const desc    = "We will call you in this window. Nothing to prepare.";

  return [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    `PRODID:-//${niche.display_name}//Lead Review//EN`,
    "BEGIN:VEVENT",
    `UID:${uid}`,
...
const FOOTER   =
  `You are receiving this because you submitted an enquiry on ${niche.domain}.`;
