 ...
 * reachable AND has actively responded, so it is safe to hand to the partner firm.
 *
...
export async function promoteIfContactable(leadId: string): Promise<PromoteResult> {
  try {
    ...
    if (handoff.sent === true || handoff.skipped) {
      ...
      // operator clicks "I have forwarded this to the partner firm" in the
      // handoff email (POST /api/leads/forwarded/[token]), so 'forwarded' means a
      // real partner-firm hand-over, not merely that our brief email was delivered.
    } else {
