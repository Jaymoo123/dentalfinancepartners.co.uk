/**
 * divorce-finances lead-nurture channel sender: implements the engine's
 ...
 * inbound.propertytaxpartners.co.uk. No site-specific sending domain.
 */
...
function serviceFrom(): string {
  const name = process.env.LEAD_SERVICE_FROM_NAME || niche.display_name;
...
export function twilioAuth(): TwilioAuth | null {
  ...
  if (account.startsWith("SK")) {
    console.error(
      "[lead-nurture/divorce] TWILIO_ACCOUNT_SID looks like an API Key SID (SK...). " +
