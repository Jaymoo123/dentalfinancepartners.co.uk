/**
 * Daily digest composer and runner for the divorce-finances lead-nurture
 * system.
 *
 * gatherDigestData: reads vw_lead_nurture_health, vw_lead_nurture_stuck, and
 * last-7d send_failed events (filtered to source='divorce-finances').
 *
...
export async function gatherDigestData(siteKey: string): Promise<DigestData> {
  ...
  if (failedEvents.length > 0) {
    ...
    const nameMap = new Map<string, string | null>(
      // ponytail: JS-filter by source='divorce-finances' to prevent cross-site bleed
      (nameRes.data ?? [])
        .filter((r) => r.source === "divorce-finances")
...
export function composeDigestEmail(d: DigestData): {
  ...
  lines.push(`Automated digest from the divorce-finances lead-nurture system.`);
...
export async function runNurtureDigest(): Promise<{ sent: boolean }> {
  const d = await gatherDigestData("divorce-finances");
  ...
  try {
    await getResend().emails.send({
      from: getFromAddress(),
      to: resolveLeadTo("divorce-finances"),
      subject,
      text,
    });
    return { sent: true };
  } catch (err) {
    console.error("[nurture-digest/divorce] digest email send failed", err);
    return { sent: false };
  }
}
