...
export async function runLeadRetentionPurge(opts: {
  ...
  // Fetch candidate leads: divorce-finances source, created before the cutoff,
  // capped at BATCH_LIMIT so a large historical backlog cannot time out a
  // single run. Rows whose extras already contain anonymised_at are filtered
  // out in JS to avoid a complex PostgREST JSONB filter in the query string.
