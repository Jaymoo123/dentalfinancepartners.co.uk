 ...
 *
 * Uses the validated niche config (@/config/niche-loader), this site's own
 * convention, so the labels stay single-sourced with the rest of the app.
 */

import { niche } from "@/config/niche-loader";

const ROLE_OPTION_MAP: Record<string, string> = Object.fromEntries(
  (niche.lead_form?.role_options ?? []).map((o: { value: string; label: string }) => [o.value, o.label]),
);
