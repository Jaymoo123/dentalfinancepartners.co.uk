/**
 * Observability health reader and guardrail evaluator for the divorce-finances
 * Specialists lead-nurture system.
 *
 * getNurtureHealth: reads vw_lead_nurture_health for site_key='divorce-finances'.
 ...
 * optionally auto-pauses via nurture-control.
 *
 * CONVENTIONS
...
export async function getNurtureHealth(
  siteKey: string = "divorce-finances",
...
export async function runNurtureGuardrails(opts: {
  ...
  const health = await getNurtureHealth("divorce-finances");
  if (!health) {
  ...
  if (!alreadyAlerted && process.env.RESEND_API_KEY) {
    ...
    try {
      ...
      console.error("[nurture-health/divorce] guardrail alert send failed", err);
    }
  }
  ...
  if (opts.autopauseEnabled && verdict.shouldPause && !control.paused) {
    try {
      ...
      console.error("[nurture-health/divorce] auto-pause failed", err);
    }
  }
