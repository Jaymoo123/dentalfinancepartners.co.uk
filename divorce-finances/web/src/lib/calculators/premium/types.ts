/**
 * Types for the premium calculator tier on the divorce-finances site.
 *
 * Ported from Dentists/web/src/lib/tools/premium/types.ts and adapted for
 * divorce-finances (dvf prefix; orange/slate tokens; calculators not tools).
 ...
 * are already available across the site.
 */
import type {
  CalcField,
  CalcValues,
...
/* ---------------------------------------------------------------------------
 * Scenario comparison (two options side by side)
 * ------------------------------------------------------------------------- */
...
export interface ChartSeries {
   ...
   * Use the site token system: var(--accent) = orange, var(--dark) = slate.
   * The site does NOT define --gold or --navy (those are Dentists-only).
   * Never use var(--primary) -- this site does not define --primary.
   */
  color: string;
}
...
export interface PremiumToolConfig {
  /** stable id; matches resources.ts toolId. Storage prefix: dvf. */
  id: string;
  topic: TopicKey;
  title: string;
