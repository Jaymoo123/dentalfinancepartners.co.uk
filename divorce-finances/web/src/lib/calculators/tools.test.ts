 ...
 *   order £321 (HMCTS, from 13 July 2026, SI 2026/642 fee 5.4).
 *   Help with Fees: £1,420 single / £2,130 couple / +£425 per child aged 0-13
 *   (£710 for 14+; tool uses £425) income thresholds, 50p per £1 taper,
 *   £4,250 capital limit (£16,000 at 66+).
...
describe("divorce-cost-calculator", () => {
  ...
  it("contested route uses the £321 fee and warns", () => {
    // 628 + 600..1800 + 321 + 12000..36000 = 13549..38749
    const r = divorceCostCalculator.compute({ route: "solicitor", finances: "contested", hwfLikely: false });
    expect(r.headline.value).toBe("£13,549 to £38,749");
    expect(r.headline.tone).toBe("warn");
    expect(r.rows!.some((row) => row.value === "£321")).toBe(true);
  });
});
...
describe("help-with-fees-checker", () => {
  ...
  it("partial remission taper: £630 over threshold pays £315", () => {
    // threshold 1420 + 2*425 = 2270; excess 630; contribution 315; remission 313
    const r = helpWithFeesChecker.compute({
    ...
    expect(r.verdict!.positive).toBe(true);
    expect(r.headline.value).toBe("£315");
    expect(r.rows!.find((x) => x.label === "Fee remitted")!.value).toBe("£313");
    expect(r.rows!.find((x) => x.label.includes("threshold"))!.value).toBe("£2,270");
  });
