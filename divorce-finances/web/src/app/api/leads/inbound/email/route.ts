/**
 * Inbound email webhook for Resend: processes replies to divorce-finances nurture outreach.
 *
 * When a lead replies to a nurture email, Resend posts an email.received event here.
 * The route verifies the Svix signature, resolves the sender to a lead, detects
 * opt-outs, and classifies the reply before updating contactability state.
 *
 ...
 *   Inbound routing domain: this site's domain (or a reply.* subdomain)
 *   Endpoint URL: https://<this site's domain>/api/leads/inbound/email
 *   Events: email.received (inbound only)
 *   Signing secret (whsec_...): set as LEAD_RESEND_INBOUND_SECRET
 *
 * Classification: the AI copy/classify layer (sequence-gen.ts, lib/ai/anthropic)
 * is estate-central and not ported here (Opus-only content, Sonnet mechanical
 * build only). Deterministic opt-out detection still runs first; anything left
 * over defaults straight to genuine_reply, the same safe fallback Property uses
 * whenever its AI classifier is unconfigured or gated off.
 *
...
// so our own quoted footer ("reply STOP") can never opt a lead out.
...
export async function POST(req: NextRequest) {
  ...
  // Resolve sender to a divorce-finances lead (most recent match wins).
  ...
  try {
    ...
    // No AI classify layer on this site (see file header): every non-opt-out
    // reply is treated as genuine_reply, the same safe fallback Property uses
    // when its classifier is unconfigured. Never drops a real reply; an
    // auto-responder just gets treated as a real one, which only means an
    // unnecessary ack/ops-notify.
    {
