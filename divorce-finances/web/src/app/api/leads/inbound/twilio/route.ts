 ...
 * resolve the sender to a divorce-finances lead, and either record their reply
 * as a contactability signal or honour a STOP opt-out.
 *
 * Property also runs a "concierge" AI reply-drafting layer on this path;
 * that stack is estate-central and not ported here, so a positive reply
 * always takes the plain acknowledgeReply branch (Property's non-concierge
...
export async function POST(req: NextRequest) {
  ...
  try {
    if (intent === "opt_out") {
      ...
      // No concierge layer on this site (see file header): always the plain ack.
      ...
      // "who is this") from being miscounted as positive engagement.
