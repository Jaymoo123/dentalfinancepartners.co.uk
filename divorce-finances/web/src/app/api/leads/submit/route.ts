/**
 * divorce-finances lead submission chokepoint.
 *
 * The factory handles:
 ...
 *
 * onLeadInserted: enrols every brand-new lead into the nurture sequence.
...
const baseHandler = createLeadSubmitHandler({ source: "divorce-finances" });
...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  if (result.success && result.leadId) {
    ...
    try {
      ...
      console.error("[leads/submit/divorce] enrol failed (non-fatal)", err);
    }
