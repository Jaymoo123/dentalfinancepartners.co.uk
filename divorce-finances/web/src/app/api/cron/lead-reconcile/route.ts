 ...
 *   - role is not 'resource' (in-house consent resource downloads,
 *     intentionally not nurtured), AND
 *   - its status is still 'new' or 'nurturing' (any terminal / handled status
 *     is deliberately excluded).
 * extras.honeypot is a monitoring tag only, so tagged leads stay eligible.
 */
...
async function run(req: NextRequest): Promise<NextResponse> {
  ...
  // 1. Pull the recent divorce-finances leads in the lookback window (newest first).
    ...
    for (const row of toEnrol) {
      try {
        const lead: NurtureLead = {
          ...
          source: row.source ?? "divorce-finances",
