/**
 * Daily lead-nurture digest cron for divorce-finances: fires at 07:00 UTC
 * and sends a plain-text operator summary covering the last 24 hours. Quiet on
 * dormant days (no sends, no stuck, no failures).
 *
...
async function run(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.error("[lead-nurture-digest/divorce] heartbeat write failed", err);
  }
  ...
  try {
    ...
    console.error("[lead-nurture-digest/divorce] unexpected error", err);
