/**
 * Lead-nurture cron for divorce-finances: fires due follow-up steps for
 * leads that have not yet responded. Driven by Vercel Cron (hourly).
 *
 * Dormancy (two layers):
 *   Auth: CRON_SECRET must match (bearer), or 401.
 *   Master arm: LEAD_NURTURE_ENABLED. Until that is set, the run
 *   short-circuits with zero processing so no lead is ever advanced while the
 *   system is dormant.
 *
...
async function run(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.error("[lead-nurture-cron/divorce] heartbeat write failed", err);
  }
  ...
  if (effectiveArmed) {
    try {
      ...
      console.error("[lead-nurture-cron/divorce] aux-cron failed (non-fatal)", err);
    }
  }
  ...
  if (cronArmed) {
    try {
      ...
      console.error("[lead-nurture-cron/divorce] guardrail scan failed", err);
    }
  }
