        ...
        <section key={s.sessionId} className="overflow-hidden rounded-xl border border-slate-200 bg-white">
          <header className="border-b border-slate-100 bg-slate-50 px-4 py-2.5">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500">
              Visit {si + 1} &middot; {new Date(s.startTs).toLocaleDateString("en-GB", { weekday: "short", day: "numeric", month: "short" })}
            </h3>
          </header>
            ...
            {s.lines.map((l, i) => (
              <li key={i} className="relative flex gap-3 pb-3 last:pb-0">
                ...
                <div className="min-w-0">
                  ...
                  <span className={cn("text-sm", l.kind === "convert" && "font-semibold text-emerald-800", l.kind === "reading" && "italic text-slate-500", l.kind === "close" && "text-slate-500", l.kind === "open" && "font-medium text-slate-900")}>
                    {l.text}
                  </span>
                </div>
              </li>
