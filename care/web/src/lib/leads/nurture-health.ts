/**
 * Observability health reader and guardrail evaluator for the Care
 * lead-nurture system.
 *
 * Mirrors Property/web/src/lib/leads/nurture-health.ts exactly, with
 * 'property' literals replaced by 'care' and the Care Home Tax
...
export async function runNurtureGuardrails(opts: {
  ...
  const health = await getNurtureHealth("care");
  if (!health) {
  ...
  if (!alreadyAlerted && process.env.RESEND_API_KEY) {
    ...
    try {
      ...
      console.error("[nurture-health/care] guardrail alert send failed", err);
    }
  }
  ...
  if (opts.autopauseEnabled && verdict.shouldPause && !control.paused) {
    try {
      ...
      console.error("[nurture-health/care] auto-pause failed", err);
    }
  }
