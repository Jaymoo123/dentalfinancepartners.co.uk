/**
 * Care lead-nurture cron: fires due follow-up steps for leads that have
...
import { buildCareLeadNurtureConfigs, buildLeadMessageContext } from "@/config/lead-nurture";
...
async function run(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.error("[lead-nurture-cron/care] heartbeat write failed", err);
  }

  const configs = buildCareLeadNurtureConfigs();
  ...
  if (cronArmed) {
    try {
      ...
      console.error("[lead-nurture-cron/care] guardrail scan failed", err);
    }
  }
