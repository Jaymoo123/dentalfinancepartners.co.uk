/**
 * Server chokepoint for Care Home Tax lead submission.
 *
...
const sharedHandler = createLeadSubmitHandler({ source: "care" });
...
export async function POST(req: NextRequest) {
  ...
  const source = String(body.source ?? "care").toLowerCase();
  ...
  if (!isResource && !json.merged) {
    try {
      ...
      console.error("[leads/submit/care] enrol/instant-touch failed", e);
    }
  }
