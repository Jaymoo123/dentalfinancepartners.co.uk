/**
 * Server chokepoint for Crypto Tax Partners lead submission.
 *
...
const sharedHandler = createLeadSubmitHandler({ source: "crypto" });
...
export async function POST(req: NextRequest) {
  ...
  const source = String(body.source ?? "crypto").toLowerCase();
  ...
  if (!isResource && !json.merged) {
    try {
      ...
      console.error("[leads/submit/crypto] enrol/instant-touch failed", e);
    }
  }
