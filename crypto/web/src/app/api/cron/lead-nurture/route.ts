/**
 * Crypto lead-nurture cron: fires due follow-up steps for leads that have
...
import { buildCryptoLeadNurtureConfigs, buildLeadMessageContext } from "@/config/lead-nurture";
...
async function run(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.error("[lead-nurture-cron/crypto] heartbeat write failed", err);
  }

  const configs = buildCryptoLeadNurtureConfigs();
  ...
  if (cronArmed) {
    try {
      ...
      console.error("[lead-nurture-cron/crypto] guardrail scan failed", err);
    }
  }
