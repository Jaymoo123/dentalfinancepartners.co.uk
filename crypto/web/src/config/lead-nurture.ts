/**
 * Crypto lead-nurture composition: the contactability sequence and
 * detail-capture sequence for Crypto Tax Partners enquiries.
 *
 ...
 *
 * Audience: UK cryptoasset holders. Investors, day traders, DeFi and staking
 * participants, NFT creators, miners, and businesses holding or accepting
 * crypto.
 *
...
const SEQUENCE_NAME = "crypto_contactability";
const DETAIL_CAPTURE_SEQUENCE_NAME = "crypto_detail_capture";
...
const COMPANY = "Crypto Tax Partners";
const SIGNOFF = `Speak soon, the team at ${COMPANY}`;
const FOOTER =
  "You are receiving this because you submitted an enquiry on cryptotaxpartners.co.uk.";
...
// ── Calculator slug map (verified against crypto/web/src/lib/calculators/registry.ts) ──
...
const CALC_MAP: Record<string, { name: string; path: string }> = {
  ...
  disclosure: {
    ...
    path: "/calculators/crypto-disclosure-estimator",
  },
  trader_status: {
    name: "Investor vs Trader Status Checker",
    path: "/calculators/investor-vs-trader-checker",
  },
  staking_mining: {
    name: "Staking and Mining Income Estimator",
    path: "/calculators/staking-mining-income-estimator",
  },
};
...
function calcKeyFrom(intentCategory: string | undefined): string | undefined {
  ...
  // Fuzzy fallbacks
  if (/disclos|nudge|voluntary/.test(norm)) return "disclosure";
  if (/trader|day_trad|investor/.test(norm)) return "trader_status";
  if (/stak|mining|airdrop/.test(norm)) return "staking_mining";
  if (/cgt|capital_gain|disposal/.test(norm)) return "cgt";
  return undefined;
}
...
export async function buildLeadMessageContext(
  ...
  return {
    ...
    variant: undefined, // ponytail: no A/B split for crypto; add when needed
    engagementVariant,
    missingFields,
    missingPhrase,
    detailsUrl,
...
// Topics: CGT on disposals, staking and mining income, DeFi, trader status,
// HMRC nudge letters and disclosure, Self Assessment.
  ...
  // ── Step 1: T0 SMS + WhatsApp ──────────────────────────────────────────────
  {
    ...
    buildMessages: (c) => [
      smsMsgWithGen(
        c,
        "t0_sms",
        `Hi ${c.firstName}, it's the team at Crypto Tax Partners. Thanks for your enquiry. Reply YES and one of our specialists will call you. ${c.optOutText}`,
      ),
  ...
  // ── Step 3: Day 1 SMS + WhatsApp ──────────────────────────────────────────
  {
    ...
    buildMessages: (c) => [
      smsMsgWithGen(
        c,
        "day1_sms",
        `Hi ${c.firstName}, following up on your crypto tax enquiry. We have some specialist time free this week for a short call. Reply YES and we will call you. ${c.optOutText}`,
      ),
    ...
    buildMessages: (c) => [
      emailMsg(
        ...
        [
          ...
          "For what it is worth, the moments a review tends to earn its keep are when a letter arrives from HMRC, when a filing deadline is close, or when a few years of exchange history need piecing together at once. If one of those turns up, reply to this email whenever, and we will pick it straight back up.",
          "All the best.",
        ],
        "breakup_day11",
        { cta: null, secondary: null },
  ...
  return {
    ...
    onSequenceExhausted: async (leadId) => {
      try {
        ...
        console.error("[lead-nurture/crypto] mark lead unreachable failed", err);
      }
    },
  };
}
      ...
      return [
        emailMsg(
          ...
          "detail_capture_t0",
          {
            cta: null,
            secondary: null,
            ...detailGreeting(c, "Hi, the team at Crypto Tax Partners here."),
          },
        ),
      ];
    },
    ...
    buildMessages: (c) => {
      ...
      return [
        emailMsg(
          ...
          [
            "One quick pointer while your enquiry is with us. The step people most often underestimate is gathering the transaction history itself, because exchanges close accounts, retire export tools and change formats, and older records get harder to retrieve the longer they are left. Pulling yours together early makes everything after it easier.",
          ...
          "detail_capture_day3",
          {
            cta: null,
            secondary: null,
            ...detailGreeting(c, "Hi, the team at Crypto Tax Partners."),
          },
        ),
      ];
    },
      ...
      return [
        emailMsg(
          ...
          [
            ...
            `If you would still like a free review, just reply with ${ask}, whether that is next week or next year. The moments it tends to matter most are a letter arriving from HMRC, a filing deadline coming up, or a busy year with a lot of activity to account for.`,
            "All the best.",
          ],
          "detail_capture_day7",
          {
            cta: null,
            secondary: null,
            ...detailGreeting(c, "Hi, the team at Crypto Tax Partners, one last time."),
          },
        ),
      ];
    },
  ...
  return {
    ...
    onSequenceExhausted: async (leadId) => {
      try {
        ...
        console.error("[lead-nurture/crypto] detail-capture exhaustion failed", err);
      }
    },
  };
}

export function buildCryptoLeadNurtureConfig(
...
export function buildCryptoLeadNurtureConfigs(): LeadNurtureConfig[] {
