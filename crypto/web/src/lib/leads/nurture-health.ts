/**
 * Observability health reader and guardrail evaluator for the Crypto
 * lead-nurture system.
 *
 * Mirrors Property/web/src/lib/leads/nurture-health.ts exactly, with
 * 'property' literals replaced by 'crypto' and the Crypto Tax Partners
...
export async function runNurtureGuardrails(opts: {
  ...
  const health = await getNurtureHealth("crypto");
  if (!health) {
  ...
  if (!alreadyAlerted && process.env.RESEND_API_KEY) {
    ...
    try {
      ...
      console.error("[nurture-health/crypto] guardrail alert send failed", err);
    }
  }
  ...
  if (opts.autopauseEnabled && verdict.shouldPause && !control.paused) {
    try {
      ...
      console.error("[nurture-health/crypto] auto-pause failed", err);
    }
  }
