 ...
 *
 * House style: no em-dashes. British English. Crypto Tax Partners brand.
 */
...
export function buildIcsForSlot(params: IcsSlotParams): string {
  ...
  const uid     = `${params.leadId}-${params.date}-${params.windowKey}@cryptotaxpartners.co.uk`;
  const summary = `Crypto tax review call (${params.label})`;
  const desc    = "A crypto tax specialist will call you in this window. Nothing to prepare.";

  return [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Crypto Tax Partners//Lead Review//EN",
    "BEGIN:VEVENT",
    `UID:${uid}`,
...
export async function runLeadAuxScans(): Promise<{ reminders: number; nudges: number }> {
  ...
  try {
          ...
          if (sendRowId) {
            try {
              ...
              const { html, text } = renderLeadServiceEmail({
                ...
                paragraphs: [
                  `Your free crypto tax review call is tomorrow, ${windowPhrase}.`,
                  "One of our crypto tax specialists will ring you then. They will have read your enquiry before they call, the call takes about 20 minutes, and there is nothing to prepare.",
              ...
              await markSendStatus(sendRowId, "failed");
              console.error("[lead-aux-cron/crypto] T-24 email error", err);
            }
          }
        }
        ...
        if (!isMorning) {
          ...
          if (inT2 && inSendWindow(now, true)) {
            ...
            if (sendRowId && lead.phone) {
              try {
                const body =
                  `Hi ${firstName}, your free crypto tax review call is later today, ${label}. ` +
                  `Your crypto tax specialist will call you then. Need a different time? Just reply and we will rearrange. Reply STOP to opt out.`;
                ...
                await markSendStatus(sendRowId, "failed");
                console.error("[lead-aux-cron/crypto] T-2 SMS error", err);
              }
        ...
        if (eligibleIds.length > 0 && inSendWindow(now, true)) {
          ...
          for (const lead of nudgeLeadsRes.data) {
            ...
            try {
              ...
              await markSendStatus(sendRowId, "failed");
              console.error("[lead-aux-cron/crypto] abandoned nudge error", err);
            }
          }
        }
      }
    ...
    console.error("[lead-aux-cron/crypto] unexpected top-level error", err);
  }
