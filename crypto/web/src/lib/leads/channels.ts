/**
 * Crypto lead-nurture channel sender: implements the engine's ChannelSender
 ...
 *
 * Mirrors Property/web/src/lib/leads/channels.ts with Crypto Tax Partners
...
function serviceFrom(): string {
  const name = process.env.LEAD_SERVICE_FROM_NAME || "Crypto Tax Partners";
...
export function twilioAuth(): TwilioAuth | null {
  ...
  if (account.startsWith("SK")) {
    console.error(
      "[lead-nurture/crypto] TWILIO_ACCOUNT_SID looks like an API Key SID (SK...). " +
