...
export function SiteFooter() {
  return (
    <footer className="border-t border-neutral-200 bg-[#0e1a3a] text-neutral-300">
      <div className={`${siteContainerLg} py-12`}>
        ...
        <ul className="mt-6 flex flex-wrap gap-4 text-sm">
          {footerLinks.map((link) => (
            <li key={link.href}>
              <Link href={link.href} className="text-neutral-300 transition-colors hover:text-white">
                {link.label}
              </Link>
            </li>
          ))}
        </ul>
        <p className="mt-8 border-t border-white/10 pt-6 text-xs leading-relaxed text-neutral-500">
