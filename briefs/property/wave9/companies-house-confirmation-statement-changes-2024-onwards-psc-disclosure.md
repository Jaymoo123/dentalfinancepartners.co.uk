# Wave 9 brief: companies-house-confirmation-statement-changes-2024-onwards-psc-disclosure
...
**Source markdown path on launch:** `Property/web/content/blog/companies-house-confirmation-statement-changes-2024-onwards-psc-disclosure.md`
**Live URL on launch:** https://www.propertytaxpartners.co.uk/blog/incorporation-and-company-structures/companies-house-confirmation-statement-changes-2024-onwards-psc-disclosure

---

## Manager pre-decisions

- **Suggested slug:** `companies-house-confirmation-statement-changes-2024-onwards-psc-disclosure`
...
> The LANDLORD-LTDCO ANNUAL-HOUSEKEEPING walkthrough for the post-4-March-2024 confirmation-statement regime, written for directors / PSCs of property SPVs and FICs. Page anchors on **CA 2006 s.853A (confirmation statement)** as amended by **ECCTA 2023 Part 1 Chapter 2** plus the inserted/amended provisions: registered-office "appropriate address" rule under amended **CA 2006 s.86** (no PO boxes; address where documents can be acknowledged); registered-email-address requirement under new **CA 2006 s.88A** (Stage 2 verifies inserted-section number); lawful-purpose statement on incorporation + at every confirmation statement (Stage 2 enumerates the inserted-section provision). All effective 4 March 2024 per the Commencement No. 2 Regulations (Stage 2 enumerates verbatim SI number). (a) **The four things that changed on 4 March 2024.** Registered office must be an "appropriate address". Registered email address must be supplied. Lawful-purpose statement must be made. PSC disclosure tightened (Stage 2 enumerates specifics). (b) **What this means for landlord SPVs using a virtual office or accountant's address.** "Appropriate address" rules in PO-box-only arrangements and pure mail-forwarding services where mail isn't actually acknowledged by a human. Specialist company-formation agents have repositioned services around this rule. Page maps the practical question: is my current registered office still compliant? (c) **The new registered email — who sees it.** CH does NOT publish the registered email on the public register; it's for CH-to-company correspondence only. Page clears the common misconception. (d) **PSC disclosure: what's new + what's coming.** ECCTA reforms tightened PSC information accuracy + introduced ID verification for PSCs as part of the Part 1 Chapter 1 phased rollout (B1 territory). The confirmation statement is the operational moment when PSC information is re-asserted annually; from autumn 2025 / 2026 (phased), PSCs in scope must have completed ID verification before the confirmation statement is filed. (e) **Fees + timing.** Confirmation statement fee from CA 2006 fee schedule (verify operative figures at write time — set by regs); 14-day filing window after the confirmation period; striking-off risk under CA 2006 s.1000 for missed filings. (f) **What the page is NOT:** not a general "what is a confirmation statement" 101 page (well covered by existing competitor inventory + gov.uk); not the ID-verification mechanics walkthrough (B1 territory). Cross-link to B1 (ID verification) as the upstream prerequisite + to `landlord-incorporation-step-by-step-guide-uk` as the incorporation pillar.

If your reasoning suggests the slug/category should differ, you may override, but log the override and reason in the per-page work-log below.

**Stage 1 seed note (for Stage 2 + write-time per §16.35 / §16.36):** confirmation-statement fees are set by regulations and may shift. PSC ID-verification commencement state is volatile per B1's note. The "appropriate address" enforcement timeline (transition for existing arrangements) is on the Companies House campaign page — Stage 2 verifies + session re-verifies at write time. Inserted-section numbers (s.88A registered email, etc.) are seed-cited; Stage 2 confirms them verbatim against legislation.gov.uk.
...
**Primary (confirmation statement framework):**
- CA 2006 s.853A (confirmation statement — duty + content) as amended by ECCTA 2023 Part 1 Chapter 2. Stage 2 enumerates verbatim section + amending ECCTA section.
- CA 2006 s.86 (registered office — "appropriate address" amendment by ECCTA). Stage 2 confirms amending ECCTA section.
- CA 2006 s.88A (registered email — INSERTED by ECCTA; Stage 2 confirms verbatim inserted-section number against legislation.gov.uk — DO NOT trust the seed's "s.88A" without verification, the section number is illustrative pending Stage 2 verification).
- Lawful-purpose statement provision (CA 2006 inserted-section; Stage 2 enumerates).
- CA 2006 PSC framework (Part 21A / Schs 1A-1B) — pre-existing baseline; ECCTA tightens via Part 1 Chapter 4 (Stage 2 confirms section/paragraph references).

**Commencement chain:**
- The Economic Crime and Corporate Transparency Act 2023 (Commencement No. 2, Transitional and Saving Provisions) Regulations 2024 (SI number TBC by Stage 2) — brought CA 2006 s.853A amendments + new s.86 / s.88A into force on **4 March 2024**.

**Adjacent (penalty / consequence):**
- CA 2006 s.1000 (registrar's power to strike off — for missed confirmation statements).
- CA 2006 s.858 + s.859 (offences for false / non-compliant statements).

**Fees:**
- CA 2006 fees schedule (set by the Registrar of Companies (Fees) Regulations — Stage 2 verifies current SI + operative figures; commonly cited as £34 online / £62 paper but verify).

---

## Key questions for Stage 2 + write-time

1. **Verbatim inserted-section number for registered email.** Stage 2 confirms whether s.88A is correct, or whether the inserting Act used a different numbering (e.g. s.86A or a separate inserted block). Treat seed's "s.88A" as candidate citation pending verification.
2. **Operative confirmation-statement fee.** Stage 2 confirms against current CH fee schedule.
3. **"Appropriate address" enforcement timeline.** Stage 2 confirms the transition / enforcement window: was 4 March 2024 hard cut-off, or is there an enforcement-from-X date for existing arrangements? Companies House campaign page is the source.
4. **PSC ID-verification gate at confirmation-statement filing.** Stage 2 confirms whether ID verification of in-scope PSCs is now a precondition of valid confirmation-statement filing as at brief-finalisation, or still pending. State explicitly with operative-as-at date.
5. **Lawful-purpose statement wording.** Stage 2 quotes the inserted-section's statutory wording verbatim (avoids the common-paraphrase trap; the page should quote the exact statutory language).
...
Stage 2 fetch + read instruction: standard `httpx` + `BeautifulSoup`. Extract treatment of: the four 4-March-2024 changes (registered email; appropriate address; lawful purpose; PSC tightening); the PSC ID-verification gate (most competitor pages elide); the "registered email is private" point (frequent misconception); fees / timing.

Candidate competitor sources for Stage 2 to triage:
- Big-four / mid-tier guidance on ECCTA confirmation-statement changes (PwC / Deloitte / EY / BDO / RSM / Saffery / Evelyn).
- Specialist company-secretarial firms (Diligent / Wilmington / specialist law firms — Charles Russell Speechlys / Travers Smith / Stevens & Bolton).
- Incorporation-services firms with updated content post-March 2024 (1st Formations / Companies Made Simple / Inform Direct).
- Property-accountant blogs writing on landlord-LtdCo annual housekeeping (provestor / less-tax-for-landlords / property-tribes affiliates).

**Borrowable patterns:** 4-change summary tables; appropriate-address decision flowcharts; registered-email-FAQ blocks.

---

## Closest existing pages (cannibalisation context — top 5 from wave9_cannibalisation_check.md; top score 0.09, clean ✅ net-new)

- 0.09 — `section-24-higher-rate-taxpayers-changes-2027` (false-positive tokenisation on "changes" — not relevant)
- 0.08 — `ated-overseas-companies-voluntary-compliance-otm-letters` (adjacent CH-touching content for overseas-co landlords — not duplicate; potential cross-link as a parallel compliance topic)
- 0.05 — `corporation-tax-rates-property-companies-2026-27` (adjacent LtdCo context — not duplicate)
- 0.05 — `landlord-tax-changes-2026-complete-guide` (annual-changes round-up — should link to B3 as one of the operational changes)
- 0.05 — `accountant-corporation-tax-property-companies` (service page — natural CTA target / back-link to the operational pillar)

**Cannibalisation discipline:** no overlap blockers. Back-link from `landlord-tax-changes-2026-complete-guide` (raise INTERNAL_LINK flag). Forward-link to B1 (ID verification) as the upstream prerequisite for the PSC-ID-gate sub-section.

---

## Redirect overlap (on launch)

No existing slug overlaps with the post-March-2024 confirmation-statement-changes scope. No middleware edit required on initial launch (Stage 2 verifies against `middleware.ts`).
...
- CA 2006 s.853A: https://www.legislation.gov.uk/ukpga/2006/46/section/853A
- CA 2006 s.86: https://www.legislation.gov.uk/ukpga/2006/46/section/86
- CA 2006 inserted s.88A (registered email — Stage 2 verifies actual section number) — URL TBC by Stage 2 after section-number confirmation.
- ECCTA 2023 c. 56 Part 1 Chapter 2: https://www.legislation.gov.uk/ukpga/2023/56/part/1/chapter/2 (Stage 2 confirms chapter scope)
- The relevant Commencement No. 2 Regulations 2024 (Stage 2 enumerates SI number + URL).

**Government guidance (mandatory write-time WebFetch — phased rollout sensitivities):**
- https://changestoukcompanylaw.campaign.gov.uk/ (Companies House master campaign page)
- https://www.gov.uk/guidance/confirmation-statement-guidance (operative how-to guidance — verify post-March-2024 updates reflected)
- https://www.gov.uk/government/news/changes-to-uk-company-law (Companies House news index — phased-rollout announcements)
- Companies House fee schedule page (operative fee figures).

**House position cross-refs:**
- §11 (Companies House reforms / ECCTA / RoE — high-level locked timeline; **see F-10 Wave 9 mini-lock candidate**)
- §21.4 (LtdCo CT context — adjacent for landlord-director audience)
- §21.5 (FIC mechanics — adjacent; FIC reserved-matters articles + PSC complexity intersect with the post-March-2024 PSC tightening)
...
- Verify CA 2006 s.853A amended wording on legislation.gov.uk.
- Verify CA 2006 s.86 amended wording for "appropriate address".
- Verify the registered-email inserted-section number + wording verbatim.
- Verify the lawful-purpose-statement provision verbatim — quote the statutory wording (not paraphrase).
- Verify operative confirmation-statement fee against current CH schedule.
- Verify the operative status of PSC ID-verification as gate on confirmation-statement filing as at write date.
- Verify the Commencement No. 2 Regulations SI number.

### Voice
- No em-dashes.
- Practical, specific. Named legislation; statutory section references verbatim once Stage 2 has confirmed numbering.
- Anonymised personas only.

### Lead-gen architecture
- LeadForm auto-injected at footer; never duplicate.
- Lead-form role segments emphasise: Existing LtdCo director (multiple SPVs) / Recently-incorporated landlord / FIC-structure family-office user.

### Schema
- FAQs in frontmatter; FAQPage JSON-LD auto-emitted. Target 10-12 FAQs covering: what changed on 4 March 2024; what is an "appropriate address"; is my virtual office still compliant; who can see my registered email; what is the lawful-purpose statement; when must PSCs have ID-verified; current confirmation-statement fee; striking-off risk; how does this differ from the annual accounts deadline (common conflation); B1 cross-link FAQ.
...
### Anti-templating
- Differentiator is the four-changes-on-4-March-2024 frame + the appropriate-address compliance question for virtual-office users + the registered-email-is-private clarification + the PSC ID-gate at filing. Write to it.
...
- Step 1: Read §11 (or §11.A if manager has locked Wave 9 ECCTA mini-lock by Stage 2).
- Step 7: §16.35 verifications listed in "Universal rules" above. Section-number verification is CRITICAL for B3 because seed cites are illustrative pending Stage 2 verification.
