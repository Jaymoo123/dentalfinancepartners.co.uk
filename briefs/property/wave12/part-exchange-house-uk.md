...
- 2026-08-21 **QA FIX ROUND applied** (ADJUDICATIONS.md; findings in `qa/FACTUAL_routes.md` BLK-A12-1 + ADV-A12-2/3/4 and `qa/EDITORIAL_batch3.md` §5 + S1/S2/S4/S7). 14 edits, one file, no commit.

  **BLOCKER (factual), BLK-A12-1, CLOSED.** The CGT section said "the garden and grounds run past the half hectare line, so the extra land sits outside the relief", which drops the s.222(3) extension and reads the two permitted-area tests as one. Replaced with the QA drop-in: half a hectare including the ground the house stands on as the starting point, a larger area only where genuinely needed to enjoy the house as a home judged against its size and character, then the §1.Q difference stated plainly (the builder's stamp duty test shares the baseline but does not tie the larger area to enjoyment as a residence, so a big garden can pass one and fail the other). The QA drop-in's two 39-to-41-word sentences were split into four to hold the route-page mean-sentence band; wording and substance otherwise verbatim. This is the §1.Q differentiator landing on the page for the first time.

  **BLOCKER (editorial), ruling 6 / rule 13, CLOSED.** The CGT block closed on four "For X, read Y" links. Now one link, to the complete CGT guide, per the QA drop-in. Dropped: the PRR page, the deadlines page and the CGT calculator. CGT-block link count measured at 1.

  **Advisories and batch rulings applied:** ADV-A12-2 Consumer Code hedged with the warranty-provider clause in body and FAQ 12 (§26.15 condition now carried, not just the permission); ADV-A12-3 metaDescription now attributes the range to Zoopla (152 chars); ADV-A12-4 NHQB link added on first mention of the code. Ruling 1 boast purge: "Here is the comparison the builders' pages do not print" to "Here is the comparison in full", and "One thing no brochure tells you" deleted; the two honest data-gap lines kept per S1's carve-out, with "the one dated figure in the market" reworded to "the one published figure comes from". Ruling 2: `summary` cut from 61 words to 40, answer first, no catalogue, and the duplicated contents sentence deleted from body paragraph 2. Ruling 5 / S4: closer rebuilt, "Before you" opener and colon triad gone, named tool and "in under a minute" kept, output now "what one would have left you". Editorial A12-4: at-a-glance lead-in "The short version" to "What the deal actually is". Ruling 8: `reviewerCredentials` kept as written per the report's own call.

  **Not applicable, checked:** ruling 13 after-tax route set (this page prices commission only, HOA 1.42% inc VAT, confirmed identical to A13 and the pillar in FACTUAL_routes); rulings 10 and 11 (no EPC figure and no EPC deductibility claim on this page); ruling 12 (no portal content); ruling 9 (no compound H2 named for A12); ruling 4 (FAQs already second person, no byte-identical pair named).

  **Verification after the round:** words 2,548 (band 1,600 to 3,400); mean sentence 16.55 (13 to 17); Flesch 78.0 (68+); "you"/1k 45.5 (28 to 50, down from 48.1); hard statute 0; question H2s 9 of 10; em-dashes 0, en-dashes 0; FAQs 12; metaTitle 48, metaDescription 152, summary 40 words; all 8 internal link targets confirmed on disk including the pillar route, the calculator tool and the three Wave 12 siblings; `frontmatter_lint.py --check` and `validate_blog_content.py` both pass, 0 errors. No figure changed in the round; the only numeric touch was moving £4,260 into the shortened summary, still hedged and still 1.42% of the stated £300,000.
