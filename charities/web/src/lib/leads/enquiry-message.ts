...
// ponytail: the reference site inherited Property's intent keys verbatim and
// nothing reads them, so rather than invent speculative charity keys the map is
// empty and only the neutral fallback is live. Populate it when the enrichment
// layer actually emits charity intent categories.
const CATEGORY_PHRASE_MAP: Record<string, string> = {};

/**
 * Neutral fallback phrase when no clean echo exists, derived from the
 * intent category then role. Falls back to a generic charity finance phrase.
...
export function categoryPhrase(
  ...
  return "get your charity's finance position clear";
}
