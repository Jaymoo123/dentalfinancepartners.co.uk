/**
 * Observability health reader and guardrail evaluator for the Charities
 * lead-nurture system.
 *
 * Mirrors Property/web/src/lib/leads/nurture-health.ts exactly, with
 * 'property' literals replaced by 'charities' and the Trustee Tax
...
export async function runNurtureGuardrails(opts: {
  ...
  const health = await getNurtureHealth("charities");
  if (!health) {
  ...
  if (!alreadyAlerted && process.env.RESEND_API_KEY) {
    ...
    try {
      ...
      console.error("[nurture-health/charities] guardrail alert send failed", err);
    }
  }
  ...
  if (opts.autopauseEnabled && verdict.shouldPause && !control.paused) {
    try {
      ...
      console.error("[nurture-health/charities] auto-pause failed", err);
    }
  }
