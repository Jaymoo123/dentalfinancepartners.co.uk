...
/**
 * Multi-step lead mini-form — Property's deployed winning treatment, locked ON
 ...
 * The server stores honeypot rows flagged and returns success; nothing is
 * silently dropped client-side.
 * LD-04: consent_text stored byte-identical to the rendered label.
 * LD-05: visitor_id/session_id stitched onto every lead row.
 */
...
export function LeadForm({
  ...
  // AN-02: form lifecycle tracking — field names + outcome only, never values.
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // LD-03: pass the honeypot value to the server rather than aborting
    // client-side (client-side aborts lost real leads to browser autofill).
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6" noValidate aria-busy={status === "loading"}>
      {/* LD-03: honeypot — visually hidden, non-semantic name, autofill-proof. */}
      <div
        aria-hidden="true"
