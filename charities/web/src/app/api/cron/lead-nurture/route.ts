/**
 * Charities lead-nurture cron: fires due follow-up steps for leads that have
...
import { buildCharitiesLeadNurtureConfigs, buildLeadMessageContext } from "@/config/lead-nurture";
...
async function run(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.error("[lead-nurture-cron/charities] heartbeat write failed", err);
  }

  const configs = buildCharitiesLeadNurtureConfigs();
  ...
  if (cronArmed) {
    try {
      ...
      console.error("[lead-nurture-cron/charities] guardrail scan failed", err);
    }
  }
