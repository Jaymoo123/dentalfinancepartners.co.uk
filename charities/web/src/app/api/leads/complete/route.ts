/**
 * "Complete your details" endpoint for Trustee Tax.
 *
 ...
 * (no Twilio Lookup re-check on this path), so accept saved phone
...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error("[leads/complete/charities] lead lookup failed", e);
  ...
  if (patchKeys.length > 0) {
    try {
      ...
      console.error("[leads/complete/charities] lead update failed", e);
      return NextResponse.json({ success: false, error: "server" }, { status: 500 });
    }
  }
  ...
  try {
    ...
    console.error("[leads/complete/charities] audit event failed (non-fatal)", e);
  }
  ...
  try {
    ...
    console.error("[leads/complete/charities] confirmed event failed (non-fatal)", e);
  }
  try {
    await adminUpdate(
      "lead_nurture_state",
    ...
    console.error("[leads/complete/charities] nurture state promote failed (non-fatal)", e);
  }
  try {
    await adminUpdate(
      "leads",
    ...
    console.error("[leads/complete/charities] lead status promote failed (non-fatal)", e);
  }
