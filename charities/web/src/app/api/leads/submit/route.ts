/**
 * Server chokepoint for Trustee Tax lead submission.
 *
...
const sharedHandler = createLeadSubmitHandler({ source: "charities" });
...
export async function POST(req: NextRequest) {
  ...
  const source = String(body.source ?? "charities").toLowerCase();
  ...
  if (!isResource) {
    try {
      ...
      console.error("[leads/submit/charities] enrol/instant-touch failed", e);
    }
  }
