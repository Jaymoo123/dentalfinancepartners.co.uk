"""R3 tier2 expats — Stage 2: fetch-verify surviving candidate domains.

Adapted from tier1_care/s2_verify_fetch.py. Script gathers evidence; Claude judges.
Writes raw/verify_evidence.json. Resumable: pass domains as args to restrict.
"""
...
EXPAT_LINK_RE = re.compile(
    r"expat|non-?resident|non-?dom|nrcgt|nrl\b|leaving-?the-?uk|moving-?abroad|"
    r"split-?year|residence|overseas|international-?tax|foreign-?income|fig-?regime|"
    r"double-?tax|dual-?resid|us-?tax|american", re.I)
EXPAT_TEXT_RE = re.compile(
    r"expat|non-?resident|non-?dom|NRCGT|non-resident landlord|NRL scheme|"
    r"leaving the UK|moving abroad|split[- ]year|statutory residence test|\bSRT\b|"
    r"overseas landlord|foreign income and gains|FIG regime|double taxation|"
    r"dual residen|remittance basis|arriving in the UK|returning to the UK|"
    r"offshore|worldwide income|tax treaty", re.I)
UK_RE = re.compile(r"\bUK\b|United Kingdom|HMRC|Companies House|ICAEW|ACCA|CIOT|\bVAT\b|£|"
...
def page_summary(html: str, url: str) -> dict:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if urlparse(full).netloc.replace("www.", "") == urlparse(url).netloc.replace("www.", "") \
                and EXPAT_LINK_RE.search(href):
            links.append(full)
    return {"url": url, "title": title, "meta": meta, "h1": h1, "h2": h2,
            "expat_excerpt": excerpt,
            "expat_mentions": len(EXPAT_TEXT_RE.findall(text)),
            "uk_signals": sorted(set(x.group(0) for x in UK_RE.finditer(text)))[:10],
            "expat_links": list(dict.fromkeys(links))[:8],
...
def main() -> None:
    ...
    for dom in cands:
        ...
        try:
            ...
            print(f"{dom}: EXC {e}")
            continue
        for link in rec["homepage"]["expat_links"]:
            ...
            try:
                ...
                continue
        out[dom] = rec
        print(f"{dom}: OK mentions={rec['homepage']['expat_mentions']} "
