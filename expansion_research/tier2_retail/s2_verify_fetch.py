"""R3 tier2 retail — Stage 2: fetch-verify every surviving candidate domain.
...
RETAIL_LINK_RE = re.compile(
    r"retail|shop-?owner|shopkeeper|convenience|newsagent|off-?licence|franchis|"
    r"high-?street|independent-?shop|store|epos|point-?of-?sale|merchandis", re.I)
RETAIL_TEXT_RE = re.compile(
    r"retail(?:er)?s?\b|shop owner|shopkeeper|independent shop|convenience store|"
    r"newsagent|off-?licence|high street|franchis(?:e|ee|ing)|EPOS|point of sale|"
    r"stock(?:take|control| valuation)|retail scheme|business rates|store owner", re.I)
...
def page_summary(html: str, url: str) -> dict:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if urlparse(full).netloc.replace("www.", "") == urlparse(url).netloc.replace("www.", "") \
                and RETAIL_LINK_RE.search(href):
            links.append(full)
    return {"url": url, "title": title, "meta": meta, "h1": h1, "h2": h2,
            "retail_excerpt": excerpt,
            "retail_mentions": len(RETAIL_TEXT_RE.findall(text)),
            "uk_signals": sorted(set(x.group(0) for x in UK_RE.finditer(text)))[:10],
            "retail_links": list(dict.fromkeys(links))[:8],
...
def main() -> None:
    ...
    for dom in cands:
        ...
        try:
            ...
            print(f"{dom}: EXC {e}")
            continue
        for link in rec["homepage"]["retail_links"]:
            ...
            try:
                ...
                continue
        out[dom] = rec
        print(f"{dom}: OK mentions={rec['homepage']['retail_mentions']} "
