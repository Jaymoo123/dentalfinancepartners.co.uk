"""R3 tier1 manufacturing — Stage 2: fetch-verify every surviving candidate domain.

Adapted from tier1_care/s2_verify_fetch.py. Script gathers evidence; Claude judges.
Writes raw/verify_evidence.json (resumable: pass domains as args to run a chunk).
"""
...
MFG_LINK_RE = re.compile(
    r"manufactur|engineering|engineers?\b|factory|factories|fabricat|precision|aerospace|"
    r"automotive|industrial|machin|tooling|patent-?box|capital-?allowance|"
    r"r-?(?:and|&|%26)-?d|rd-?tax|research-?development|food-?and-?drink|cbam|export", re.I)
MFG_TEXT_RE = re.compile(
    r"manufactur|engineering (?:firm|company|companies|business|sector|clients?)|"
    r"factory|factories|fabricat|precision engineering|aerospace|automotive|"
    r"plant (?:and|&) machinery|tooling|work.?in.?progress|\bWIP\b|stock valuation|"
    r"standard costing|patent box|capital allowance|full expensing|"
    r"R&D|research and development|\bCBAM\b|industrial|production line|CNC", re.I)
UK_RE = re.compile(r"\bUK\b|United Kingdom|HMRC|Companies House|ICAEW|ACCA|\bVAT\b|£|"
                   r"England|Scotland|Wales|London|Manchester|Birmingham|Sheffield|Leeds", re.I)
...
def main() -> None:
    ...
    for dom in cands:
        ...
        try:
            ...
            print(f"{dom}: EXC {e}")
            continue
        for link in rec["homepage"]["mfg_links"]:
            ...
            try:
                ...
                continue
        out[dom] = rec
        print(f"{dom}: OK mentions={rec['homepage']['mfg_mentions']} "
