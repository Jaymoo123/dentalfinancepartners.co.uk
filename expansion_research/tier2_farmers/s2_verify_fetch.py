"""R3 tier2 farmers — Stage 2: fetch-verify every surviving candidate domain.

Adapted from tier1_hospitality/s2_verify_fetch.py. Script gathers evidence; Claude judges.
...
FARM_LINK_RE = re.compile(
    r"farm|agricultur|rural|estate|landed|landowner|equine|dairy|livestock|arable", re.I)
FARM_TEXT_RE = re.compile(
    r"farm(?:er|ing|s)?|agricultur\w*|rural business|landed estate|rural estate|"
    r"herd basis|farmers'? averaging|agricultural property relief|APR|"
    r"diversification|landowner|smallholding|arable|dairy|livestock|equine|"
    r"tenant farmer|agricultural holdings|farm business tenancy", re.I)
...
def page_summary(html: str, url: str) -> dict:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if urlparse(full).netloc.replace("www.", "") == urlparse(url).netloc.replace("www.", "") \
                and FARM_LINK_RE.search(href):
            links.append(full)
    return {"url": url, "title": title, "meta": meta, "h1": h1, "h2": h2,
            "farm_excerpt": excerpt,
            "farm_mentions": len(FARM_TEXT_RE.findall(text)),
            "uk_signals": sorted(set(x.group(0) for x in UK_RE.finditer(text)))[:10],
            "farm_links": list(dict.fromkeys(links))[:8],
...
def main() -> None:
    ...
    for dom in cands:
        ...
        try:
            ...
            print(f"{dom}: EXC {e}")
            continue
        for link in rec["homepage"]["farm_links"]:
            ...
            try:
                ...
                continue
        out[dom] = rec
        print(f"{dom}: OK mentions={rec['homepage']['farm_mentions']} "
