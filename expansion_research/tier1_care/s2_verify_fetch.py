"""R3 tier1 care — Stage 2: fetch-verify every surviving candidate domain.
...
CARE_LINK_RE = re.compile(
    r"care-?home|domiciliary|home-?care|nursing-?home|residential-?care|supported-?living|"
    r"social-?care|care-?agenc|care-?provider|care-?sector|childrens?-?home|cqc|"
    r"live-?in-?care|healthcare|health-?and-?social", re.I)
CARE_TEXT_RE = re.compile(
    r"care home|domiciliary|home care|nursing home|residential care|supported living|"
    r"social care|care agenc|care provider|care sector|children'?s home|CQC|"
    r"live-?in care|sleep-?in|care operator|care business", re.I)
...
def page_summary(html: str, url: str) -> dict:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if urlparse(full).netloc.replace("www.", "") == urlparse(url).netloc.replace("www.", "") \
                and CARE_LINK_RE.search(href):
            links.append(full)
    return {"url": url, "title": title, "meta": meta, "h1": h1, "h2": h2,
            "care_excerpt": excerpt,
            "care_mentions": len(CARE_TEXT_RE.findall(text)),
            "uk_signals": sorted(set(x.group(0) for x in UK_RE.finditer(text)))[:10],
            "care_links": list(dict.fromkeys(links))[:8],
...
def main() -> None:
    ...
    for dom in cands:
        ...
        try:
            ...
            print(f"{dom}: EXC {e}")
            continue
        for link in rec["homepage"]["care_links"]:
            ...
            try:
                ...
                continue
        out[dom] = rec
        print(f"{dom}: OK mentions={rec['homepage']['care_mentions']} "
