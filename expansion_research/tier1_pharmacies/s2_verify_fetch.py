"""R3 tier1 pharmacies — Stage 2: fetch-verify every surviving candidate domain.
...
PHARM_LINK_RE = re.compile(
    r"pharmac|chemist|locum|dispensing|nhs|healthcare|medical|fp34|drug-tariff", re.I)
PHARM_TEXT_RE = re.compile(
    r"pharmac(?:y|ies|ist)|chemist|locum|dispensing|NHS contract|FP34|drug tariff|NHSBSA|"
    r"prescription|category m|owings|superintendent pharmacist|GPhC|Pharmacy First", re.I)
...
def page_summary(html: str, url: str) -> dict:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if urlparse(full).netloc.replace("www.", "") == urlparse(url).netloc.replace("www.", "") \
                and PHARM_LINK_RE.search(href):
            links.append(full)
    return {"url": url, "title": title, "meta": meta, "h1": h1, "h2": h2,
            "pharm_excerpt": excerpt,
            "pharm_mentions": len(PHARM_TEXT_RE.findall(text)),
            "uk_signals": sorted(set(x.group(0) for x in UK_RE.finditer(text)))[:10],
            "pharm_links": list(dict.fromkeys(links))[:8],
...
def main() -> None:
    ...
    for dom in cands:
        ...
        try:
            ...
            print(f"{dom}: EXC {e}")
            continue
        for link in rec["homepage"]["pharm_links"]:
            ...
            try:
                ...
                continue
        out[dom] = rec
        print(f"{dom}: OK mentions={rec['homepage']['pharm_mentions']} "
