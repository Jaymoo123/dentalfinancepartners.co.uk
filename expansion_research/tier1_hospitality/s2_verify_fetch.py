"""R3 tier1 hospitality — Stage 2: fetch-verify every surviving candidate domain.

Adapted from pilot_charities/s2_verify_fetch.py. Script gathers evidence; Claude judges.
...
HOSP_LINK_RE = re.compile(
    r"hospitality|restaurant|pub\b|pubs\b|bar[s]?\b|takeaway|take-away|hotel|guest-?house|"
    r"bed-and-breakfast|b-and-b|cafe|coffee-shop|catering|caterer|tronc|toms\b|food-and-drink|"
    r"food-and-beverage|leisure", re.I)
HOSP_TEXT_RE = re.compile(
    r"hospitality|restaurant|\bpubs?\b|\bbars?\b|takeaway|take-away|hotel|guest ?house|"
    r"bed and breakfast|caf[eé]|catering|caterer|tronc|chef|licensed trade|food and (?:drink|beverage)|"
    r"gross profit margin|wet sales|dry sales|coffee shop", re.I)
...
def main() -> None:
    ...
    for dom in cands:
        ...
        try:
            ...
            print(f"{dom}: EXC {e}")
            continue
        for link in rec["homepage"]["hosp_links"]:
            ...
            try:
                ...
                continue
        out[dom] = rec
        print(f"{dom}: OK mentions={rec['homepage']['hosp_mentions']} "
