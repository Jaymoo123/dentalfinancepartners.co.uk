"""R3 pilot charities — Stage 5: union topic pool + estate dedup gate.
...
JUNK_RE = re.compile(
    r"canada|\bnz\b|australia|ireland(?!.*northern)|\busa\b|american|india|jobs?\b|salary|"
    r"vacanc|course|degree|login|near me|software|quickbooks|xero\b|sage\b|moose jaw|"
    r"instagram|facebook|asdasd|photos|eagles|jesus christ|caf charity account", re.I)
...
def collect_pool() -> dict[str, dict]:
    ...
    def add(term: str, src: str, meta: dict | None = None) -> None:
        ...
        if len(term) < 6 or JUNK_RE.search(term):
            return
        # must be charity/nonprofit-scope relevant
        if not re.search(r"charit|cic\b|community interest|social enterprise|not[- ]for[- ]profit|"
                         r"non[- ]profit|gift aid|gasds|independent examin|sorp|trustee|cio\b|"
                         r"church|parish|donat|fundrais|volunteer|grant", term, re.I):
            return
